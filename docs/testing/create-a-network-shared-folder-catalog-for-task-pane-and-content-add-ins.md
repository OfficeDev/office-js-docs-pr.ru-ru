...
---
# <a name="sideload-office-add-ins-for-testing-from-a-network-share"></a>Загрузка неогрузки надстройки Office для тестирования из сетевой сети

Вы можете протестировать надстройку Office в клиенте Office, который находится в Windows, опубликуя манифест в сетевой папке (инструкции ниже). Этот вариант развертывания предназначен для использования после завершения разработки и тестирования на localhost и для тестирования надстройки с нелокализованного сервера или облачной учетной записи.

> [!IMPORTANT]
> Развертывание по сетевой доской не поддерживается для производственных надстройок. Этот метод имеет следующие ограничения:
> 
> - Надстройка может быть установлена только на компьютерах с Windows.
...
Эта статья относится только к тестированию надстройки Word, Excel, PowerPoint и Project и только в Windows. Для выполнения тестирования на другой платформе или тестирования надстроек Outlook см. одну из указанных ниже тем о загрузке неопубликованных надстроек.
...
8. Чтобы закрыть **диалоговое** окно **"Параметры",** выберите кнопку "ОК".
...
Вы можете удалить ранее загруженную неогруженную надстройку, с помощью очистки кэша Office на компьютере. Сведения о том, как очистить кэш в Windows, можно найти в статье ["Очистка кэша Office".](clear-cache.md#clear-the-office-cache-on-windows)
