...
Этот процесс поддерживается только **Excel**, **OneNote**, **PowerPoint** и **Word**. Инструкции по боковой загрузке вручную см. в следующем разделе. В этом примере предполагается, что вы используете проект, созданный с генератором [Yeoman для Office надстройки](https://github.com/OfficeDev/generator-office).
