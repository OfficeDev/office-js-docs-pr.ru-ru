...
Если вы используете новые функции, для которых требуются изменения файла манифеста, в Visual Studio могут возникнуть ошибки проверки. Например, при добавлении **\<Runtimes\>** элемента для реализации общей [](runtimes.md#shared-runtime)среды выполнения может появиться следующее сообщение об ошибке проверки.
...
- [Загрузка неопубликоваемой надстройки Office на Компьютере Mac](sideload-an-office-add-in-on-mac.md)  
- [Загрузка неопубликоваемой надстройки Office на iPad](sideload-an-office-add-in-on-ipad.md)  
