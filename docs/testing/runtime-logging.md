...
> [!NOTE]
> Функция ведения журнала в время работы в настоящее время доступна для Office 2016 или более поздней версии на настольном компьютере.
...
2. Добавьте раздел реестра `RuntimeLogging` в раздел `HKEY_CURRENT_USER\SOFTWARE\Microsoft\Office\16.0\WEF\Developer\`.
    ...
    > [!NOTE]
    > Если ключ (папка) еще не существует в папке, выполните `Developer` `HKEY_CURRENT_USER\SOFTWARE\Microsoft\Office\16.0\WEF\` следующие действия, чтобы создать его.
    >
...
2. Откройте приложение **Терминал** и настройте параметры ведения журнала в среде выполнения с помощью команды `defaults`:
    ...
    Установите одно из следующих значений, чтобы включить ведение журнала в времени работы `<bundle id>` для соответствующего приложения:
