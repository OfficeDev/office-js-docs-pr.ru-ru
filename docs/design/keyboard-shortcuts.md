---
title: Настраиваемые сочетания клавиш в Office надстроек
description: Узнайте, как добавлять в надстройку настраиваемые сочетания клавиш, также называемые сочетаниями клавиш, Office надстройки.
ms.date: 11/22/2021
...
ms.locfileid: "65752871"
---
# <a name="add-custom-keyboard-shortcuts-to-your-office-add-ins"></a>Добавление настраиваемых сочетаний клавиш в надстройки Office надстроек
...
Чтобы добавить настраиваемые сочетания клавиш, надстройка будет использовать общую среду выполнения. Дополнительные сведения см. в разделе "Настройка [надстройки для использования общей среды выполнения"](../develop/configure-your-add-in-to-use-a-shared-runtime.md).
...
После выполнения предыдущих действий надстройка может переключать видимость области задач, нажимая **клавиши CTRL+ALT+UP** и **CTRL+ALT+DOWN**. Такое же поведение показано в Excel [](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/excel-keyboard-shortcuts) сочетаниях клавиш в репозитории PnP Office надстроек в GitHub.
...
> [!NOTE]
> Подсказки клавиш, также известные как последовательные сочетания клавиш, такие как сочетание клавиш Excel для выбора цвета заливки **ALT+H, H**, не поддерживаются в Office надстройки.

## <a name="avoid-key-combinations-in-use-by-other-add-ins"></a>Избегайте сочетаний клавиш, используемых другими надстройки

Существует множество сочетаний клавиш, которые уже используются Office. Избегайте регистрации сочетаний клавиш для уже используемых надстроек, однако в некоторых случаях может потребоваться переопределить существующие сочетания клавиш или обработать конфликты между несколькими надстройкими, которые зарегистрировали одно и то же сочетание клавиш.
...
Пользователь может выбрать действие, которое будет выполнять сочетание клавиш. После выбора предпочтения сохраняются для дальнейшего использования того же ярлыка. Параметры ярлыка сохраняются для каждого пользователя на каждой платформе. Если пользователь хочет изменить свои настройки, он может вызвать команду "Сброс Office настройки  надстроек" из поля поиска помощника. Вызов команды очищает все параметры ярлыка надстройки пользователя, и при следующей попытке использования конфликтующего ярлыка пользователю снова будет предложено ввести диалоговое окно конфликта.

![Поле поиска помощника в Excel отображает действие сброса Office настройки ярлыка надстройки.](../images/add-in-reset-shortcuts-action.png)
...
- Если вам нужны дополнительные сочетания клавиш, просмотрите список [](https://support.microsoft.com/office/1798d9d5-842a-42b8-9c99-9b7213f0040f)Excel и не используйте их в надстройке.
...
- Если на компьютере Windows или Mac команда "Сбросить параметры ярлыка надстроек Office" недоступна в меню поиска, пользователь может вручную добавить команду на ленту, настроив ленту через контекстное меню.
...
В следующем примере ключ является `default` резервным ключом для любой платформы, которая не указана. Не указана только Windows, `default` поэтому ключ будет применяться только к Windows.
...
- См. [Excel примере](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/excel-keyboard-shortcuts) надстройки для сочетаний клавиш.
