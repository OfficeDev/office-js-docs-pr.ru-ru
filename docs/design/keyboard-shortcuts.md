...
1. Для основной части функции используйте метод [Office. AddIn. showTaskpane](/javascript/api/office/office.addin.md#showastaskpane--) , чтобы открыть область задач надстройки. После завершения код должен выглядеть следующим образом:
...
1. Добавьте второй вызов `Office.actions.associate` функции, чтобы сопоставить `HIDETASKPANE` действие с функцией, которая вызывает [Office. AddIn. Hide](/javascript/api/office/office.addin.md#hide--). Ниже приведен пример.
...
Полная схема для ярлыков JSON [extended-manifest.schema.jsвключена](https://developer.microsoft.com/en-us/json-schemas/office-js/extended-manifest.schema.json).
