...
Метод **рекуеступдате** также используется для переключения видимости настраиваемой контекстной вкладки. Подробные сведения об этом и примерах кода можно найти [в статье Включение и отключение команд надстроек](contextual-tabs.md#toggle-tab-visibility-and-the-enabled-status-of-a-button-at-the-same-time).
