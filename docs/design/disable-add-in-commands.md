...
> [!NOTE]
> Набор **требований RibbonApi 1.1** еще не поддерживается в манифесте, поэтому его нельзя указать в разделе **Требования манифеста.** Чтобы проверить поддержку, код должен вызвать `Office.context.requirements.isSetSupported('RibbonApi', '1.1')` . Если и *только в том* случае, если этот вызов возвращается, код может вызывать `true` API- включить или отключить. Если вызов `isSetSupported` возвращается, все настраиваемые команды надстройки включены `false` все время. Необходимо разработать производственную надстройка и все инструкции в приложении, чтобы учитывать, как она будет работать, если набор требований **RibbonApi 1.1** не поддерживается. Дополнительные сведения и примеры использования см. в Office приложениях и требованиях `isSetSupported` [API, особенно](../develop/specify-office-hosts-and-api-requirements.md)проверках времени запуска для поддержки набора методов и [требований.](../develop/specify-office-hosts-and-api-requirements.md#runtime-checks-for-method-and-requirement-set-support) (Раздел [Укажите, какие Office](../develop/specify-office-hosts-and-api-requirements.md#specify-which-office-versions-and-platforms-can-host-your-add-in) и платформы могут вмещая надстройка этой статьи, не применяется к Ленте 1.1.)
...
3. В зависимости от того, содержит ли надстройка области задач, файл функций или настраиваемую Excel, необходимо сделать один или несколько из следующих трех действий.
    ...
    - Если надстройка содержит области задач, установите `resid` атрибут [Действия.](../reference/manifest/action.md)[ Элемент SourceLocation](../reference/manifest/sourcelocation.md) в точно такой же строке, как и для элемента `resid` **Runtime** на шаге 1; например, `Contoso.SharedRuntime.Url` . Элемент должен выглядеть следующим образом:`<SourceLocation resid="Contoso.SharedRuntime.Url"/>`.
    - Если надстройка содержит Excel настраиваемую функцию, установите `resid` атрибут [Страницы](../reference/manifest/page.md).[ Элемент SourceLocation](../reference/manifest/sourcelocation.md) точно такой же строки, как и для элемента `resid` **Runtime** на шаге 1; например, `Contoso.SharedRuntime.Url` . Элемент должен выглядеть следующим образом:`<SourceLocation resid="Contoso.SharedRuntime.Url"/>`.
    - Если надстройка содержит файл функции, установите атрибут элемента FunctionFile точно такой же строкой, как вы использовали для элемента Runtime на `resid` [](../reference/manifest/functionfile.md) `resid` шаге 1; например,  `Contoso.SharedRuntime.Url` . Элемент должен выглядеть следующим образом:`<FunctionFile resid="Contoso.SharedRuntime.Url"/>`.
