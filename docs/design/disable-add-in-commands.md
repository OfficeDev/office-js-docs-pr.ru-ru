...
API enable/disable относятся к набору требований [RibbonApi 1.1](/javascript/api/requirement-sets/common/ribbon-api-requirement-sets) .
...
1. В элементе манифеста [Runtimes](/javascript/api/manifest/runtimes) добавьте следующий дочерний элемент: `<Runtime resid="Contoso.SharedRuntime.Url" lifetime="long" />`. (Если в манифесте еще нет элемента **Runtimes** , создайте его в качестве первого ребенка в **элементе Host** в разделе **VersionOverrides** .)
...
3. В зависимости от того, содержит ли надстройка области задач, файл функций или настраиваемую Excel, необходимо сделать один или несколько из следующих трех действий.
    ...
    - Если надстройка содержит области задач, установите `resid` атрибут [Действия](/javascript/api/manifest/action).[ Элемент SourceLocation](/javascript/api/manifest/sourcelocation) в точно такой же `resid` строке, как и для элемента **Runtime** на шаге 1; например, `Contoso.SharedRuntime.Url`. Элемент должен выглядеть следующим образом:`<SourceLocation resid="Contoso.SharedRuntime.Url"/>`.
    - Если надстройка содержит Excel настраиваемую функцию, `resid` установите атрибут [Страницы](/javascript/api/manifest/page).[ Элемент SourceLocation](/javascript/api/manifest/sourcelocation) точно такой же строки`resid`, как и для элемента **Runtime** на шаге 1; например, `Contoso.SharedRuntime.Url`. Элемент должен выглядеть следующим образом:`<SourceLocation resid="Contoso.SharedRuntime.Url"/>`.
    - Если надстройка содержит файл функции, `resid` установите атрибут элемента [FunctionFile](/javascript/api/manifest/functionfile) `resid` точно такой же строкой, как вы использовали для элемента **Runtime** на шаге 1; например, `Contoso.SharedRuntime.Url`. Элемент должен выглядеть следующим образом:`<FunctionFile resid="Contoso.SharedRuntime.Url"/>`.
