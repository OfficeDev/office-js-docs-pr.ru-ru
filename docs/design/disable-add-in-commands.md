...
API и разметка манифеста, описанные в этой статье, требуют, чтобы манифест надстройки указал, что она должна использовать [общую среду выполнения](../testing/runtimes.md#shared-runtime). Для этого выполните следующие действия.
...
Во-вторых, назначьте обработчиков. Обычно это делается в функции **Office.onReady** , как в следующем примере, который назначает обработчики (созданные на следующем шаге) **событиям onActivated** и **onDeactivated** всех диаграмм на листе.
