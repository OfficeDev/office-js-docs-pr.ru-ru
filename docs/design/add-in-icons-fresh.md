...
![Пример рекомендации по изменению значков на уровне, а не при уменьшении значков. Например, может потребоваться уменьшить число элементов в маленьком значке, а не просто увеличить изображение.](../images/icon-resizing.png)
...
Значки Office обычно состоят из базового элемента с перекрывающимися действиями и абстрактными модификаторами. Модификаторы действия представляют такие понятия, как "добавить", "открыть", "создать" или "закрыть". Концептуальные модификаторы представляют состояние, изменение или описание значка.
...
- Ограничивайте количество элементов в значках. На 32 точек Ограничьте количество модификаторов до двух. В 16 ПКС Ограничьте количество модификаторов до 1.
...
Все модификаторы должны иметь однопикселный прозрачный отрезки между каждым элементом, включая фон. Элементы не должны сильно перекрывать друг друга. Создайте пустое пространство между метками и границами. Размеры модификаторов могут немного отличаться, но используйте указанные ниже числа в качестве отправной точки.

|Размер значка|Отступ вокруг базового элемента|Размер модификатора|
|:---|:---|:---|
|16 точек|нуль|9 точек|
|20 пк;|1 пк|10 пкс|
|24 пк;|1 пк|12 точек|
|32 точек|2 пк|14 пкс|
|40 пк;|2 пк|20 пк;|
|48 пк;|3 пк|22 ПКС|
|64 точек|5 пк|29 точек|
|80 точек|5 пк|38 точек|
...
- Используйте цвета единообразно для всех размеров значков. Цветовые палитры для значков Office незначительно отличаются в зависимости от размера значков. 16 точек и мелкие значки немного темнее и более насыщенны, чем 32 пикселов и более крупные значки. Если не учитывать эти незначительные корректировки, цвета могут варьироваться в зависимости от размеров значка.
...
|Темно-серый (32)|128, 128, 128|#808080| ![Темно-серый цвет для 32 точек и больше](../images/color-dark-gray-32.png) |32 точек и выше|
|умеренно серый (32)|158, 158, 158|#9E9E9E| ![Средний серый цвет для 32 px и выше](../images/color-medium-gray-32.png) |32 точек и выше|
...
|Темно-серый (16)|114, 114, 114|#727272| ![Темно-серый цвет для 16 точек и меньше](../images/color-dark-gray-16.png) |16 точек и ниже|
|Умеренно серый (16)|144, 144, 144|#909090| ![Средний серый цвет для 16 точек и меньше](../images/color-medium-gray-16.png) |16 и меньше|
|Синий (32)|77, 130, 184|#4d82B8| ![Синий цвет для 32 px и выше](../images/color-blue-32.png) |32 точек и выше|
|Синий (16)|74, 125, 177|#4A7DB1| ![Синий цвет для 16 точек и меньше](../images/color-blue-16.png) |16 точек и ниже|
|Желтый (ВСЕ)|234, 194, 130|#EAC282| ![Желтый цвет для всех размеров изображения](../images/color-yellow-all.png) |Все размеры|
|Оранжевый (32)|231, 142, 70|#E78E46| ![Оранжевый цвет для 32 точек и больше](../images/color-orange-32.png) |32 точек и выше|
|Оранжевый (16)|227, 142, 70|#E3751C| ![Оранжевый цвет для 16 точек и меньше](../images/color-orange-16.png) |16 точек и ниже|
|Розовый (ВСЕ)|230, 132, 151|#E68497| ![Розовый цвет для всех размеров изображения](../images/color-pink-all.png) |Все размеры|
|Зеленый (32)|118, 167, 151|#76A797| ![Зеленый цвет для 32 точек и больше](../images/color-green-32.png) |32 точек и выше|
|Зеленый (16)|104, 164, 144|#68A490| ![Зеленый цвет для 16 точек и меньше](../images/color-green-16.png) |16 точек и ниже|
|Красный (32)|216, 99, 68|#D86344| ![Красный цвет для 32 точек и больше](../images/color-red-32.png) |32 точек и выше|
|Красный (16)|214, 85, 50|#D65532| ![Красный цвет для 16 точек и меньше](../images/color-red-16.png) |16 точек и ниже|
|Сиреневый (32)|152, 104, 185|#9868B9| ![Сиреневый цвет для 32 точек и больше](../images/color-purple-32.png) |32 точек и выше|
|Сиреневый (16)|137, 89, 171|#8959AB| ![Сиреневый цвет для 16 точек и меньше](../images/color-purple-16.png) |16 точек и ниже|
