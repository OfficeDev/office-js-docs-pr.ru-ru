...
По умолчанию, если надстройка пытается перейти на URL-адрес в домене, отличном от домена, где размещена начальная страница (указан в элементе [SourceLocation](../../reference/manifest/sourcelocation.md) файла манифеста), этот URL-адрес откроется в новом окне браузера, а не в области надстроек ведущего приложения Office. Это поведение по умолчанию защищает пользователя от непредвиденной навигации по страницам в области надстроек из-за встроенных элементов **iframe**.

Чтобы переопределить это поведение, добавьте каждый домен, который требуется открыть в окне надстройки, в список доменов в элементе [AppDomains](../../reference/manifest/appdomains.md) файла манифеста. Если надстройка пытается перейти по URL-адресу домена, не входящего в список, то страница откроется в новом окне браузера (не в области надстроек).
