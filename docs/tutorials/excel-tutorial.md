...
- Чтобы протестировать надстройку в этом руководстве, необходимо подключиться к Интернету.
...
9. Замените `TODO5` приведенным ниже кодом. Примечание:
...
2. Выполните команду `npm run build` , чтобы преES6 исходный код в более раннюю версию JavaScript, поддерживаемую Internet Explorer (которая используется в некоторых версиях Excel для запуска надстроек Excel).
...
7. Замените `TODO1` приведенным ниже кодом. Обратите внимание на то, что для исключения строки заголовков в коде вместо метода `Table.getDataBodyRange` используется метод `getRange`, чтобы получить нужный диапазон данных для диаграммы.
