...
5. В вызове `Office.onReady` метода Расположите строку, которая назначает обработчик нажатия `freeze-header` кнопки, и добавьте следующий код после этой строки. Вы создадите метод `openDialog` на одном из следующих этапов.
