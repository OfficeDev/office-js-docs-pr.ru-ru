...
3. Найдите элемент `<main>` и удалите все строки, расположенные после открывающего тега `<main>` и перед закрывающим тегом `</main>`.

4. Добавьте следующую разметку сразу после открывающего тега `<main>`:
...
1. Выполните следующие действия, чтобы запустить локальный веб-сервер и загрузить неопубликованную надстройку.
    ...
    - Чтобы проверить надстройку в Excel, выполните следующую команду в корневом каталоге своего проекта. При этом запускается локальный веб-сервер (если он еще не запущен) и открывается приложение Excel с загруженной надстройкой.

        ```command&nbsp;line
        npm start
        ```

    - Чтобы проверить надстройку в Excel в Интернете, выполните следующую команду в корневом каталоге своего проекта. При ее выполнении будет запущен локальный веб-сервер (если он еще не запущен).
...
4. Нажмите кнопку **Создать диаграмму**. Будет создана диаграмма, включающая только данные из отфильтрованных строк. Метки точек данных в нижней части диаграммы отсортированы согласно заданному для нее порядку, то есть по именам продавцов в обратном алфавитном порядке.
...
2. Найдите элемент `<Control>`. Этот элемент определяет кнопку **Показать область задач** на вкладке **Главная**, которую вы используете для запуска надстройки. Мы добавим вторую кнопку в эту же группу на ленте **Главная**. Добавьте следующий код между закрывающим тегом `</Control>` и закрывающим тегом `</Group>`.
...
4. Следующие три параметра `TODO` устанавливают идентификаторы ресурсов или `resid`. Сейчас вам нужно присвоить идентификаторы ресурсам. Кнопка должна называться "Переключение защиты", но у строки должен быть *идентификатор* "ProtectionButtonLabel", поэтому элемент `Label` выглядит следующим образом:
...
6. Для элемента `Action` в исходном элементе `Control` задан тип `ShowTaskpane`, но новая кнопка не будет открывать область задач. Она будет выполнять специальную функцию, которую вы создадите позже. Поэтому замените `TODO5` на `ExecuteFunction` (тип действия для кнопок, запускающих специальные функции). Открывающий тег элемента `Action` должен выглядеть следующим образом:
...
### <a name="add-code-to-fetch-document-properties-into-the-task-panes-script-objects"></a>Добавление кода для получения свойств документа в объекты сценария области задач
...
2. Очистите кэш Office, удалив содержимое (все файлы и вложенные папки) папки кэша. Это необходимо для полного удаления старой версии надстройки из клиентского приложения.
...
3. Добавьте в файл **popup.html** следующий код. Примечание.
   ...
   - На странице есть поле `<input>`, где пользователь будет вводить свое имя, и кнопка, при нажатии которой это имя будет отправлено в область задач, где оно будет отображаться.

   - Код загружает сценарий под названием **popup.js**, который будет создан на более позднем этапе.
