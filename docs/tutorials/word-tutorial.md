...
9. Замените `TODO1` в `insertParagraph()` функции приведенный ниже код. Примечание.
...
1. Выполните указанные ниже действия, чтобы запустить локальный веб-сервер и загрузить неопубликованную надстройку.
    ...
    - Чтобы протестировать надстройку в Word в Интернете, выполните следующую команду в корневом каталоге проекта. После выполнения этой команды запустится локальный веб-сервер (если он еще не запущен).
...
6. Замените `TODO1` в `insertTextIntoRange()` функции приведенный ниже код. Обратите внимание:
