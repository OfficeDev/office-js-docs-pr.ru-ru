...
- Элемент **\<Group\>** определяет группу новых кнопок, а ресурс **groupLabel** задает подпись.
