...
Если вы создаете надстройки Office для запуска в Word, возможно, вы уже знаете, что API JavaScript для Office (Office. js) предлагает несколько форматов для чтения и записи контента документа. Они называются типами приведения и содержат простой текст, таблицы, HTML-код и Office Open XML.
...
> [!NOTE]
> Если вы добавите приведенную здесь разметку в XML-файл вместе с тегами объявления XML для Version и MSO-Application в начале файла (показан на рисунке 13), вы можете открыть его в Word как документ Word. Кроме того, без этих тегов можно открыть файл с помощью **> открыть** в Word. **Режим совместимости** отображается в строке заголовка в Word, так как вы удалили параметры, которые определяют Word как документ Word. Так как вы добавляете эту разметку в существующий документ Word, это не повлияет на содержимое.
...
В этой функции Обратите внимание, что все, кроме последней строки, используются для получения сохраненной разметки для использования в вызове метода [setSelectedDataAsync](/javascript/api/office/office.document#setselecteddataasync-data--options--callback-) в конце функции. `setSelectedDataASync` требуется только указать, что нужно вставить содержимое, и тип приведения.
...
- Открывающий тег **w:document** включает несколько пространств имен (**xmlns**). Многие из них связаны с определенными типами содержимого и необходимы, только если относятся к вашему содержимому.
    ...
    Обратите внимание, что префикс для тегов в части документа ссылается обратно на пространства имен. В этом примере единственным префиксом, используемым в тегах в части Document. XML, является **w:**, поэтому единственное пространство имен, которое нужно оставить в открывающем теге **в:документ** , — **xmlns: w**.
...
- **Псевдоним** это свойство Title в диалоговом окне Свойства элемента управления контентом в Word. Это обязательное свойство (представляющее имя элемента), если вы планируете привязывать его к элементу управления в надстройке.
...
- Атрибут **Appearance** используется для скрытия контейнера элемента управления для более четкого внешнего вида. Эта функция появилась в Word 2013, как видно с помощью пространства имен W15. Так как это свойство используется, пространство имен W15 сохраняется в начале части Document. XML.

- Атрибут **шовингплчдр** — это необязательный параметр, который задает содержимое по умолчанию, которое включается в элемент управления (текст в этом примере) как содержимое заполнителя. Таким образом, если пользователь нажимает или щелкает мышью в области управления, выделяется весь контент, а не то, как редактируемое содержимое, в котором пользователь может вносить изменения.
...
- Элемент управления содержимым должен иметь имя (это поле " **название** " в диалоговом окне Свойства элемента управления содержимым, которое соответствует тегу **псевдонима** в разметке Office Open XML). В этом примере кода указывается, где следует разместить привязку.
...
После выполнения кода, если вы изучите разметку документа, в котором созданная надстройка привязывается, вы увидите две части каждой привязки. В разметке для элемента управления содержимым, в котором была добавлена привязка (в Document. XML), вы увидите атрибут **W15: вебекстенсионлинкед/**.
...
> [!NOTE]
> При просмотре разметки обратите внимание на дополнительные пространства имен, используемые в теге а:блип. Вы увидите в документе Document. XML, который **кслмнс:** пространство имен (основное пространство имен DrawingML) динамически размещается в начале использования ссылок DrawingML, а не в начале части Document. XML. Однако пространство имен связей (r) должно храниться там, где он отображается в начале документа. XML. Проверьте разметку изображения на наличие дополнительных требований к пространству имен. Помните о том, что вам не нужно знать, какие типы контента требуют пространств имен, вы можете легко узнать, как проверить префиксы тегов во всем документе Document. XML.
...
Обратите внимание, что разметка для схемы SmartArt в документе Document. XML содержит ссылки на идентификатор связи в части "макет", "данные", "цвета" и "Экспресс-стили". Вы можете удалить ссылки из файла Document. XML в части цвета и стили при удалении этих частей и их определений, и это, по крайней мере, является оптимальным действием, так как вы удаляете эти отношения, но не получится об ошибке, так как они не нужны для вставки схемы в документ. Найдите эти ссылки в файле document. XML в теге **DGM: релидс** . Независимо от того, будет ли это действие выполнено, сохраните ссылки на идентификатор связи для требуемой структуры и частей данных.
...
- [Обзор API JavaScript для Office: привязка данных и пользовательские XML-части](https://msdn.microsoft.com/magazine/dn166930.aspx)
