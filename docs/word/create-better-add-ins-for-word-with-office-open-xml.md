...
Если вы строите надстройки Office для работы в Word, возможно, вы уже знаете, что API JavaScript для Office (Office.js) предлагает несколько форматов для чтения и записи содержимого документа. Они называются типами приведения, и к ним относятся обычный текст, таблицы, HTML и Office Open XML.
...
> [!NOTE]
> В этой статье термины  "типы  контента" и "богатый контент" относятся к типам содержимого, которое можно вставить в документ Word.
...
Office предлагает широкий спектр макетов схем SmartArt (и вы можете использовать Office Open XML для создания собственных).
...
Предположим, вам нужно вставить всего лишь абзац текста с прямым форматированием, как показано на рисунке 1. При захвате Форматированный текст в Формате Office Open XML вы увидите большой объем  `getSelectedDataAsync` разметки. Она включает элемент package, который представляет весь документ и содержит несколько частей (обычно называемые частями документа или, в Office Open XML, частями пакета), как показано на рисунке 13. Каждая часть представляет отдельный файл в пакете.

> [!TIP]
> Вы можете редактировать разметку Office Open XML в текстовом редакторе, например в Блокноте. Если открыть его в Visual Studio, вы можете использовать редактор >**Advanced > Format Document** (CTRL+K, CTRL+D), чтобы отформатировать пакет для упростить редактирование. Then you can collapse or expand document parts or sections of them, as shown in Figure 12, to more easily review and edit the content of the Office Open XML package. Each document part begins with a **pkg:part** tag.
...
> [!NOTE]
> Если добавить показанную здесь разметку в XML-файл вместе с тегами объявления XML для версии и mso-application в верхней части файла (показано на рисунке 13), вы можете открыть его в Word как документ Word. Кроме того, без этих тегов вы можете открыть его с помощью файла> **Открыть** в Word. Вы увидите **режим** совместимости в заголовке заголовка в Word, так как вы удалили параметры, которые сообщите Word, что это документ Word. Так как вы добавляете эту разметку в существующий документ Word, это не повлияет на содержимое.
...
- Открывающий тег **w:document** включает несколько пространств имен (**xmlns**). Многие из них связаны с определенными типами содержимого и необходимы, только если относятся к вашему содержимому.
    ...
    Обратите внимание, что префикс для тегов в части документа относится к пространствам имен. В этом примере единственным префиксом, используемым в тегах в части document.xml, является **w:**, поэтому единственное пространство имен, которое необходимо оставить в открываемом теге **w:document,** **— xmlns:w**.
...
- Псевдоним **— это свойство** заголовка в диалоговом окне "Свойства управления содержимым" в Word. Данное свойство (представляющее имя элемента) является обязательным, если вы планируете привязать элемент управления в надстройке.
...
- Атрибут **внешнего** вида используется для скрытие контейнера управления для более простого вида. Эта функция была впервые представлена в Word 2013, как видно по использованию пространства имен w15. Так как это свойство используется, пространство имен w15 сохраняется в начале части document.xml.

- Атрибут **showingPlcHdr** является необязательным параметром, который задает содержимое по умолчанию, которое вы включаете в этот контроль (текст в этом примере) в качестве содержимого-замещений. Поэтому если пользователь щелкнет область элемента управления, будет выбрано все содержимое, что отличается от редактируемого содержимого.
...
- У управления содержимым должно быть  имя (это поле "Заголовок" в диалоговом окне "Свойства управления содержимым", соответствующее тегу **"Псевдоним"** в разметке Office Open XML). Так код определяет, где следует разместить привязку.
...
- При первом вызове метода метод проверяет состояние результата, чтобы узнать, не удалось ли получить привязку, так как именоватый элемент не существует в документе (т. е. элемент управления содержимым с именем `addFromNamedItemAsync` MyContentControlTitle в этом примере). Если да, код добавляет этот контроль в активную точку выбора (с помощью) и `setSelectedDataAsync` затем привязывает его.
...
Если вы посмотрите на разметку документа, в котором надстройка создала привязки, после выполнения кода, то увидите две части для каждой привязки. В разметке для управления содержимым, в который была добавлена привязка (в document.xml), вы увидите атрибут **w15:webExtensionLinked/**.
...
Как и `setSelectedDataAsync` при вводе, вы указываете вставляемый контент и тип приведите. Единственное дополнительное требование для записи в привязку — определение привязки по идентификатору. Обратите внимание, что идентификатор привязки, используемый в этом примере (bindings#myBinding), соответствует идентификатору (myBinding), заданному при создании в предыдущей функции.
...
> [!NOTE]
> При изучении разметки обратите внимание на дополнительные пространства имен, используемые в теге a:blip. В document.xml вы увидите, что пространство имен **xlmns:a** (основное пространство имен drawingML) динамически размещено в начале использования ссылок drawingML, а не в верхней document.xml части. Однако пространство имен связей (r) необходимо сохранить в начале части document.xml. Проверьте разметку изображения на наличие дополнительных пространств имен. Remember that you don't have to memorize which types of content require what namespaces, you can easily tell by reviewing the prefixes of the tags throughout document.xml.
...
- [Изучение API JavaScript для Office: привязка данных и настраиваемые XML-части](/archive/msdn-magazine/2013/april/microsoft-office-exploring-the-javascript-api-for-office-data-binding-and-custom-xml-parts)