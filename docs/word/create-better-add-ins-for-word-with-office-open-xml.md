...
Если вы создаете надстройки Office для запуска в Word, возможно, вы уже знаете, что API JavaScript для Office (Office.js) предлагает несколько форматов для чтения и записи содержимого документа. Они называются типами приведения, и к ним относятся обычный текст, таблицы, HTML и Office Open XML.

Итак, что вы можете сделать, если вам нужно добавить в документ форматированный контент, например изображения, форматированные таблицы, диаграммы или просто форматированный текст? Начните с API, доступных в наборах обязательных элементов [WordApi](/javascript/api/requirement-sets/word/word-api-requirement-sets) , чтобы узнать, предоставляют ли они необходимые данные. Пример см. в фрагменте кода " [Вставка форматированного текста](https://github.com/OfficeDev/office-js-snippets/blob/prod/samples/word/25-paragraph/insert-formatted-text.yaml) " в [лаборатории](https://appsource.microsoft.com/product/office/wa104380862) сценариев в Word.

Если API еще не доступны, вы можете использовать HTML для вставки некоторых типов содержимого, например рисунков. В зависимости от ситуации у HTML-приведения могут быть недостатки, такие как ограниченные возможности форматирования и расположения контента.

Поскольку документы Word (DOCX и DOTX) написаны на языке Office Open XML, вы можете вставить практически любой тип контента, который пользователь может добавить в документ Word, используя практически любой тип форматирования. Определить разметку Office Open XML, необходимую для этого, проще, чем вы думаете.

> [!NOTE]
> Office Open XML — это также язык документов PowerPoint и Excel (а начиная с Office 2013 — и документов Visio). However, currently, you can coerce content as Office Open XML only in Office Add-ins created for Word. Дополнительные сведения об Office Open XML, включая полную справочную документацию по языку, см. в [разделе "См. также](#see-also) ".

Для начала ознакомьтесь с некоторыми типами содержимого, которые можно вставить с помощью приведения Office Open XML.
Скачайте пример кода [Load и write Open XML в](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) надстройке Word, которая содержит разметку Office Open XML и код Office.js, необходимый для вставки любого из следующих примеров в Word.

> [!NOTE]
> В этой статье термины "типы контента" и  "форматированный контент" относятся к типам расширенного содержимого, которое можно вставить в документ Word.
...
Используйте прямое форматирование, чтобы точно указать, как будет выглядеть текст независимо от существующего форматирования в документе пользователя.
...
Используйте стиль, чтобы автоматически координировать внешний вид текста, вставляемого с документом пользователя.

*Рис. 3. Пример изображения.*

![Пример логотипа.](../images/office15-app-create-wd-app-using-ooxml-fig03.png)

Используйте тот же метод для вставки любого формата изображения, поддерживаемого Office.
...
Используйте элементы управления содержимым надстройки для добавления содержимого в указанном (привязанном) расположении, а не в выбранном месте.
...
Вставка встроенных или настраиваемых фигур рисования с текстом и эффектами форматирования или без них.
...
Включите форматирование текста, границы, заливку, размер ячейки или любое необходимое форматирование таблицы.
...
Используйте встроенные или пользовательские стили таблиц так же легко, как и стиль абзаца для текста.

*Рис. 10. Диаграмма SmartArt*

![Динамичная диаграмма SmartArt в Word.](../images/office15-app-create-wd-app-using-ooxml-fig10.png)

Office предлагает широкий спектр макетов схем SmartArt (и вы можете использовать Office Open XML для создания собственных элементов).

*Рис. 11. Диаграмма*

![Диаграмма в Word.](../images/office15-app-create-wd-app-using-ooxml-fig11.png)

Вы можете вставлять диаграммы Excel в виде динамических диаграмм в документы Word. Это также означает, что вы можете использовать их в надстройке для Word.
Как показано в предыдущих примерах, вы можете использовать приведение Office Open XML для вставки контента любого типа, который пользователь может вставить в собственный документ.
Существует два простых способа получить нужную разметку Office Open XML. Добавьте расширенное содержимое в пустой документ Word, а затем сохраните его в формате документа Word XML или используйте тестовую надстройку с методом [getSelectedDataAsync](/javascript/api/office/office.document#office-office-document-getselecteddataasync-member(1)) , чтобы получить разметку. Оба подхода дают одинаковые результаты.

> [!NOTE]
> Документ Office Open XML фактически представляет собой сжатый пакет файлов, представляющих содержимое документа. Сохранение файла в формате документа Word XML позволяет преобразовать весь пакет Office Open XML в один XML-файл, `getSelectedDataAsync` который также можно получить при извлечении разметки Office Open XML.
...
В этом разделе вы будете использовать некоторые распространенные сценарии, которые мы получили от сообщества разработчиков надстроек Office, чтобы показать вам методы упрощения Office Open XML для использования в надстройке. Мы рассмотрим разметку для некоторых типов содержимого, показанных выше, а также сведения, необходимые для минимизации полезных данных Office Open XML. Мы также рассмотрим код, необходимый для вставки мультимедиа в документ в выделенном месте, и использование Office Open XML с объектом bindings для добавления или замены содержимого в указанных местах.

## <a name="explore-the-office-open-xml-document-package"></a>Изучение пакета документов Office Open XML

При использовании [getSelectedDataAsync](/javascript/api/office/office.document#office-office-document-getselecteddataasync-member(1)) для извлечения office Open XML для выбора содержимого (или при сохранении документа в формате Документа Word XML) вы получаете не только разметку, которая описывает выбранное содержимое. Это весь документ с множеством параметров и параметров, которые почти наверняка не нужны. Если вы используете этот метод из документа, содержащего надстройку области задач, полученная разметка будет включать даже область задач.
...
Предположим, вам нужно вставить всего лишь абзац текста с прямым форматированием, как показано на рисунке 1. При использовании форматированного текста в Office Open XML  `getSelectedDataAsync`вы увидите большой объем разметки. Она включает элемент package, который представляет весь документ и содержит несколько частей (обычно называемые частями документа или, в Office Open XML, частями пакета), как показано на рисунке 13. Каждая часть представляет отдельный файл в пакете.

> [!TIP]
> Измените разметку Office Open XML в текстовом редакторе, например в Блокноте. Если вы откроете его в Visual Studio, используйте >**Advanced > Format Document** (CTRL+K, CTRL+D), чтобы отформатировать пакет для упрощения редактирования. Then you can collapse or expand document parts or sections of them, as shown in Figure 12, to more easily review and edit the content of the Office Open XML package. Each document part begins with a **pkg:part** tag.
...
Для некоторых других типов содержимого, показанных в начале этого раздела, также требуются дополнительные части (кроме показанных на рис. 13), и вы ука добавите их позже в этом разделе. Сейчас же, так как вы увидите большинство частей, показанных на рисунке 13, в разметке любого пакета документа Word, кратко рассмотрим каждую часть и ее функции:
...
- В примере на рис. 1 непосредственно применяется форматирование текста (т. е. каждый параметр форматирования шрифта и абзаца применяется по отдельности). Но если вы используете стиль (например, если вы хотите, чтобы текст автоматически влиял на форматирование стиля "Заголовок 1" в конечном документе), как показано ранее на рис. 2, вам потребуется часть styles.xml части, а также определение связи для него. Дополнительные сведения см. в разделе "Добавление [объектов, использующих дополнительные части Office Open XML"](#add-objects-that-use-additional-office-open-xml-parts).

## <a name="insert-document-content-at-the-selection"></a>Вставка содержимого документа при выделении
...
Вы изменили приведенный здесь пример Office Open XML, как описано в предыдущем разделе, чтобы оставить только необходимые части документа и только необходимые элементы в каждой из этих частей. Вы узнаете, как изменить разметку самостоятельно (и подробнее об оставшихся фрагментах) в следующем разделе темы.
...
После сохранения предыдущего файла Office Open XML в виде XML-файла, доступного из вашего решения, используйте следующую функцию, чтобы задать форматированное текстовое содержимое в документе с помощью приведения Office Open XML.

В этой функции все строки, кроме последней, используются для получения сохраненной разметки и ее использования в методе [setSelectedDataAsync](/javascript/api/office/office.document#office-office-document-setselecteddataasync-member(1)) в конце функции. `setSelectedDataASync` требует только указания вставляемого содержимого и типа приведения.

> [!NOTE]
> *Замените yourXMLfilename* именем и путем XML-файла, так как вы сохранили его в решении. Если вы не знаете, куда включить XML-файлы в решение или как ссылаться на них в коде, см. пример кода "Загрузить и написать [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) " в примере кода надстройки Word, а также рабочий пример разметки и JavaScript, показанный здесь.
...
## <a name="create-your-own-markup-best-practices"></a>Создание собственной разметки: рекомендации

Рассмотрим разметку, необходимую для вставки предыдущего примера форматированного текста, более подробно.

Для начала просто удалим из пакета все части документа, кроме RELS-файла и document.xml. Затем вы измените эти две необходимые части, чтобы упростить процесс.
...
Так document.xml является основной частью документа, в которую вы поместите содержимое, ознакомьтесь с этой частью. (На рисунке 14, который идет после этого списка, показано, как некоторые базовые теги содержимого и теги форматирования связаны с тем, что вы видите в документе Word.)
...
- Открывающий тег **w:document** включает несколько пространств имен (**xmlns**). Многие из них связаны с определенными типами содержимого и необходимы, только если относятся к вашему содержимому.
    ...
    Обратите внимание, что префикс для тегов в части документа относится к пространствам имен. В этом примере единственным префиксом, используемым в тегах во всей части document.xml, является **w:**, поэтому единственным пространством имен, которое необходимо оставить в открывающего тега **w:document** , является **xmlns:w**.
...
1. В диалоговом окне "Параметры Word" выберите пункт **Центр управления безопасностью** и нажмите кнопку **Параметры центра управления безопасностью**.
1. В диалоговом окне "Центр управления безопасностью" выберите пункт **Параметры конфиденциальности** и снимите флажок **Сохранять случайное число для улучшения точности объединения**.

Чтобы удалить теги RSID из существующего документа, попробуйте использовать следующий ярлык, открыв документ в Office Open XML.

1. Указав точку вставки в тексте документа, нажмите клавиши **CTRL+HOME**, чтобы перейти в начало документа.
1. Нажмите клавиши **ПРОБЕЛ**, **DELETE**, **ПРОБЕЛ**. Затем сохраните документ.
...
## <a name="use-the-same-office-open-xml-structure-for-different-content-types"></a>Использование одной и той же структуры Office Open XML для различных типов контента
...
Чтобы просмотреть разметку Office Open XML для примеров каждого из этих типов контента, показанных ранее на рисунках 5–8, изучите пример кода "Загрузка и запись [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) " в примере кода надстройки Word, на который ссылаются в разделе обзора.

Прежде чем перейти к этой теме, обратите внимание на различия для нескольких из этих типов контента и способы замены нужных элементов.

### <a name="understand-drawingml-markup-office-graphics-in-word-what-are-fallbacks"></a>Общие сведения о разметке drawingML (графики Office) в Word: что такое резервирование?

Если разметка фигуры или текстового поля выглядит гораздо сложнее, чем ожидалось, есть причина. После выхода Office 2007 появились форматы Office Open XML, а также новый графический модуль Office, который был полностью интегрирован в PowerPoint и Excel. В Word 2007 поддерживается только часть этого модуля, обновленный модуль диаграмм Excel, графические объекты SmartArt и расширенные средства работы с рисунками. Для фигур и текстовых полей Word 2007 продолжает использовать старые объекты рисунков (VML). Только в Word 2010 были реализованы дополнительные возможности для поддержки обновленных средств работы с фигурами и рисунками.
...
Как правило, как показано в примерах фигуры и текстового поля, включенных в пример кода "Загрузка и запись [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) " в примере кода надстройки Word, резервную разметку можно удалить. Word автоматически добавляет отсутствующие резервные разметки к фигурам при сохранении документа. Но если вы хотите сохранить резервную разметку для поддержки всех пользовательских сценариев, никто не помешает вам это сделать.
...
В примерах кода загрузка и запись [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в надстройке Word и [Word-Add-in-Get-Set-EditOpen-XML](https://github.com/OfficeDev/Word-Add-in-Get-Set-EditOpen-XML) текстовое поле и фигура настраиваются с использованием различных типов параметров упаковки и размещения текста. (Кроме того, помните, что для изображений в этих примерах кода используется форматирование "в тексте", которое размещает графический объект на опорной линии текста.)
...
### <a name="work-with-content-controls"></a>Работа с элементами управления содержимым
...
Также имейте в виду, что привязка данных XML и Office.js могут взаимодействовать с пользовательскими XML-частями в приложении, поэтому можно интегрировать эти мощные средства. Сведения о работе с настраиваемыми частями XML в Office JavaScript API см. в разделе [Дополнительные ресурсы](#see-also) далее в этой статье.

Работа с привязками в надстройке Word описывается в следующем разделе. Сначала рассмотрим пример Office Open XML, необходимый для вставки элемента управления содержимым форматированного текста, который можно привязать к использованию надстройки.
...
- Псевдоним **— это свойство** заголовка в диалоговом окне "Свойства элемента управления содержимым" в Word. Данное свойство (представляющее имя элемента) является обязательным, если вы планируете привязать элемент управления в надстройке.
...
- Атрибут **внешнего** вида используется для скрытия контейнера элементов управления, чтобы более понятный вид. Эта функция была представлена в Word 2013, как показано при использовании пространства имен w15. Так как это свойство используется, пространство имен w15 сохраняется в начале части document.xml.

- Атрибут **showingPlcHdr** — это необязательный параметр, который задает содержимое по умолчанию, которое вы включаете в элемент управления (текст в этом примере) в качестве заполнителя содержимого. Поэтому если пользователь щелкнет область элемента управления, будет выбрано все содержимое, что отличается от редактируемого содержимого.

- Хотя пустой знак абзаца ( **w:p/** ), предшествующий тегу **SDT** , не требуется для добавления элемента управления содержимым (и добавляет вертикальное пространство над элементом управления в документе Word), он гарантирует, что элемент управления будет помещен в собственный абзац. Это может быть важно в зависимости от типа и форматирования содержимого, которое будет добавлено в элемент управления.
...
> [!NOTE]
> Атрибут части документа, удаленный из тега **w:sdt** этого примера, может быть указан в элементе управления содержимым для указания ссылки на отдельную часть в пакете, где может храниться контент заполнителя (части, расположенные в каталоге глоссария пакета Office Open XML). Хотя часть документа — это термин, используемый для XML-частей (то есть файлов) в пакете Office Open XML, термин "части документа", используемый в свойстве sdt, ссылается на тот же термин в Word, который используется для описания некоторых типов контента, включая стандартные блоки и быстрые части свойств документа (например, встроенные элементы управления привязкой к данным XML). If you see parts under a glossary directory in your Office Open XML package, you may need to retain them if the content you're inserting includes these features. Для типичного элемента управления содержимым, к которому вы планируете использовать привязку из надстройки, они не требуются. Just remember that, if you do delete the glossary parts from the package, you must also remove the document part attribute from the w:sdt tag.

В следующем разделе мы расскажем, как создавать и использовать привязки в надстройке Word.

## <a name="insert-content-at-a-designated-location"></a>Вставка содержимого в указанное расположение

Вы уже рассмотрели, как вставить содержимое в активное выделение в документе Word. Если создать привязку к именованному элементу управления содержимым в документе, вы сможете вставлять такие же типы содержимого в этот элемент управления.
...
- Элемент управления содержимым должен иметь имя (это поле  "Заголовок" в диалоговом окне "Свойства элемента управления содержимым", соответствующее тегу **псевдонима** в разметке Office Open XML). Так код определяет, где следует разместить привязку.
...
- Пытается создать привязку к именованному элементу управления содержимым с помощью метода [addFromNamedItemAsync](/javascript/api/office/office.bindings#office-office-bindings-addfromnameditemasync-member(1)).

  Сначала выполните этот шаг, если существует возможный сценарий для надстройки, в котором именованный элемент управления уже мог существовать в документе при выполнении кода. Например, это может понадобиться, если шаблон, в который была добавлена надстройка и с которым она была сохранена, предназначен для работы с надстройкой, уже содержащей элемент управления. Это также может понадобиться для привязки к элементу управления, ранее размещенному надстройкой.

- `addFromNamedItemAsync` Обратный вызов в первом вызове метода проверяет состояние результата, чтобы узнать, не завершился ли привязка ошибкой, так как именованного элемента в документе нет (то есть элемент управления содержимым с именем MyContentControlTitle в этом примере). В этом случае код добавляет элемент управления в активную точку выбора (с `setSelectedDataAsync`помощью) и привязывается к ней.
...
Если вы посмотрите на разметку документа, в котором надстройка создала привязки, после выполнения кода, то увидите две части для каждой привязки. В разметке для элемента управления содержимым, куда была добавлена привязка (в document.xml), вы увидите атрибут **w15:webExtensionLinked/**.

В части документа с именем webExtensions1.xml вы увидите список созданных привязок. Каждый из них определяется с помощью идентификатора привязки и атрибута идентификатора применимого элемента управления, например ниже, где атрибут **appref** является идентификатором элемента управления содержимым: **we:binding id="myBinding" type="text" appref="1382295294"/**.
...
## <a name="add-objects-that-use-additional-office-open-xml-parts"></a>Добавление объектов, использующих дополнительные части Office Open XML
...
При использовании стилей абзацев или стилей таблиц для форматирования содержимого применяется тот же подход к редактированию разметки, который вы просмотрели в предыдущем примере с текстом, отформатированным напрямую. Однако разметка для работы со стилями абзацев значительно проще, поэтому это пример, описанный здесь.
...
Использование стиля — хороший пример того, насколько важно читать и понимать разметку для вставляемого содержимого, так как ссылка на другую часть документа не является явной. Если в разметку добавить определение стиля и не добавлять styles.xml, информация о стиле в document.xml будет игнорироваться независимо от того, используется ли этот стиль в документе пользователя.
...
- После сведений о стилях можно увидеть определение каждого стиля, используемого документом, на основе которого была создана разметка. В эти данные входят некоторые стили по умолчанию, используемые при создании нового документа, и они могут не относиться к вашему документу. Вы можете удалить определения стилей, которые не используются содержимым.
   ...
   > [!NOTE]
   > Каждый встроенный стиль заголовка имеет связанный стиль char, который является версией стиля символов того же формата заголовка. Если вы не применили стиль заголовка как стиль знака, его можно удалить. Если стиль используется как стиль знака, он отображается в document.xml в теге свойств прогона (**w:rPr**), а не в теге свойств абзаца (**w:pPr**). Последнее происходит, только если вы применили стиль только к части абзаца, но также может произойти, если стиль был применен неправильно.
...
Например, единственным содержимым, которое необходимо сохранить из styles.xml для примера текста, показанного на рис. 2, который форматируется с использованием стиля "Заголовок 1", является следующее:
...
#### <a name="edit-the-markup-for-content-using-table-styles"></a>Изменение разметки для содержимого с помощью стилей таблиц
...
Разметка изображения включает ссылку по крайней мере на одну часть, которая содержит двоичные данные изображения. Для сложного изображения разметка может состоять из сотен страниц, при этом изменить ее нельзя. Так как вам не нужно касаться двоичных частей, вы можете просто свернуть его, если используете структурированный редактор, например Visual Studio, чтобы вы могли легко просматривать и редактировать остальную часть пакета.

Если просмотреть пример разметки для простого изображения, показанного ранее на рис. 3, который доступен в примере кода, на который ссылались ранее, и напишете [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в надстройке Word, вы увидите, что разметка для изображения в document.xml содержит сведения о размере и положении, а также ссылку на часть, содержащую двоичные данные изображения. Эта ссылка входит в тег **a:blip** следующим образом:

```XML
<a:blip r:embed="rId4" cstate="print">
```

Имейте в виду, что так как ссылка на связь используется явным образом ( **r:embed="rID4** "), и эта связанная часть необходима для отрисовки изображения, если двоичные данные не включены в пакет Office Open XML, вы получите сообщение об ошибке. Это отличается от styles.xml, как описано выше, что не вызовет ошибку, если опустить связь, так как связь не указана явным образом и связь является частью, которая предоставляет атрибуты для содержимого (форматирование), а не является частью самого содержимого.

> [!NOTE]
> При изучении разметки обратите внимание на дополнительные пространства имен, используемые в теге a:blip. В document.xml вы увидите, что пространство имен **xlmns:a** (основное пространство имен drawingML) динамически размещается в начале использования ссылок drawingML, а не в верхней document.xml части. Однако пространство имен связей (r) необходимо сохранить в начале части document.xml. Проверьте разметку изображения на наличие дополнительных пространств имен. Remember that you don't have to memorize which types of content require what namespaces, you can easily tell by reviewing the prefixes of the tags throughout document.xml.
...
Ознакомьтесь с необходимой разметкой для отформатированного изображения, показанного на рис. 4 (который использует эффекты слоев, среди прочего) в примере кода надстройки "Загрузить" и напишите [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в примере кода надстройки Word.

### <a name="work-with-smartart-diagrams"></a>Работа со схемами SmartArt

Со схемой SmartArt связано четыре части, но только две из них являются обязательными. Вы можете изучить пример разметки SmartArt в примере кода надстройки Word для загрузки и записи [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) . Сначала ознакомьтесь с кратким описанием каждой из частей и их преимуществами.
...
- drawing1.xml. Эта часть не всегда требуется, но если вы применяли пользовательское форматирование к элементам в экземпляре схемы, например непосредственному форматированию отдельных фигур, может потребоваться сохранить ее.

- colors1.xml: эта часть не требуется. Она содержит сведения о цвете стиля, но цвета диаграммы по умолчанию будут соответствовать цветам активной темы форматирования в целевом документе в зависимости от стиля цвета SmartArt, применяемого на вкладке "Работа с рисунками SmartArt" в Word перед сохранением разметки Office Open XML.

- quickStyles1.xml: эта часть не требуется. Как и часть цветов, ее можно удалить, так как схема будет принимать определение примененного стиля SmartArt, доступного в конечном документе (то есть он будет автоматически координироваться с темой форматирования в конечном документе).

> [!TIP]
> Файл layout1.xml SmartArt — это хороший пример мест, где можно дополнительно обрезать разметку, но это может оказаться нецелесообразным (так как он удаляет такой небольшой объем разметки относительно всего пакета). Если вы хотите избавиться от всей возможной разметки, можно удалить тег **dgm:sampData** и его содержимое. Следующий пример определяет, как эскиз диаграммы отображается в коллекциях стилей SmartArt. Но если его опустить, будут использоваться данные по умолчанию.
...
> [!NOTE]
> Как и в случае со схемами SmartArt, если содержимое содержит несколько диаграмм, они будут нумеруться последовательно, заменяя "1" в именах файлов, перечисленных здесь.
...
Но, как и в случае со SmartArt, вы можете удалить части цветов и стилей. Если вы использовали стили диаграммы и стили цветов, доступные для форматирования диаграммы, диаграмма будет автоматически применять соответствующее форматирование при вставке в целевой документ.

См. измененную разметку для примера диаграммы, показанной на рисунке 11 в примере кода надстройки Word, и написание [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) .

## <a name="edit-the-office-open-xml-for-use-in-your-task-pane-add-in"></a>Изменение Office Open XML для использования в надстройке области задач

Вы уже знаете, как определить и изменить содержимое в разметке. Если задача по-прежнему кажется сложной при просмотре огромного пакета Office Open XML, созданного для вашего документа, ниже приведен краткий обзор рекомендуемых действий, которые помогут быстро изменить этот пакет.
...
1. При подготовке кода Office Open XML для использования в надстройке вы почти всегда можете удалить несколько частей пакета документов. Вы можете сначала удалить их и связанные определения связей, что сразу значительно уменьшит пакет. Они включают theme1, fontTable, параметры, webSettings, эскиз, основной файл свойств и файл свойств надстройки, части `taskpane` или `webExtension`.

1. Удалите все части, не связанные с содержимым, такие как сноски, верхние и нижние колонтитулы, которые вам не требуются. Опять же, не забудьте удалить соответствующие связи.

1. Изучите часть document.xml.rels, чтобы узнать, требуются ли какие-либо файлы, указанный в ней, для содержимого, например файл изображения, часть стилей или части схемы SmartArt. Удалите связи для всех частей, которые не требуются для содержимого, и удалите связанную часть. Если для содержимого не нужны части документа, указанные в document.xml.rels, вы можете удалить и этот файл.

1. Если у вашего контента есть дополнительная RELS-часть (например, chart#.xml.rels), изучите ее, чтобы узнать, существуют ли другие части, которые можно удалить (например, экспресс-стили для диаграмм), и удалите связь из файла и соответствующую часть.

1. Измените document.xml, чтобы удалить пространства имен, не указанные в части, свойства раздела, если содержимое не содержит разрыв раздела, и любую разметку, не связанную с содержимым, которое вы хотите вставить. При вставке фигур или текстовых полей вы также можете удалить резервную разметку.

1. Измените все другие обязательные части, разметку которых можно удалить без влияния на ваш контент, например часть стилей.
...
> [!TIP]
> Если вы обновляете фрагмент Office Open XML в существующем решении во время разработки, удалите временные файлы Интернета перед повторным запуском решения, чтобы обновить Office Open XML, используемый кодом. Разметка, которая входит в состав решения в XML-файлах, кэшируется на компьютере. Вы можете удалить временные файлы Интернета в веб-браузере по умолчанию. Чтобы получить доступ к параметрам Интернета и удалить эти параметры из Visual Studio 2019, в меню  "Отладка" выберите пункт "**Параметры"**. Затем в поле **Среда** выберите **Браузер**, а затем выберите **Параметры Internet Explorer**.

## <a name="create-an-add-in-for-both-template-and-standalone-use"></a>Создание надстройки как для шаблона, так и для автономного использования

В этом разделе вы ознакомились с несколькими примерами того, что можно сделать с помощью Office Open XML в надстройки. Вы рассмотрели широкий спектр примеров форматированного контента, которые можно вставлять в документы с помощью типа приведения Office Open XML, а также методы JavaScript для вставки этого содержимого в выбранное или в указанное (привязанное) расположение.

Итак, что еще нужно знать, если вы создаете надстройку как для автономного использования (то есть вставки из Магазина или из расположения собственного сервера), так и для использования в предварительно созданном шаблоне, предназначенном для работы с надстройкой? Возможно, вы уже знаете все, что нужно.

Разметка для заданного типа контента и методов ее вставки совпадает независимо от того, предназначена ли надстройка для автономной работы или работы с шаблоном. Если вы используете шаблоны, созданные для работы с надстройкой, убедитесь, что код JavaScript включает обратные вызовы, учитывающие ситуации, в которых содержимое уже существует в документе (как показано в примере привязки в разделе [Добавление именованного элемента управления содержимым и привязка к нему](#add-and-bind-to-a-named-content-control)).
...
- Полная справочник по языку и связанная документация по Open XML: [стандартный ECMA-376: форматы файлов Office Open XML](https://www.ecma-international.org/publications/standards/Ecma-376.htm)
- [Изучение API JavaScript для Office: привязка данных и пользовательские XML-части](/archive/msdn-magazine/2013/april/microsoft-office-exploring-the-javascript-api-for-office-data-binding-and-custom-xml-parts)
