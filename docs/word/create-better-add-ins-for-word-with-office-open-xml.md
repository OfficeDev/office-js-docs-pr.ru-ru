...
Если вы создаете надстройки Office для запуска в Word, возможно, вы уже знаете, что API JavaScript Office (Office.js) предлагает несколько форматов для чтения и записи содержимого документа. Они называются типами приведения, и к ним относятся обычный текст, таблицы, HTML и Office Open XML.
...
> [!NOTE]
> Office Open XML — это также язык документов PowerPoint и Excel (а начиная с Office 2013 — и документов Visio). However, currently, you can coerce content as Office Open XML only in Office Add-ins created for Word. Дополнительные сведения о Office Open XML, включая полную справочную документацию по языку, см. в [разделе "См. также](#see-also)".

Для начала ознакомьтесь с некоторыми типами содержимого, которые можно вставить с помощью приведения Office Open XML.
Скачайте пример кода Load и напишите [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в надстройке Word, которая содержит разметку Office Open XML и код Office.js, необходимый для вставки любого из следующих примеров в Word.
...
Используйте тот же метод для вставки любого Office, поддерживаемого форматом изображения.
...
Office предлагает широкий набор макетов схем SmartArt (и вы можете использовать Office Open XML для создания собственных элементов).

*Рис. 11. Диаграмма*

![Диаграмма в Word.](../images/office15-app-create-wd-app-using-ooxml-fig11.png)

Вы можете вставлять Excel как динамические диаграммы в документы Word. Это также означает, что их можно использовать в надстройке для Word.
Как показано в предыдущих примерах, можно использовать приведение Open XML Office для вставки контента любого типа, который пользователь может вставить в собственный документ.
...
> [!NOTE]
> Документ Office Open XML фактически представляет собой сжатый пакет файлов, представляющих содержимое документа. Сохранение файла в формате документа Word XML позволяет преобразовать весь пакет Open XML Office в один XML-файл, `getSelectedDataAsync` который также можно получить при извлечении разметки Open XML Office Open XML.
...
Предположим, вам нужно вставить всего лишь абзац текста с прямым форматированием, как показано на рисунке 1. Когда вы захватите Office Open XML `getSelectedDataAsync`для форматированного текста, вы увидите большой объем разметки. Она включает элемент package, который представляет весь документ и содержит несколько частей (обычно называемые частями документа или, в Office Open XML, частями пакета), как показано на рисунке 13. Каждая часть представляет отдельный файл в пакете.

> [!TIP]
> Измените Office разметку Open XML в текстовом редакторе, например Блокнот. Если вы откроете его в Visual Studio, используйте >**Advanced > Format Document** (CTRL+K, CTRL+D), чтобы отформатировать пакет для упрощения редактирования. Then you can collapse or expand document parts or sections of them, as shown in Figure 12, to more easily review and edit the content of the Office Open XML package. Each document part begins with a **pkg:part** tag.

*Рис. 12. Свертывание и развертывание частей пакета для редактирования в Visual Studio*

![Office фрагмент кода Open XML для части пакета в Visual Studio.](../images/office15-app-create-wd-app-using-ooxml-fig12.png)
...
Вы изменили приведенный здесь Office Open XML, как описано в предыдущем разделе, чтобы оставить только необходимые части документа и только необходимые элементы в каждой из этих частей. Вы узнаете, как изменить разметку самостоятельно (и подробнее об оставшихся фрагментах) в следующем разделе темы.
...
После сохранения предыдущего Office Open XML в виде XML-файла, доступного из решения, используйте следующую функцию, чтобы задать форматированное текстовое содержимое в документе с помощью Office приведения Open XML.
...
> [!NOTE]
> _Замените yourXMLfilename_ именем и путем XML-файла, так как вы сохранили его в решении. Если вы не знаете, куда включить XML-файлы в решение или как ссылаться на них в коде, см. пример кода "Загрузить и написать [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) " в примере кода надстройки Word, а также рабочий пример разметки и JavaScript, показанный здесь.
...
## <a name="use-the-same-office-open-xml-structure-for-different-content-types"></a>Использование одной и той же Office Open XML для различных типов контента
...
### <a name="understand-drawingml-markup-office-graphics-in-word-what-are-fallbacks"></a>Общие сведения о разметке drawingML (Office графики) в Word: что такое резервные версии?
...
Работа с привязками в надстройке Word описывается в следующем разделе. Сначала взгляните на пример файла Open XML Office, необходимого для вставки элемента управления содержимым форматированного текста, который можно привязать к использованию надстройки.
...
- Элемент управления содержимым должен иметь имя (это поле  "Заголовок" в диалоговом окне "Свойства элемента управления содержимым", соответствующее тегу **псевдонима** в разметке Open XML Office Open XML). Так код определяет, где следует разместить привязку.
...
## <a name="add-objects-that-use-additional-office-open-xml-parts"></a>Добавление объектов, использующих дополнительные Office части Open XML
...
Имейте в виду, что поскольку ссылка на связь используется явным образом (**r:embed="rID4**"), и эта связанная часть необходима для отрисовки изображения, если двоичные данные не включены в пакет Open XML Office, вы получите ошибку. Это отличается от styles.xml, как описано выше, что не вызовет ошибку, если опустить связь, так как связь не указана явным образом и связь является частью, которая предоставляет атрибуты для содержимого (форматирование), а не является частью самого содержимого.
...
## <a name="edit-the-office-open-xml-for-use-in-your-task-pane-add-in"></a>Измените Office Open XML для использования в надстройке области задач

Вы уже знаете, как определить и изменить содержимое в разметке. Если задача по-прежнему кажется сложной при просмотре огромного Office Open XML, созданного для документа, ниже приведен краткий обзор рекомендуемых действий, которые помогут быстро изменить этот пакет.
...
В этом разделе вы ознакомились с несколькими примерами того, что можно сделать с Office Open XML в надстройки. Вы рассмотрели широкий спектр примеров форматированного типа контента, которые можно вставлять в документы с помощью типа приведения Open XML Office, а также методы JavaScript для вставки этого содержимого в выбранное или в указанное (привязанное) расположение.
...
- Полная справочник по языку и связанная с ним документация по Open XML: [Стандарт ecMA-376: Office форматы файлов Open XML](https://www.ecma-international.org/publications/standards/Ecma-376.htm)
- [Изучение Office JavaScript: привязка данных и пользовательские XML-части](/archive/msdn-magazine/2013/april/microsoft-office-exploring-the-javascript-api-for-office-data-binding-and-custom-xml-parts)
