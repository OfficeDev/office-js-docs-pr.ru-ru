...
Используйте тот же метод для вставки любого Office формата изображений с поддержкой Office.
...
Вы можете вставлять диаграммы Excel в документы Word как динамические диаграммы. Это также означает, что их можно использовать в надстройке для Word. Как видно из предыдущих примеров, с помощью приведения Office Open XML можно вставлять содержимое практически любого типа, доступного пользователям для вставки в документы. Получить нужную разметку Office Open XML можно двумя простыми способами. Добавьте форматированное содержимое в пустой документ Word и сохраните файл в формате XML-документа Word или используйте тестовую надстройку с методом [getSelectedDataAsync](/javascript/api/office/office.document#office-office-document-getselecteddataasync-member(1)), чтобы захватить разметку. Оба способа, по сути, обеспечивают одинаковый результат.

> [!NOTE]
> Документ Office XML на самом деле представляет собой сжатый пакет файлов, которые представляют содержимое документа. Сохранение файла в формате Word XML Document предоставляет весь XML-пакетOffice Open, сплощенный в один XML-файл, `getSelectedDataAsync` который также используется для получения разметки Office Open XML.
...
Предположим, вам нужно вставить всего лишь абзац текста с прямым форматированием, как показано на рисунке 1. При захвате Office XML `getSelectedDataAsync`для отформатированный текст с помощью, вы увидите большое количество разметки. Она включает элемент package, который представляет весь документ и содержит несколько частей (обычно называемые частями документа или, в Office Open XML, частями пакета), как показано на рисунке 13. Каждая часть представляет отдельный файл в пакете.
...
![Office фрагмент кода XML для части пакета в Visual Studio.](../images/office15-app-create-wd-app-using-ooxml-fig12.png)
...
Вы отредактировали Office пример Open XML, показанный здесь, как описано в предыдущем разделе, чтобы оставить только необходимые части документов и только необходимые элементы в каждой из этих частей. Подробнее о том, как изменить разметку, вы можете узнать в следующем разделе темы.
...
> [!NOTE]
> Если добавить разметку, показанную здесь, в XML-файл вместе с тегами XML-декларации для версии и mso-application в верхней части файла (показано на рисунке 13), вы можете открыть его в Word в качестве документа Word. Или, без этих тегов, вы все еще можете открыть его с **помощью** **FileOpen** >  в Word. Режим совместимости будет см **.** в панели заголовков Word, так как вы удалили параметры, которые говорят Word, что это документ Word. Поскольку вы добавляете эту разметку в существующий документ Word, это не повлияет на содержимое.

### <a name="javascript-for-using-setselecteddataasync"></a>Код JavaScript для использования метода setSelectedDataAsync

После сохранения предыдущего Office XML в виде XML-файла, доступного из решения, используйте следующую функцию для набора форматного текстового контента в документе с помощью Office принуждения open XML.

В этой функции все строки, кроме последней, используются для получения сохраненной разметки и ее использования в методе [setSelectedDataAsync](/javascript/api/office/office.document#office-office-document-setselecteddataasync-member(1)) в конце функции. `setSelectedDataASync` требуется только указать содержимое, которое необходимо вставить, и тип принуждения.

> [!NOTE]
> _Замените yourXMLfilename_ именем и пути XML-файла, как вы сохранили его в решении. Если вы не знаете, где включить [XML-файлы](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в решение или как ссылаться на них в коде, см. в примере кода надстройки Word и рабочего примера разметки и JavaScript, показанных здесь.
...
Поскольку document.xml является основной частью документа, в которой вы размещены контент, пройдитесь по этой части быстро. (На рисунке 14, который идет после этого списка, показано, как некоторые базовые теги содержимого и теги форматирования связаны с тем, что вы видите в документе Word.)
...
Чтобы проверить разметку Office XML для примеров каждого из этих типов контента, показанных ранее в рисунках 5-8, ознакомьтесь с разделом Нагрузка и напишите Open [XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в примере кода надстройки Word, на который ссылается в разделе обзор.
...
Как правило, как вы видите примеры фигуры и текстовых полей, включенных в код Load и записываю [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в примере кода надстройки Word, разметка отката может быть удалена. Word автоматически добавляет недостающую разметку отката к фигурам при сэкономлении документа. Но если вы хотите сохранить резервную разметку для поддержки всех пользовательских сценариев, никто не помешает вам это сделать.
...
В примерах кода Загружайте и записывайте [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в надстройке [Word и Word-Add-in-Get-Set-EditOpen-XML](https://github.com/OfficeDev/Word-Add-in-Get-Set-EditOpen-XML), текстовое поле и форма настраиваются с использованием различных типов параметров упаковки текста и позиционирования. (Кроме того, помните, что для изображений в этих примерах кода используется форматирование "в тексте", которое размещает графический объект на опорной линии текста.)
...
- Управление контентом должно иметь имя (это поле **Title** в диалоговом окне Свойства управления контентом, соответствующее тегу **Alias** в разметке open XML Office XML). Так код определяет, где следует разместить привязку.
...
- Пытается создать привязку к именованному элементу управления содержимым с помощью метода [addFromNamedItemAsync](/javascript/api/office/office.bindings#office-office-bindings-addfromnameditemasync-member(1)).
  ...
  Сначала выполните этот шаг, если существует возможный сценарий для надстройки, где имя управления уже может существовать в документе при выполнении кода. Например, это может понадобиться, если шаблон, в который была добавлена надстройка и с которым она была сохранена, предназначен для работы с надстройкой, уже содержащей элемент управления. Это также может понадобиться для привязки к элементу управления, ранее размещенному надстройкой.

- `addFromNamedItemAsync` При первом вызове метода вызов проверяет состояние результата, чтобы узнать, не удалось ли связывание, так как названный элемент не существует в документе (то есть элемент управления контентом с именем MyContentControlTitle в этом примере). Если это так, код добавляет управление в точку активного выбора (с `setSelectedDataAsync`помощью), а затем привязывает к ней.
...
Если вы посмотрите на разметку документа, в котором надстройка создала привязки, после выполнения кода, то увидите две части для каждой привязки. В разметке для управления контентом, в которой добавлена привязка (document.xml), вы увидите атрибут **w15:webExtensionLinked/**.
...
В примере кода Load и записи [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в надстройке Word можно увидеть отредактированы примеры разметки для всех этих типов контента. Эти типы содержимого можно вставить, используя код JavaScript, показанный ранее (и представленный в примерах кода) для вставки содержимого в выделенное место и записи содержимого в указанное расположение с помощью привязок.
...
Тот же подход к редактированию разметки, который вы посмотрели в предыдущем примере с текстом с прямым форматированием, применяется при использовании стилей абзацев или стилей таблиц для формата контента. Однако разметка для работы со стилями абзац значительно проще, так что вот пример, описанный здесь.
...
- После сведений о стилях можно увидеть определение каждого стиля, используемого документом, на основе которого была создана разметка. В эти данные входят некоторые стили по умолчанию, используемые при создании нового документа, и они могут не относиться к вашему документу. Вы можете удалить определения стилей, которые не используются содержимым.
   ...
   > [!NOTE]
   > Каждый встроенный стиль заголовка имеет связанный стиль Char, который является версией стиля символов одного и того же формата заголовка. Если вы не применили стиль заголовка как стиль знака, его можно удалить. Если стиль используется как стиль знака, он отображается в document.xml в теге свойств прогона (**w:rPr**), а не в теге свойств абзаца (**w:pPr**). Последнее происходит, только если вы применили стиль только к части абзаца, но также может произойти, если стиль был применен неправильно.
...
Разметка изображения включает ссылку по крайней мере на одну часть, которая содержит двоичные данные изображения. Для сложного изображения разметка может состоять из сотен страниц, при этом изменить ее нельзя. Так как вам никогда не придется прикасаться к двоичным частям, вы можете просто свернуть его, если вы используете структурированный редактор, например Visual Studio, чтобы вы могли легко просмотреть и изменить остальную часть пакета.

Если вы проверите пример разметки для простого изображения, показанного ранее на рис. 3, доступного в примере кода с ранее ссылкой Нагрузка и напишите Open [XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в надстройке Word, вы увидите, что разметка для изображения в document.xml включает сведения о размере и положении, а также ссылку на связь с частью, которая содержит двоичные данные изображения. Эта ссылка входит в тег **a:blip** следующим образом:

```XML
<a:blip r:embed="rId4" cstate="print">
```

Следует помнить, что, так как ссылка на отношения явно используется (**r:embed="rID4"** и эта связанная часть требуется для отрисовки изображения, если вы не включаете двоичные данные в пакете Office Open XML, вы получите ошибку. Файл styles.xml, описанный ранее, не вызовет ошибки, если его опустить, так как связь не указана явно и ссылается на часть, предоставляющую атрибуты содержимого (форматирование), а не входящую в само содержимое.

> [!NOTE]
> При изучении разметки обратите внимание на дополнительные пространства имен, используемые в теге a:blip. В document.xml вы увидите, что **пространство имен xlmns:a** (основное пространство имен drawingML) динамически помещается в начале использования ссылок drawingML, а не в верхней части document.xml. Однако пространство имен связей (r) необходимо сохранить в начале части document.xml. Проверьте разметку изображения на наличие дополнительных пространств имен. Remember that you don't have to memorize which types of content require what namespaces, you can easily tell by reviewing the prefixes of the tags throughout document.xml.
...
Но, как и в случае со SmartArt, вы можете удалить части цветов и стилей. Если для форматирования диаграммы использовались стили и стили цветов, диаграмма автоматически принимает применимое форматирование при вставки в документ назначения.

См. измененную разметку на примере диаграммы, показанной на рисунке 11 в "Нагрузка" и напишите [Open XML](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/word-add-in-load-and-write-open-xml) в примере кода надстройки Word.
...
Вы уже знаете, как определить и изменить содержимое в разметке. Если задача по-прежнему кажется сложной, если вы посмотрите на массовый пакет Office Open XML, созданный для документа, ниже приводится краткое описание рекомендуемых действий, которые помогут вам быстро изменить этот пакет.
...
- [Изучение API Office JavaScript: привязка к данным и пользовательские XML-части](/archive/msdn-magazine/2013/april/microsoft-office-exploring-the-javascript-api-for-office-data-binding-and-custom-xml-parts)
