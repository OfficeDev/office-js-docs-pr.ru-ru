...
> [!TIP]
> Можно изменить Office XML в текстовом редакторе, например Блокнот. Если вы откроете его в Visual Studio, вы можете использовать **edit >Advanced > Format Document** (Ctrl+K, Ctrl+D) для формата пакета для более простого редактирования. Then you can collapse or expand document parts or sections of them, as shown in Figure 12, to more easily review and edit the content of the Office Open XML package. Each document part begins with a **pkg:part** tag.
