...
Если вы заинтересованы в определенном типе сущностей, используйте `getEntitiesByType`метод, возвращающий массив только нужных объектов. Перечисление [EntityType](/javascript/api/outlook/office.mailboxenums.entitytype) представляет все типы известных сущностей, которые можно извлечь.
...
В следующем примере используется `getEntitiesByType` текущий элемент, чтобы задать для переменной `addresses` результаты предыдущего набора правил.
...
В приведенном ниже `getFilteredEntitiesByName(name)` примере показано использование текущего элемента для задания переменной `videos` для получения массива результатов, которые совпадают с регулярным выражением предыдущего `ItemHasKnownEntity` правила.
...
- Вы можете извлекать URL-адреса, явно указанные в теле элемента, но не URL-адреса, встроенные в текст гиперссылок в HTML-элементе. Рассмотрите возможность `ItemHasRegularExpressionMatch` использования правила для получения явных и внедренных URL-адресов. Укажите `BodyAsHTML` в качестве _PropertyName_и регулярное выражение, которое соответствует URL-адресам в качестве _RegExValue_.
