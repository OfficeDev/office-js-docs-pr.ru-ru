...
При реализации логики проверки, описанной в предыдущем разделе, необходимо также требовать, чтобы домен утверждения совпадал с доменом автооткрытия `amurl` для пользователя. Для этого необходимо использовать или реализовать [автообнаружие для Exchange.](/exchange/client-developer/exchange-web-services/autodiscover-for-exchange)

- Для Exchange Online утвердим, что это хорошо известный домен (или принадлежит к географическому или специальному облаку (Office 365 URL-адреса и диапазоны `amurl` https://outlook.office365.com:443/autodiscover/metadata/json/1) [IP-адресов).](/microsoft-365/enterprise/urls-and-ip-address-ranges?view=o365-worldwide&preserve-view=true)

- Если у вашей службы надстройки есть предварительная конфигурация с клиентом пользователя, можно установить, доверять `amurl` ли этому.
...
Если надстройка не может проверить использование этих параметров, вы можете отключить надстройку изящно с соответствующим уведомлением пользователю, если проверка подлинности необходима для рабочего процесса надстройки. `amurl`
...
Создайте уникальный идентификатор для Exchange учетной записи, соединив URL-адрес документа метаданных проверки подлинности с Exchange идентификатором учетной записи. Если у вас есть уникальный идентификатор, используйте его для создания единой системы входов (SSO) для Outlook веб-службы надстройки. Дополнительные сведения об использовании уникального идентификатора для единого входа см. в статье [Проверка подлинности пользователя с помощью маркера удостоверения для Exchange](authenticate-a-user-with-an-identity-token.md).
