...
Когда запрос EWS SOAP завершен, Outlook вызывает метод вызова с одним аргументом, который является [объектом AsyncResult](/javascript/api/office/office.asyncresult). Метод вызова `AsyncResult` может получить доступ к двум свойствам объекта: `value` свойству, которое содержит ответ XML SOAP операции EWS, `asyncContext` и необязательно свойство, которое содержит все данные, `userContext` переданные в качестве параметра. Обычно метод вызываемого вызова затем обрабатывает XML в ответе SOAP, чтобы получить любую соответствующую информацию, и обрабатывает эту информацию соответствующим образом.
...
- Укажите префикс для имени тега при использовании метода DOM `getElementsByTagName`, чтобы включить поддержку Internet Explorer.
  ...
  `getElementsByTagName` ведет себя по-разному в зависимости от типа браузера. Например, ответ EWS может содержать следующий XML (форматированный и сокращенный для отображения).
   ...
   Код, как и в следующем, будет работать в браузере, как Chrome, чтобы получить XML, закрытый тегами `ExtendedProperty` .
   ...
   В Internet Explorer необходимо включить `t:` префикс имени тега следующим образом.
...
- Используйте свойство DOM, `textContent` чтобы получить содержимое тега в ответе EWS следующим образом.
...
В следующем примере `makeEwsRequestAsync` вызывается использование [операции GetItem](/exchange/client-developer/web-service-reference/getitem-operation) для получения субъекта элемента. В этом примере содержатся следующие три функции.

- `getSubjectRequest`&ndash; Принимает ID элемента в качестве ввода и возвращает XML для запроса SOAP `GetItem` для вызова указанного элемента.

- `sendRequest`&ndash; Вызывает `getSubjectRequest` запрос SOAP для выбранного элемента, затем передает запрос SOAP и метод вызова, `callback``makeEwsRequestAsync` чтобы получить субъект указанного элемента.
...
Outlook надстройки могут получать доступ к подмножество операций, доступных в EWS с помощью `makeEwsRequestAsync` метода. Если вы не знакомы с операциями EWS `makeEwsRequestAsync` и как использовать метод для доступа к операции, начните с примера запроса SOAP для настройки _аргумента данных_ .
...
При использовании метода `makeEwsRequestAsync` запрос проверки подлинности с помощью учетных данных учетной записи электронной почты текущего пользователя. Метод `makeEwsRequestAsync` управляет учетными данными для вас, чтобы не предоставлять учетные данные проверки подлинности с вашим запросом.

> [!NOTE]
> Администратор сервера должен использовать [new-WebServicesVirtualDirectory](/powershell/module/exchange/client-access-servers/New-WebServicesVirtualDirectory?view=exchange-ps&preserve-view=true) или групплет [Set-WebServicesVirtualDirectory](/powershell/module/exchange/client-access-servers/Set-WebServicesVirtualDirectory?view=exchange-ps&preserve-view=true), чтобы установить параметр _OAuthAuthAuthentication_ в каталоге  EWS `makeEwsRequestAsync` сервера клиентского доступа, чтобы включить метод для запросов EWS.

Ваша надстройка должна указать разрешение `ReadWriteMailbox` в манифесте надстройки для использования `makeEwsRequestAsync` метода. Сведения об использовании разрешения `ReadWriteMailbox` см. в разделе [Разрешение ReadWriteMailbox](understanding-outlook-add-in-permissions.md#readwritemailbox-permission) [в разделе Understanding Outlook надстройки](understanding-outlook-add-in-permissions.md).
...
См. ниже для создания дополнительных служб для надстройок с использованием веб-API ASP.NET.
