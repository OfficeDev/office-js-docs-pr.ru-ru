...
После завершения запроса SOAP EWS Outlook вызывает метод обратного вызова с одним аргументом, который является [объектом AsyncResult](/javascript/api/office/office.asyncresult) . Метод обратного `AsyncResult` вызова может получить доступ к двум свойствам объекта: `value` свойству, содержащее ответ XML SOAP операции EWS, `asyncContext` и при необходимости к свойству, которое содержит любые данные, `userContext` переданные в качестве параметра. Как правило, метод обратного вызова анализирует XML-код в ответе SOAP, чтобы получить любую соответствующую информацию, и обрабатывает эту информацию соответствующим образом.
...
- `sendRequest`&ndash; Вызывает `getSubjectRequest` запрос SOAP для выбранного элемента, а затем передает запрос SOAP и метод обратного вызова, `callback``makeEwsRequestAsync` чтобы получить тему указанного элемента.
...
1. Укажите метод обратного вызова и вызовите его `makeEwsRequestAsync`.
