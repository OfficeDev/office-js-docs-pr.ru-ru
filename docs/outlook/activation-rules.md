...
- [Элемент Rule (RuleCollection complexType)](../reference/manifest/rule.md#rulecollection), совмещающий несколько правил с помощью логических операторов.
 ...
 > 
 > `FormType` Атрибут применяется к правилам активации в манифесте версии 1.1, но не определен в `VersionOverrides` версии 1.0. Поэтому его нельзя использовать, если [](../reference/manifest/rule.md#itemis-rule) в `VersionOverrides` узле используется элемент.
