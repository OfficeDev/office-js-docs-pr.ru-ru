...
Надстройки, которые активируются на основе событий, рассчитаны на кратковременную работу до 300 секунд. Мы рекомендуем вызвать метод, чтобы надстройка сигнализирует о том, что обработка события `event.completed` запуска завершена. Надстройка также заканчивается, когда пользователь закрывает окно составить.
