...
В следующей таблице перечислены события, доступные в настоящее время, и поддерживаемые клиенты для каждого события. При поднятии события обработник получает объект, который может включать сведения, определенные `event` типу события. Столбец **JSON для событий** включает ссылку на соответствующий объект, где это применимо.
...
|---|---|---|---|---|
|`OnNewMessageCompose`|При составлении нового сообщения (включает ответ, ответ все и вперед), но не при редактировании, например, черновика.|Неприменимо|[1.10](../reference/objectmodel/requirement-set-1.10/outlook-requirement-set-1.10.md)|Windows, веб-браузер, новый предварительный просмотр пользовательского интерфейса Mac|
|`OnNewAppointmentOrganizer`|О создании новой встречи, но не о редактировании существующего.|Неприменимо|[1.10](../reference/objectmodel/requirement-set-1.10/outlook-requirement-set-1.10.md)|Windows, веб-браузер, новый предварительный просмотр пользовательского интерфейса Mac|
