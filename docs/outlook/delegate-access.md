...
В этой статье описывается, как включить в надстройке Outlook общие папки (также известные как доступ к делегатам[) и](/javascript/api/requirement-sets/outlook/preview-requirement-set/outlook-requirement-set-preview#shared-mailboxes) общие почтовые ящики (теперь в предварительном просмотре), в том числе разрешения, Office API JavaScript.
...
| Client | Exchange Online | Exchange 2019 на месте<br>(Накопительное обновление 1 или более позднее) | Exchange 2016<br>(Накопительное обновление 6 или более позднее) | Exchange 2013 |
|---|:---:|:---:|:---:|:---:|
...
Дополнительные сведения о том, где надстройки делают и не активируются в целом, обратитесь к пунктам почтовых ящиков, доступным в разделе надстройки на странице Outlook обзор надстройки.[](outlook-add-ins-overview.md#mailbox-items-available-to-add-ins)

## <a name="supported-permissions"></a>Поддерживаемые разрешения

В следующей таблице описываются разрешения, Office API JavaScript для делегатов и пользователей общих почтовых ящиков.
...
Чтобы включить общие папки и сценарии общих почтовых ящиков в надстройке, необходимо настроить элемент [SupportsSharedFolders](/javascript/api/manifest/supportssharedfolders) `true` в манифесте под родительским элементом `DesktopFormFactor`. В настоящее время другие форм-факторы не поддерживаются.

Чтобы поддерживать вызовы REST от делегата, установите узел [Разрешений](/javascript/api/manifest/permissions) в манифесте `ReadWriteMailbox`.
...
> [!TIP]
> В качестве делегата можно использовать REST для получения содержимого Outlook, прикрепленного к элементу Outlook [или групповой публикации](/graph/outlook-get-mime-message#get-mime-content-of-an-outlook-message-attached-to-an-outlook-item-or-group-post).
...
В режиме композитации сообщений [getSharedPropertiesAsync](/javascript/api/outlook/office.messagecompose#outlook-office-messagecompose-getsharedpropertiesasync-member(1)) не поддерживается в Outlook в Интернете или Windows, если не будут выполнены следующие условия.
