...
В этой статье описывается, как включить в надстройке Outlook общие папки (также известные как доступ к делегатам[) и](../reference/objectmodel/preview-requirement-set/outlook-requirement-set-preview.md#shared-mailboxes) общие почтовые ящики (теперь в предварительном просмотре), в том числе разрешения, Office API JavaScript.
