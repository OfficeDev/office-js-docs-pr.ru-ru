...
| Client | Exchange Online | Exchange 2016 в локальной среде<br>(накопительный пакет обновления 6 или более поздней версии) | Exchange 2019 в локальной среде<br>(накопительное обновление 1 или более поздней версии) |
|---|:---:|:---:|:---:|
...
Обработчики событий надстройки должны быть определены по времени или вызваны (дополнительные сведения см. в статье [о](../develop/loading-the-dom-and-runtime-environment.md#startup-of-an-outlook-add-in) запуске надстройки Outlook и инициализации надстройки Office надстройки).[](../develop/initialize-add-in.md)`Office.initialize` `Office.onReady()` Если код обработчика динамически определяется определенными обстоятельствами во время инициализации, необходимо создать функцию-заглушек для вызова обработчика после его полного определения. На функцию заглушки необходимо ссылаться в атрибуте элемента **Event** `FunctionName` манифеста. Это решение гарантирует, что обработчик определен и готов к ссылке один раз `Office.initialize` или выполняется `Office.onReady()` .
