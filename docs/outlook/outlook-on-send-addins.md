...
В следующей таблице показаны поддерживаемые сочетания клиент-сервер для функции при отправке. Исключенные сочетания не поддерживаются.

| Client | Exchange Online | Локальное exchange 2016<br>(Накопительный обновления 6 или более поздней версии) | Локальное exchange 2019<br>(Накопительный обновления 1 или более поздней версии) |
|---|:---:|:---:|:---:|
|Windows:<br>версия 1910 (сборка 12130.20272) или более поздней|Да|Да|Да|
|Mac:<br>сборка 16.30 или более поздней|Да|Нет|Нет|
...
> [!NOTE]
> Функция при отправке была официально выпущена в наборе требований 1.8 (подробные сведения см. в текущей поддержке [сервера и](../reference/requirement-sets/outlook-api-requirement-sets.md#requirement-sets-supported-by-exchange-servers-and-outlook-clients) клиента). Однако обратите внимание, что матрица поддержки функции является супермножеством наборов требований.

> [!IMPORTANT]
> Надстройки, в которых используется функция отправки, не разрешены в [AppSource.](https://appsource.microsoft.com)
...
Проверка проводится на стороне клиента в Outlook, когда инициирует событие отправки, и у надстройки есть до 5 минут до инициализации. Если проверка не удалась, отправка элемента блокируется, и на информационной панели отображается сообщение об ошибке с запросом на действие пользователя.

> [!NOTE]
> В Outlook в Интернете, когда функция проверки при отправке активна в сообщении, составленного на вкладке браузера Outlook, элемент перенаправляется в собственное окно браузера или вкладку, чтобы завершить проверку и другую обработку.
...
- **Функция приложения при отправке** &ndash; при вызове [тела. AppendOnSendAsync](/javascript/api/outlook/office.body?view=outlook-js-1.9&preserve-view=true#appendonsendasync-data--options--callback-) в обработчивом приложении при отправке возвращает ошибку.
...
> [!IMPORTANT]
> \*Функции при отправке должны работать в общих почтовых ящиках или папках, если надстройка также реализует поддержку сценариев делегирования [доступа.](delegate-access.md)
...
#### <a name="enable-the-on-send-flag"></a>Включить флаг при отправке

Администраторы могут применять проверку соответствия при отправке с помощью exchange Online PowerShell.

Чтобы все пользователи не редактировать надстройки при отправке, обрабатывают их:
...
#### <a name="turn-on-the-on-send-flag-for-a-group-of-users"></a>Включить флаг при отправке для группы пользователей
...
Чтобы отключить для пользователя проверку соответствия требованиям при отправке, назначьте политику почтовых ящиков Outlook в Интернете, в которую не включен флаг, вы можете с помощью следующих cmdlets. В этом примере используется политика почтовых ящиков *ContosoCorpOWAPolicy*.
...
Чтобы отключить функцию проверки соответствия при отправке для всех пользователей, для них назначена определенная политика почтовых ящиков Outlook в Интернете, запустите следующие cmdlets.
...
|---|---|
|Отключено|В настоящее время загруженные манифесты надстройки, которые отправляются при отправке (не обязательно последние версии), запускаются для отправляемого сообщения или элементов собрания. Это состояние или поведение по умолчанию.|
|Включен|После скачивания последних манифестов надстройки, отправляемой при отправке, из Exchange надстройки запускаются в сообщениях или собраниях. В противном случае отправка блокируется.|
...
|---|---|
|false|В настоящее время загруженные манифесты надстройки, которые отправляются при отправке (не обязательно последние версии), запускаются для отправляемого сообщения или элементов собрания. Это состояние или поведение по умолчанию.|
|true|После скачивания последних манифестов надстройки, отправляемой при отправке, из Exchange надстройки запускаются в сообщениях или собраниях. В противном случае отправка блокируется, **а** кнопка отправки отключена.|
...
### <a name="user-can-edit-item-while-on-send-add-ins-are-working-on-it"></a>Пользователь может редактировать элемент во время работы надстройки при отправке

Когда надстройки при отправке обрабатывают элемент, пользователь может изменить элемент, добавив, например, недопустимый текст или вложения. Если вы хотите запретить пользователю редактировать элемент во время обработки надстройки при отправке, можно реализовать обходное решение с помощью диалоговое окно. Это обходное решение можно использовать в Outlook в Интернете (классическая версия), Windows и Mac.

> [!IMPORTANT]
> Современный Outlook в Интернете: чтобы пользователь не редактировал элемент во время обработки надстройки при отправке, необходимо установить флаг *OnSendAddinsEnabled,* как описано в разделе "Установка надстройки Outlook, которые используют раздел "При отправке" выше в `true` этой статье. [](outlook-on-send-addins.md?tabs=modern#install-outlook-add-ins-that-use-on-send)

В обработителю при отправке:

1. Вызовите [displayDialogAsync,](/javascript/api/office/office.ui?view=outlook-js-preview&preserve-view=true#displaydialogasync-startaddress--options--callback-) чтобы открыть диалоговое окно, чтобы нажатия мыши и нажатия клавиш были отключены.

    > [!IMPORTANT]
    > Чтобы получить это поведение в классической outlook в Интернете, необходимо установить свойство [displayInIframe](/javascript/api/office/office.dialogoptions?view=outlook-js-preview&preserve-view=true#displayiniframe) в `true` `options` параметре `displayDialogAsync` вызова.

1. Реализация обработки элемента.
1. Закройте диалоговое окно. Кроме того, обработите, что произойдет, если пользователь закроет диалоговое окно.
...
- `NoSend`. Строка, представляющая собой заданный разработчиком ключ для ссылки на сообщение уведомления. С его помощью вы сможете изменить это сообщение позже. The key can't be longer than 32 characters.
