...
| Клиент | Exchange Online | Локальное exchange 2016<br>(Накопительный обновления 6 или более поздней версии) | Локальное exchange 2019<br>(Накопительный обновления 1 или более поздней версии) |
|---|:---:|:---:|:---:|
...
Проверка проводится на стороне клиента в Outlook, когда инициирует событие отправки, и у надстройки есть до 5 минут, прежде чем время не будет времени. Если проверка не удалась, отправка элемента блокируется, и на информационной панели отображается сообщение об ошибке с запросом на действие пользователя.
...
Администраторы могут обеспечить соответствие требованиям при отправке, задав для этого exchange Online PowerShell.

Чтобы все пользователи не редактировать надстройки при отправке, обрабатываются:
...
Чтобы отключить проверку соответствия при отправке для пользователя, назначьте политику почтовых ящиков Outlook в Интернете, в которую не включен флажок, с помощью следующих cmdlets. В этом примере используется политика почтовых ящиков *ContosoCorpOWAPolicy*.
...
|---|---|
|Отключено|В настоящее время загруженные манифесты надстройки для отправки (не обязательно последние версии) запускаются для отправляемого сообщения или элементов собрания. Это состояние или поведение по умолчанию.|
|Включен|После скачивания последних манифестов надстройки, отправляемой при отправке, из Exchange надстройки запускаются для сообщений или элементов собраний. В противном случае отправка блокируется.|
...
|---|---|
|false|В настоящее время загруженные манифесты надстройки для отправки (не обязательно последние версии) запускаются для отправляемого сообщения или элементов собрания. Это состояние или поведение по умолчанию.|
|true|После скачивания последних манифестов надстройки, отправляемой при отправке, из Exchange надстройки запускаются для сообщений или элементов собраний. В противном случае отправка блокируется, **а** кнопка отправки отключена.|
...
> [!IMPORTANT]
> Современный Outlook в Интернете: чтобы пользователь не редактировал элемент во время обработки надстройки при отправке, необходимо установить флаг *OnSendAddinsEnabled* так, как описано в разделе "Установка надстройки Outlook, которые используют раздел "При отправке" выше в `true` этой статье. [](outlook-on-send-addins.md?tabs=modern#install-outlook-add-ins-that-use-on-send)
...
1. Закройте диалоговое окно. Кроме того, обработать, что произойдет, если пользователь закроет диалоговое окно.
