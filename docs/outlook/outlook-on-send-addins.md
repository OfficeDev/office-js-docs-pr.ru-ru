...
В следующей таблице показаны поддерживаемые комбинации "клиент — сервер" для функции "Отправить как". Исключенные комбинации не поддерживаются.

| Client | Exchange Online | Локальная среда Exchange 2016<br>(Накопительный пакет обновления 6 или более поздней версии) | Локальная среда Exchange 2019<br>(Накопительный пакет обновления 1 или более поздней версии) |
|---|:---:|:---:|:---:|
|Windows:<br>Версия 1910 (сборка 12130,20272) или более поздняя|Да|Да|Да|
|OS<br>сборка 16,30 или более поздняя|Да|Нет|Нет|
...
> [!NOTE]
> Функция отправки при отправке официально выпущена в наборе требований 1,8 (Дополнительные сведения см. в статье [Current Server and Client Support](../reference/requirement-sets/outlook-api-requirement-sets.md#requirement-sets-supported-by-exchange-servers-and-outlook-clients) ). Однако обратите внимание, что матрица поддержки компонента является надмножеством набора требований.
...
Проверка выполняется на клиентской стороне Outlook, когда инициируется событие Send, и надстройка занимает до истечения времени до 5 минут. Если проверка завершается неудачно, отправка элемента блокируется, и на панели информации отображается сообщение об ошибке, предлагающее пользователю выполнить действие.

> [!NOTE]
> В Outlook в Интернете, когда функция отправки при отправке вызывается в сообщении, состоящем на вкладке браузера Outlook, элемент извлекается из собственного окна браузера или вкладки, чтобы выполнить проверку и другую обработку.
...
- Возможность **присоединения при отправке** &ndash; , если вы вызываете [основной текст. Аппендонсендасинк](/javascript/api/outlook/office.body?view=outlook-js-1.9&preserve-view=true#appendonsendasync-data--options--callback-) в обработчике при отправке возвращает сообщение об ошибке.
...
> [!IMPORTANT]
> \* Функция On – Send должна работать с общими почтовыми ящиками или папками, если надстройка также [реализует поддержку сценариев делегированного доступа](delegate-access.md).
...
|---|---|
|Отключено|Загруженные в текущий момент манифесты надстроек для отправки (не обязательно — последние версии) выполняются при отправке сообщений или элементов собраний. Это состояние и поведение по умолчанию.|
|Включено|После загрузки последних манифестов надстроек, отправленных с помощью Exchange, надстройки запускаются при отправке сообщений или элементов собраний. В противном случае отправка блокируется.|
...
|---|---|
|false|Загруженные в текущий момент манифесты надстроек для отправки (не обязательно — последние версии) выполняются при отправке сообщений или элементов собраний. Это состояние по умолчанию или поведение.|
|true|После загрузки последних манифестов надстроек, отправленных с помощью Exchange, надстройки запускаются при отправке сообщений или элементов собраний. В противном случае отправка блокируется, а кнопка **Отправить** отключается.|
...
### <a name="user-can-edit-item-while-on-send-add-ins-are-working-on-it"></a>Пользователь может изменять элемент при работе с надстройкой "Отправить"

В то время как надстройки, используемые при отправке, обрабатывают элемент, пользователь может редактировать элемент, добавив, например, недопустимый текст или вложения. Если вы хотите запретить пользователю изменять элемент в то время, когда ваша надстройка обрабатывает отправку, можно применить обходной путь с помощью диалогового окна. В обработчике при отправке:

1. Вызовите [displayDialogAsync](/javascript/api/office/office.ui?view=outlook-js-preview&preserve-view=true#displaydialogasync-startaddress--options--callback-) , чтобы открыть диалоговое окно, чтобы кнопки мыши и нажатия клавиш были отключены.

    > [!IMPORTANT]
    > Чтобы получить такое поведение в Outlook в Интернете, необходимо задать для [Свойства дисплайинифраме](/javascript/api/office/office.dialogoptions?view=outlook-js-preview&preserve-view=true#displayiniframe) значение `true` в `options` параметре `displayDialogAsync` вызова.

1. Реализуйте обработку элемента.
1. Закройте диалоговое окно. Кроме того, необходимо обработать то, что произойдет, если пользователь закроет диалоговое окно.
...
> [!TIP]
> Если вы используете диалоговое окно с событием при отправке, не забудьте закрыть диалоговое окно перед выполнением события.
...
- `NoSend`. Строка, представляющая собой заданный разработчиком ключ для ссылки на сообщение уведомления. С его помощью вы сможете изменить это сообщение позже. Ключ не может быть длиннее 32 символов.
