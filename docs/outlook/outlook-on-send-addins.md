...
ms.locfileid: "68664660"
---
# <a name="on-send-feature-for-outlook-add-ins"></a>Функция проверки при отправке для надстроек Outlook

Функция проверки при отправке для надстроек Outlook позволяет обрабатывать сообщение или элемент собрания или блокировать пользователей от определенных действий, а также позволяет надстройке задавать определенные свойства при отправке.

> [!NOTE]
> Функция проверки при отправке не поддерживается в надстройки, использующие манифест [Teams для надстроек Office (предварительная версия).](../develop/json-manifest-overview.md) Аналогичные эффекты можно [](autolaunch.md) получить, используя активацию на основе событий и реализуя обработчик для событий **OnMessageSend** или **OnAppointmentSend** или и того, и другое.
...
> [!NOTE]
> [Smart Alerts](smart-alerts-onmessagesend-walkthrough.md) — это более новая версия функции при отправке. Он был выпущен в [наборе обязательных элементов 1.12](/javascript/api/requirement-sets/outlook/requirement-set-1.12/outlook-requirement-set-1.12) и в нем представлены события `OnMessageSend` и события `OnAppointmentSend` . Как и функция проверки при отправке, смарт-оповещения позволяют надстройке проверять выполнение определенных условий перед отправкой почтового элемента. Интеллектуальные оповещения отличаются от функции при отправке следующим образом:
>
> - Он предлагает [параметры режима отправки](/javascript/api/manifest/launchevent#available-sendmode-options) , если вы хотите предоставить пользователям необязательные рекомендации вместо обязательных условий.
> - Она позволяет опубликовать надстройку в AppSource, если свойству **SendMode** `SoftBlock` `PromptUser` манифеста задано значение или параметр. Дополнительные сведения о публикации надстройки на основе событий см. в описании [appSource](autolaunch-store-options.md) для надстройки Outlook на основе событий.
>
> Дополнительные сведения о различиях между интеллектуальными оповещениями и функцией при отправке см. в разделе "Различия между интеллектуальными оповещениями и функцией [при отправке"](smart-alerts-onmessagesend-walkthrough.md#differences-between-smart-alerts-and-the-on-send-feature). Мы предлагаем вам [опробовать интеллектуальные оповещения, выполнив пошаговое руководство](smart-alerts-onmessagesend-walkthrough.md).
...
- **AppSource**. Надстройки Outlook, в которых используется функция проверки при отправке, невозможно публиковать в [AppSource](https://appsource.microsoft.com), так как они не проходят проверку AppSource. Надстройки, использующие функцию проверки при отправке, должны разворачиваться администраторами. Если вы хотите, чтобы надстройка была опубликована в AppSource, рассмотрите возможность использования интеллектуальных оповещений, которая является более новой версией функции при отправке. Дополнительные сведения о смарт-оповещениях и развертывании этих надстроек см. в статье "Использование интеллектуальных оповещений и событий [OnMessageSend и OnAppointmentSend](smart-alerts-onmessagesend-walkthrough.md) " в надстройке Outlook и параметрах описания [AppSource](autolaunch-store-options.md) для надстройки Outlook на основе событий.
