...
| Client | Exchange Online | Локальная версия Exchange 2016<br>(накопительный пакет обновления 6 или более поздней версии) | Локальная среда Exchange 2019<br>(накопительное обновление 1 или более поздней версии) |
|---|:---:|:---:|:---:|
...
- **AppSource**. Надстройки Outlook, в которых используется функция проверки при отправке, невозможно публиковать в [AppSource](https://appsource.microsoft.com), так как они не проходят проверку AppSource. Надстройки, использующие функцию проверки при отправке, должны разворачиваться администраторами.
  ...
  > [!IMPORTANT]
  > При запуске `npm run validate` [для](../testing/troubleshoot-manifest.md) проверки манифеста надстройки вы получите сообщение об ошибке "Надстройка почтового ящика, содержащая событие ItemSend, является недопустимой. Манифест надстройки почтового ящика содержит событие ItemSend в VersionOverrides, которое запрещено". Это сообщение появляется `ItemSend` , так как надстройки, использующие событие, необходимое для этой версии функции при отправке, не могут быть опубликованы в AppSource. Вы по-прежнему сможете загрузить неопубликованную надстройку и запустить ее при условии, что другие ошибки проверки не найдены.
...
Надстройки для Outlook в Интернете (классические), использующие функцию проверки при отправке, будут запускаться для пользователей, которым назначена политика почтовых ящиков Outlook в Интернете с флагом *OnSendAddinsEnabled*`true`.
...
1. Включите функцию проверки при отправке.

   ```powershell
    Get-OWAMailboxPolicy OWAOnSendAddinAllUserPolicy | Set-OWAMailboxPolicy –OnSendAddinsEnabled:$true
   ```

1. Назначьте политику пользователям.
...
1. Обеспечение соответствия при отправке.
...
Чтобы задать политики почтовых ящиков, администраторы могут скачать [](https://www.microsoft.com/download/details.aspx?id=49030) средство административных шаблонов, а затем получить доступ к последним административным шаблонам, запустив локальный редактор نهج المجموعة **gpedit.msc**.
...
|Включен|После скачивания последних манифестов надстроек при отправке из Exchange надстройки запускаются при отправке сообщений или элементов собрания. В противном случае отправка блокируется.|
...
1. Откройте редактор локальных نهج المجموعة (**gpedit.msc**).
...
|:------------|:------------|:--------------------------|:---------|:-------------|:-------------|
|1|Включена|Включена|Новый сеанс|Почтовый ящик 1 не может отправлять сообщение или элементы собраний из почтового ящика 2.|Not currently supported. As a workaround, use scenario 3.|
|2|Отключена|Включена|Новый сеанс|Почтовый ящик 1 не может отправлять сообщение или элементы собраний из почтового ящика 2.|Not currently supported. As a workaround, use scenario 3.|
