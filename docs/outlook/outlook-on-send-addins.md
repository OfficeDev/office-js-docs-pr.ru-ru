...
| Клиент | Exchange Online | Exchange 2016 в локальной среде<br>(накопительный пакет обновления 6 или более поздней версии) | Exchange 2019 в локальной среде<br>(накопительное обновление 1 или более поздней версии) |
|---|:---:|:---:|:---:|
...
Проверка выполняется на стороне клиента Outlook при активации события отправки, и надстройка имеет до 5 минут до истечения времени ожидания. В случае сбоя проверки отправка элемента блокируется, а на информационной панели отображается сообщение об ошибке, в котором пользователю предлагается принять меры.
...
![Снимок экрана: сообщение об ошибке с запросом на ввод строки отсутствующей темы.](../images/block-on-send-subject-cc-inforbar.png)
...
![Снимок экрана: сообщение об ошибке с сообщением о том, что пользователь обнаружил заблокированные слова.](../images/block-on-send-body.png)
...
Функция проверки при отправке поддерживается только для почтовых ящиков пользователей в Outlook в Интернете, Windows или Mac. Помимо ситуаций, когда надстройки не активируются, как указано в разделе "[](outlook-add-ins-overview.md#mailbox-items-available-to-add-ins)Почтовые ящики", доступном для надстроек на странице обзора надстроек Outlook, эта функция в настоящее время не поддерживается в автономном режиме, где доступен этот режим.
...
1. Перейдите **к user** **ConfigurationAdministrative** >  **TemplatesMicrosoft**  >  Outlook 2016  > **SecurityTrust** >  Center.
...
> [!IMPORTANT]
> Современные Outlook в Интернете. Чтобы запретить пользователю изменять элемент во время обработки надстройки при отправке, установите для *флага OnSendAddinsEnabled* `true` значение, как описано в разделе "[](outlook-on-send-addins.md?tabs=modern#install-outlook-add-ins-that-use-on-send)Установка Outlook надстроек, использующих при отправке" ранее в этой статье.
...
## <a name="debug-outlook-add-ins-that-use-on-send"></a>Отладка Outlook надстроек, использующих при отправке

Инструкции по отладке надстройки при отправке см. в разделе "Отладка надстройки без пользовательского интерфейса [Outlook надстройке](debug-ui-less.md).
