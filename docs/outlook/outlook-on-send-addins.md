...
| Клиент | Exchange Online | Локальная среда Exchange 2016<br>(Накопительный пакет обновления 6 или более поздней версии) | Локальная среда Exchange 2019<br>(Накопительный пакет обновления 1 или более поздней версии) |
|---|:---:|:---:|:---:|
...
> [!NOTE]
> Функция отправки при отправке была выпущена в наборе требований 1,8 (Дополнительные сведения см. в статье [Current Server and Client Support](../reference/requirement-sets/outlook-api-requirement-sets.md#requirement-sets-supported-by-exchange-servers-and-outlook-clients) ).
...
- Функция **присоединения при отправке** (Предварительная версия) &ndash; при вызове метода [Body. Аппендонсендасинк](/javascript/api/outlook/office.body?view=outlook-js-preview#appendonsendasync-data--options--callback-) в обработчике при отправке возвращает сообщение об ошибке.
...
1. Вызовите [displayDialogAsync](/javascript/api/office/office.ui?view=outlook-js-preview#displaydialogasync-startaddress--options--callback-) , чтобы открыть диалоговое окно, чтобы кнопки мыши и нажатия клавиш были отключены.

    > [!IMPORTANT]
    > Чтобы получить такое поведение в Outlook в Интернете, необходимо задать для [Свойства дисплайинифраме](/javascript/api/office/office.dialogoptions?view=outlook-js-preview#displayiniframe) значение `true` в `options` параметре `displayDialogAsync` вызова.
