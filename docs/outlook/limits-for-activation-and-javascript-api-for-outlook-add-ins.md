...
- При использовании регулярных выражений в `ItemHasKnownEntity` [правилах ItemHasRegularExpressionMatch](../reference/manifest/rule.md#itemhasregularexpressionmatch-rule) следует помнить о следующих ограничениях и рекомендациях, которые обычно применяются к любому приложению Outlook, а также к выражениям, описанным в таблицах 1, 2 и 3, которые отличаются в зависимости от приложения.
  ...
  - **Важно.** Текст выделяется на основе строк, которые соответствуют регулярному выражению. Тем не менее, выделены случаи не могут точно совпадать с тем, что должно происходить в результате фактического регулярного выражения утверждений, таких как отрицательный взгляд `(?!text)` вперед, look-behind и отрицательный `(?<=text)` look-behind `(?<!text)` . Например, если используется регулярное выражение `under(?!score)` "Like under, under, under score и highlight", строка "under" выделяется для всех случаев, а не только для первых двух.
...
|Установка расположения|255 символов|Метод [Location.setAsync](/javascript/api/outlook/office.location#setAsync_location__options__callback_)|Ограничение для установки расположении встречи или приглашения на собрание.|
