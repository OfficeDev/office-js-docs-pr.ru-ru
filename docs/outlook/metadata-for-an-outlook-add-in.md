...
Надстройка почты обычно загружает параметры перемещения в обработчик событий [Office.initialize](/javascript/api/office#office-initialize-reason-). В следующем примере кода JavaScript показано, как загрузить существующие параметры роуминга и получить значения 2 параметров, **имя** клиента и **customerBalance.**
...
Метод создает параметр, если параметр еще не существует, и назначает параметр `set` указанному значению. Метод `saveAsync` сохраняет параметры роуминга асинхронно. Этот пример кода передает метод вызова, когда асинхронный вызов завершается, называется с помощью одного `saveMyAddInSettingsCallback` `saveAsync`  `saveMyAddInSettingsCallback` параметра, _asyncResult_. Этот параметр является объектом [AsyncResult](/javascript/api/office/office.asyncresult), который содержит результат и все сведения об асинхронном вызове. Необязательный параметр _userContext_ можно использовать для передачи сведений о состоянии из асинхронного вызова в функцию обратного звонка.
...
Эти настраиваемые свойства, определенные для отдельных элементов, можно получить только с помощью `CustomProperties` объекта. Эти свойства отличаются от пользовательских свойств [](/office/vba/api/Outlook.UserProperties) пользователей на основе MAPI в объектной модели Outlook и расширенных свойств Exchange веб-служб (EWS). Вы не можете напрямую получить доступ с Outlook `CustomProperties` объектной модели, EWS или REST. Чтобы узнать, как получить доступ к EWS или REST, см. в разделе `CustomProperties` [Get custom properties using EWS или REST.](#get-custom-properties-using-ews-or-rest)
