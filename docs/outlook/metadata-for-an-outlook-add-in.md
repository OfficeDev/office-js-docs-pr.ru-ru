...
Надстройка почты обычно загружает параметры перемещения в обработчик событий [Office.initialize](/javascript/api/office#office-initialize-reason-). В следующем примере кода JavaScript показано, как выполняется загрузка существующих параметров перемещения и получение значений 2 параметров **customerName** и **customerBalance**:
...
Доступ к этим настраиваемой свойствам надстройки, относя к элементам, можно получить только с помощью `CustomProperties` объекта. Эти свойства отличаются от пользовательских свойств [](/office/vba/api/Outlook.UserProperties) пользователя на основе MAPI в объектной модели Outlook и расширенных свойств в веб-службах Exchange (EWS). Напрямую нельзя получить доступ с помощью объектной модели `CustomProperties` Outlook, EWS или REST. Чтобы узнать, как получить доступ с помощью EWS или REST, см. раздел "Получить настраиваемые свойства с помощью `CustomProperties` [EWS или REST".](#get-custom-properties-using-ews-or-rest)
...
- [Office.initialize](/javascript/api/office#office-initialize-reason-): инициализирует надстройку и загружает контейнер настраиваемых свойств с сервера Exchange Server.
