...
> [!NOTE]
> Функция выбора нескольких элементов в настоящее время не поддерживается в манифесте [Teams (предварительная версия),](../develop/json-manifest-overview.md) но команда работает над тем, чтобы сделать ее доступной.
