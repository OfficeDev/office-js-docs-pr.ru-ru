...
2. Установите Office 2016. 
     ...
     >**Примечание.** Если у вас еще нет Office 2016, вы можете [зарегистрироваться и получить бесплатную пробную версию на 1 месяц](http://office.microsoft.com/en-us/try/?WT%2Eintid1=ODC%5FENUS%5FFX101785584%5FXT104056786).

3.  Получите подписку на Azure.
    
     >**Примечание.** Если у вас еще нет подписки на Azure, вы можете [получить ее в рамках своей подписки MSDN](http://www.windowsazure.com/en-us/pricing/member-offers/msdn-benefits/) либо [зарегистрироваться и получить бесплатную пробную версию](https://azure.microsoft.com/en-us/pricing/free-trial). 

## <a name="step-1-create-a-shared-folder-to-host-your-add-in-xml-manifest-file"></a>Действие 1. Создание общей папки для размещения XML-файла манифеста вашей надстройки.
...
4. Щелкните папку AddinManifests правой кнопкой мыши и выберите пункты **Поделиться** > **Отдельные люди...**.
    
5. В окне **Общий доступ к файлам** щелкните стрелку раскрывающегося списка, а затем — **Все** > **Добавить** > **Поделиться**.
    
> **Примечание.** В этом пошаговом руководстве вы будете использовать локальный файловый ресурс в качестве надежного каталога, в котором вы будете хранить XML-файл манифеста надстройки. В реальных условиях вместо этого вы можете [развернуть XML-файл манифеста в каталоге SharePoint](../publish/publish-task-pane-and-content-add-ins-to-an-add-in-catalog.md) или [опубликовать надстройку в Магазине Office](https://dev.office.com/officestore/docs/submit-to-the-office-store).

## <a name="step-2-add-the-file-share-to-the-trusted-add-ins-catalog"></a>Действие 2. Добавление файлового ресурса в надежный каталог надстроек

1.  Запустите Word 2016 и создайте документ.

    >**Примечание.** Несмотря на то что в этом примере мы работаем с Word 2016, вы можете использовать любое приложение Office, поддерживающее надстройки Office, например Excel, Outlook, PowerPoint или Project 2016.
...
5. Установите флажок **Показывать в меню**. 
    ...
    >**Примечание.** Если вы храните XML-файл манифеста надстройки в общей папке, указанной в качестве надежного каталога веб-надстроек, то когда пользователь переходит на вкладку **Вставка** на ленте и выбирает пункт **Мои надстройки**, надстройка отображается в **общей папке** в диалоговом окне **Надстройки Office**.

6. Закройте Word 2016.

## <a name="step-3-create-a-web-app-in-azure"></a>Действие 3. Создание веб-приложения в Azure
...
5. В адресной строке браузера измените URL-адрес для веб-приложения так, чтобы в нем использовалось слово HTTPS, и нажмите клавишу **ВВОД**, чтобы подтвердить, что включен протокол HTTPS. В модели надстроек Office требуется, чтобы надстройки использовали протокол HTTPS.
...
4. Щелкните **Уведомления** (значок с изображением колокольчика, расположенный у верхнего края окна портала Azure) и выберите уведомление **Развертывания успешно выполнены**. Откроется страница **обзора** на портале Azure.
    ...
    >**Примечание.** По завершении развертывания сайта уведомление **Выполняется развертывание** сменится уведомлением **Развертывания успешно выполнены**.
...
6. В адресной строке браузера измените URL-адрес для веб-приложения так, чтобы в нем использовалось слово HTTPS, и нажмите клавишу **ВВОД**, чтобы подтвердить, что включен протокол HTTPS. В модели надстроек Office требуется, чтобы надстройки использовали протокол HTTPS.    

## <a name="step-4-create-an-office-add-in-in-visual-studio"></a>Действие 4. Создание надстройки Office в Visual Studio
...
6. В диалоговом окне **Служба приложений** найдите и выберите веб-приложение, которое вы создали в [действии 3 "Создание веб-приложение в Azure"](../publish/host-an-office-add-in-on-microsoft-azure.md#step-3-create-a-web-app-in-azure) и нажмите кнопку **ОК**. 
...
7. Чтобы отобразить веб-страницу для вашей надстройки, измените URL-адрес так, чтобы в нем использовался протокол HTTPS и был указан путь к HTML-странице вашей надстройки (пример: https://YourDomain.azurewebsites.net/Home.html). Это подтверждает, что веб-приложение вашей надстройки теперь размещено в Azure. Скопируйте URL-адрес корня (пример: https://YourDomain.azurewebsites.net); он потребуется вам, когда вы будете редактировать файл манифеста надстройки далее в этой статье.
    
## <a name="step-6-edit-and-deploy-the-add-in-xml-manifest-file"></a>Действие 6. Редактирование и развертывание XML-файла манифеста надстройки
...
## <a name="step-7-insert-and-run-the-add-in-in-the-office-client-application"></a>Действие 7. Вставка и запуск надстройки в клиентском приложении Office
...
6. Проверьте, что надстройка работает, выбрав какой-либо текст в документе и нажав кнопку **Highlight!** (Выделить!) в области задач. 
