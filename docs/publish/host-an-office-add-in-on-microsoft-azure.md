...
7. Чтобы отобразить веб-страницу для вашей надстройки, измените URL-адрес так, чтобы в нем использовался протокол HTTPS и был указан путь к HTML-странице вашей надстройки (пример: https://YourDomain.azurewebsites.net/Home.html). Это подтверждает, что веб-приложение вашей надстройки теперь размещено в Azure. Скопируйте URL-адрес корня (пример: https://YourDomain.azurewebsites.net); он потребуется, когда вы будете редактировать файл манифеста надстройки далее в этой статье.
