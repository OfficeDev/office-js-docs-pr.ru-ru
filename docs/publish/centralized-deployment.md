...
10. На странице **"Назначение пользователей"** выберите **"Все",** **"Определенные пользователи/группы"** или **"Только я".** С помощью поля поиска найдите пользователей и группы, для которых нужно развернуть надстройку. Для надстройки Outlook также можно выбрать метод развертывания **Fixed**, **Available** или **Optional.**
    ...
    > [!NOTE]
    > Надстройки, которые используют единый [вход,](../develop/sso-in-office-add-ins.md) будут предложено администратору согласиться на области, перечисленные в манифесте надстройки.  Если в нескольких надстройках используется та же служба(один и тот же ИД приложения Azure используется с SSO в разных надстройки), области для каждой надстройки будут иметь запрос на согласие с каждым развертыванием. На этой странице также отображается список разрешений, необходимых надстройки.

11. По завершению выберите **"Развернуть".** Этот процесс может занять до трех минут. Затем нажмите кнопку **Далее**, чтобы завершить выполнение пошаговых инструкций. Теперь надстройка будет добавлена вместе с другими приложениями Office.
...
- **Надстройка Магазина Office:** если администратор выбрал надстройку из Магазина Office при реализации централизованного развертывания через Центр администрирования Microsoft 365, а надстройка обновляется в Магазине Office, она обновется позже через централизованное развертывание. Обновление надстройки Магазина для всех пользователей может занять до 24 часов. После этого, при следующем перезапуске соответствующих приложений Office для этих пользователей, надстройка обновится. Пользователи также могут запускать обновление вручную, чтобы получить последнюю версию надстройки Store, выбрав "Вставить надстройки вкладки".  >    >    >  
