...
В этой статье описано, как создать надстройку области задач для Project профессиональный 2013, которая сравнивает данные о затратах и трудозатратах в активном проекте со средними для всех проектов в текущем экземпляре Project Web App. Надстройка использует REST с библиотекой jQuery для доступа к службе отчетов OData **ProjectData** в Project Server 2013.
...
- Для подключения к Project Web App требуется Project профессиональный 2013. Чтобы включить отладку **F5** в Visual Studio, на компьютере разработчика должен быть установлен Project профессиональный 2013.
...
Инструменты разработчика Office для Visual Studio включают шаблон для надстроек области задач для Project 2013. Если вы создаете решение с именем **HelloProjectOData**, решение содержит следующие два проекта Visual Studio:

- Проект надстройки берет имя решения. Он включает XML-файл манифеста для надстройки и предназначен для платформы .NET Framework 4,5. В процедуре 3 показаны действия по изменению манифеста для надстройки **HelloProjectOData** .

- Веб-проект называется **HelloProjectODataWeb**. Он включает веб-страницы, файлы JavaScript, CSS-файлы, изображения, ссылки и файлы конфигурации для веб-содержимого в области задач. Веб-проект предназначен для платформы .NET Framework 4. В процедурах 4 и 5 показано, как изменить файлы в веб-проекте, чтобы создать функциональные возможности надстройки **HelloProjectOData** .
...
Манифестом для проекта **HelloProjectOData** является файл HelloProjectOData. XML. При необходимости можно изменить манифест, чтобы добавить описание надстройки, ссылку на значок, сведения для дополнительных языков и другие параметры. В процедуре 3 просто изменяется отображаемое имя и описание надстройки, а также добавляется значок.
...
4. Добавьте значок, который будет отображаться в раскрывающемся списке "надстройки **Office** " на вкладке " **проект** " на ленте. Вы можете добавить файл значка в решение Visual Studio или использовать URL-адрес для значка. 

Ниже описано, как добавить файл значка в решение Visual Studio:

1. В **обозревателе решений**перейдите к папке Images.

2. Для отображения в раскрывающемся списке надстроек **Office** значок должен иметь размер 32 x 32 пикселей. Например, установите пакет SDK для Project 2013, а затем выберите папку **Images (изображения** ) и добавьте следующий файл из пакета SDK:`\Samples\Apps\HelloProjectOData\HelloProjectODataWeb\Images\NewIcon.png`
...
В области задач отображается отображаемое имя надстройки вверху, которое является значением элемента **DisplayName** в манифесте. Элемент **Body** в файле HelloProjectOData. HTML содержит другие элементы пользовательского интерфейса, как показано ниже:

- Подзаголовок, указывающий на общую функциональность или тип работы, например: **ODATA REST QUERY**.

- Кнопка **получить конечную точку ProjectData** вызывает `setOdataUrl` функцию для получения конечной точки службы **ProjectData** и отображения ее в текстовом поле. Если приложение Project не подключено к Project Web App, надстройка вызывает обработчик ошибок для отображения всплывающего сообщения об ошибке.

- Кнопка **Сравнить все проекты** отключена до тех пор, пока надстройка не получит действительную конечную точку OData. При нажатии кнопки она вызывает `retrieveOData` функцию, которая использует запрос REST для получения данных о стоимости проекта и рабочих данных из службы **ProjectData** .
...
- Таблица отображает средние значения затрат проекта, фактических затрат, трудозатрат и процент выполнения. В таблице также сравниваются значения текущего активного проекта со средними. Если текущее значение больше среднего по всем проектам, значение отображается красным цветом. Если текущее значение меньше среднего, оно отображается зеленым цветом. Если текущее значение недоступно, в таблице отображается значение **NA** синим цветом.
    ...
    > [!NOTE]
    > В этом примере данные о затратах и трудозатратах для активного проекта являются производными от опубликованных значений. При изменении значений в Project служба **ProjectData** не будет вносить изменения, пока проект не будет опубликован.
...
3. В элементе **Body** удалите существующий код из шаблона, а затем добавьте код для пользовательского интерфейса. Если элемент должен быть заполнен данными или управляться с помощью оператора jQuery, элемент должен включать уникальный атрибут **ID** . В приведенном ниже коде атрибуты **ID** для элементов **Button**, **span**и **TD** (определения ячейки таблицы), которые используются функциями jQuery, отображаются полужирным шрифтом.
   ...
   Следующий HTML-код добавляет графическое изображение, которое может быть логотипом компании. Вы можете использовать логотип или скопировать файл Невлого. png из загружаемого пакета SDK для Project 2013, а затем с помощью **обозревателя решений** добавить файл в `HelloProjectODataWeb\Images` папку.
...
Шаблон для надстройки области задач проекта включает код инициализации по умолчанию, предназначенный для демонстрации базовых действий по получению и заданию данных в документе для обычной надстройки Office 2013. Так как Project 2013 не поддерживает действия, которые записываются в активный проект, а надстройка **HelloProjectOData** не использует этот `getSelectedDataAsync` метод, можно удалить скрипт `Office.initialize` в функции и удалить `setData` функцию и `getData` функцию в файле HelloProjectOData. js по умолчанию.

JavaScript включает глобальные константы для запроса REST и глобальные переменные, используемые в нескольких функциях. Кнопка **получить конечную точку ProjectData** вызывает `setOdataUrl` функцию, которая инициализирует глобальные переменные и определяет, подключен ли Project к Project Web App.

Оставшаяся часть файла HelloProjectOData. js включает две функции: `retrieveOData` функция вызывается, когда пользователь выбирает " **Сравнить все проекты**"; `parseODataResult` функция вычисляет средние значения, а затем заполняет таблицу сравнения значениями Color и Units.

### <a name="procedure-5-to-create-the-javascript-code"></a>Процедура 5. Создание кода JavaScript

1. Удалите весь код в файле по умолчанию HelloProjectOData. js, а затем добавьте функции Global Variables и `**`Office. Initialize. В именах переменных, которые являются прописными, подразумеваются константы; Затем они используются с переменной **_pwa** для создания запроса REST в этом примере.
...
Чтобы протестировать и отладить надстройку **HelloProjectOData** с помощью Visual Studio 2015, на компьютере для разработки должен быть установлен Project профессиональный 2013. Чтобы включить различные тестовые сценарии, убедитесь, что вы можете выбрать, будет ли проект открывать файлы на локальном компьютере или подключается к Project Web App. Например, выполните следующие действия:
...
2. В диалоговом окне " **учетные записи Project Web App** " список **доступных учетных записей** может иметь несколько учетных записей Project Web App в дополнение к учетной записи локального **компьютера** . В разделе **при запуске** выберите **пункт выбрать учетную запись**.
...
- Запустите надстройку из Visual Studio, а затем откройте опубликованный проект из Project Web App, который содержит данные о затратах и трудозатратах. Убедитесь, что надстройка отображает конечную точку **ProjectData** и правильно отображает данные о затратах и трудозатратах в таблице. Вы можете использовать выходные данные в элементе управления **одататекст** , чтобы проверить запрос REST и другие сведения.

- Повторно запустите надстройку, в которой при запуске Project вы выбираете профиль локального компьютера в диалоговом окне **входа** . Откройте файл Local. MPP и протестируйте надстройку. Убедитесь, что надстройка отображает сообщение об ошибке при попытке получить конечную точку **ProjectData** .

- Снова запустите надстройку, в которой вы создадите проект с задачами с данными затрат и трудозатрат. Вы можете сохранить проект в Project Web App, но не публиковать его. Убедитесь, что надстройка отображает данные из Project Server, а не **НД** в текущем проекте.
...
2. В Visual Studio нажмите клавишу **F5**. Выполните вход в Project Web App, а затем откройте проект, созданный на предыдущем шаге. Проект можно открыть в режиме "только чтение" или в режиме редактирования.
...
5. Выберите **Сравнить все проекты**. Надстройка может приостановиться, пока она извлекает данные из службы **ProjectData** , а затем в таблице должны отображаться отформатированные средние и текущие значения.
...
8. Остановите отладку, а затем снова нажмите клавишу **F5** . Выполните вход в Project Web App, а затем создайте проект, содержащий данные о затратах и трудозатратах. Вы можете сохранить проект, но не публиковать его.
...
- Откройте проект из Project Web App, не имеющий затрат или рабочих данных для задач. В полях **текущего** столбца должны отобразиться нулевые значения.
...
- Перепишите `retrieveOData` функцию, чтобы разрешить запросы более чем из 100 проектов. Например, вы можете получить число проектов с `~/ProjectData/Projects()/$count` запросом и использовать оператор _$Skip_ и _$Top_ в запросе REST для данных Project. Выполнение нескольких запросов в цикле, а затем усреднения данных из каждого запроса. Каждый запрос данных проекта должен иметь вид: 
