...
- Visual Studio 2015 с Инструменты разработчика Office для Visual Studio содержит шаблоны, позволяющие создавать Надстройки Office и SharePoint. Убедитесь, что у вас установлена самая последняя версия Office Developer Tools. См. раздел *Средства* статьи [Надстройки Office и скачиваемые файлы для SharePoint](https://developer.microsoft.com/office/docs).
...
1. Запросите **службу ProjectData** с помощью браузера со следующим URL-адресом: **http://ServerName /ProjectServerName /_api/ProjectData**. Например, если Project Web App экземпляром`http://MyServer/pwa`, браузер отображает следующие результаты.
...
1. Вам может потребоваться предоставить свои сетевые учетные данные, чтобы увидеть результаты. Если браузер показывает сообщение "Ошибка 403, доступ запрещен", то либо у вас либо нет разрешений на вход для заданного экземпляра Project Web App, либо имеется проблема сети, требующая помощи администратора.
...
1. В **диалоговом** окне "Новый проект" разверните узлы **"** Шаблоны", **"** C#" и "**Office/SharePoint**", а затем выберите надстройки **Office**. Выберите **платформа .NET Framework 4.5.2** в раскрывающемся списке целевой платформы в верхней части центральной панели, а затем выберите надстройку **Office** (см. следующий снимок экрана).

1. Чтобы разместить оба проекта Visual Studio в одной папке, выберите **Создать каталог для решения** и найдите требуемое расположение.

1. В поле **"Имя** " введитеHelloProjectOData и нажмите кнопку **"ОК"**.

    *Рис. 1. Создание надстройки Office*

    ![Создайте надстройку Office.](../images/pj15-hello-project-o-data-creating-app.png)

1. В диалоговом окне **Выбор типа надстройки** выберите пункт **Надстройка области задач** и нажмите кнопку **Далее** (см. следующий снимок экрана).
...
1. В диалоговом окне **Выбор ведущих приложений** снимите все флажки, кроме флажка **Project** (см. следующий снимок экрана), а затем нажмите кнопку **Готово**.
...
1. Отображаемое имя по умолчанию — это имя проекта Visual Studio ("HelloProjectOData"). Например, измените значение элемента по **\<DisplayName\>** умолчанию на "Hello ProjectData".

1. Описание по умолчанию — "HelloProjectOData". Например, измените значение по умолчанию элемента Description на "Test REST queries of the ProjectData service" (тестирование запросов REST службы ProjectData).

1. Добавьте значок для отображения в раскрывающемся списке **Надстройки Office** на вкладке **PROJECT** ленты. Можно добавить файл значка в решении Visual Studio или использовать URL-адрес значка.

Ниже показано, как добавить файл значка в решение Visual Studio.

1. В **Обозреватель решений** перейдите в папку с именем Images.

1. Чтобы отображаться в раскрывающемся списке **Надстройки Office**, значок должен иметь размер 32 x 32 пикселя. Например, установите пакет SDK Project 2013, затем выберите папку **Images** и добавьте следующий файл из пакета SDK: `\Samples\Apps\HelloProjectOData\HelloProjectODataWeb\Images\NewIcon.png`
...
1. Добавьте дополнительные **элементы** скрипта для библиотек JavaScript, которые использует надстройка. Шаблон проекта содержит ссылки на файлы jQuery *[version]*.js, office.js и MicrosoftAjax.js в **папке "Скрипты** ".
...
1. Удалите весь код в файле HelloProjectOData.js по умолчанию, а затем добавьте глобальные переменные и функцию `Office.initialize` . Имена переменных, которые являются прописными буквами, означают, что они являются константами; они позже будут использоваться с переменной `_pwa` для создания запроса REST в этом примере.
...
1. Добавление `setOdataUrl` и связанные функции. Функция `setOdataUrl` вызывает и `getProjectGuid` инициализирует `getDocumentUrl` глобальные переменные. В [методе getProjectFieldAsync](/javascript/api/office/office.document) `removeAttr` анонимная функция для  параметра обратного вызова включает кнопку "Сравнить все проекты" с помощью метода в библиотеке jQuery, а затем отображает URL-адрес службы **ProjectData**. Если Project не подключен к Project Web App, функция вызывает ошибку, которая отображает всплывающее сообщение об ошибке. Файл SurfaceErrors.js содержит функцию `throwError` .

   > [!NOTE]
   > Если вы запустите Visual Studio на компьютере Project Server, чтобы использовать отладку **F5** , `_pwa` раскомментируйте код после строки, которая инициализирует глобальную переменную. Чтобы включить использование метода jQuery `ajax` при отладке на компьютере Project Server, `localhost` необходимо задать значение ДЛЯ URL-адреса PWA. При запуске Visual Studio на удаленном компьютере `localhost` URL-адрес не требуется. Перед развертыванием надстройки закомментируйте этот код.
...
1. Добавьте функцию `retrieveOData` , которая сцепляет значения для запроса REST `ajax` , а затем вызывает функцию в jQuery, чтобы получить запрашиваемые данные из **службы ProjectData** . Переменная `support.cors` включает общий доступ к ресурсам независимо от источника (CORS) с функцией `ajax` . Если инструкция `support.cors` отсутствует или задано значение `false`, `ajax` функция возвращает ошибку **"Нет транспорта** ".
   ...
   В вызове `ajax` можно использовать параметр *заголовков* или *параметр beforeSend* . Полный *параметр* является анонимной функцией, поэтому он находится в той же области, что и переменные в `retrieveOData`. Функция для полного  *параметра отображает* `odataText` `parseODataResult` результаты в элементе управления, а также вызывает метод для анализа и отображения ответа JSON. Параметр *ошибки* указывает именованную `getProjectDataErrorHandler` функцию, `odataText` `throwError` которая записывает сообщение об ошибке в элемент управления, а также использует функцию для отображения всплывающего сообщения.
...
1. Добавьте функцию `parseODataResult` , которая десериализует и обрабатывает ответ JSON из службы OData. `parseODataResult` Функция вычисляет средние значения затрат и рабочих данных до точности одного или двух десятичных разрядов, форматирует значения с правильным цветом и добавляет единицу (**$**, **hrs****%** или), а затем отображает значения в указанных ячейках таблицы.
...
1. В **диалоговом** окне "Учетные записи веб-приложения Project" список "Доступные учетные записи" может содержать несколько учетных записей Project Web App учетных записей в дополнение к учетной записи **локального** компьютера. В разделе **Во время запуска** выберите **Выбрать учетную запись**.

1. Закройте Project, чтобы среда Visual Studio могла запустить его для отладки надстройки.
...
1. В Visual Studio нажмите клавишу **F5**. Войдите в Project Web App и затем откройте проект, созданный на предыдущем шаге. Проект можно открыть в режиме чтения или в режиме редактирования.

1. На **вкладке PROJECT** ленты в раскрывающемся списке надстроек **Office** выберите **Hello ProjectData** (см. рис. 5). **Кнопка "Сравнить все проекты**" должна быть отключена.
...
1. В области **задач Hello ProjectData** выберите "Получить конечную точку **ProjectData"**. В **строке projectDataEndPoint** должен отображаться URL-адрес службы **ProjectData** , а также должна быть включена кнопка "Сравнить **все** проекты" (см. рис. 6).

1. Нажмите кнопку **Compare All Projects**. Надстройка может приостановить работу на время получения данных из службы **ProjectData**, а затем она должна отобразить отформатированные средние и текущие значения в таблице.

    *Рис. 6. Просмотр результатов запроса REST*

    ![Просмотр результатов запроса REST.](../images/pj15-hello-project-data-rest-results.png)

1. Проверьте выходные данные в текстовом поле. В нем должны отображаться путь к документу, запрос REST, сведения о состоянии и результаты JSON из вызовов и `ajax` `parseODataResult`. Выходные данные помогают понять, создать и отладить код в `parseODataResult` функции, например `projCost += Number(res.d.results[i].ProjectCost);`.
...
1. Остановите отладку и нажмите клавишу **F5** снова. Войдите в Project Web App и создайте проект, содержащий данные о материальных и трудовых затратах. Проект можно сохранить, но не публикуйте его.
