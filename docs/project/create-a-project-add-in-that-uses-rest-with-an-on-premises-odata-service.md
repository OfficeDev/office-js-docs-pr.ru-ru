...
*Рис. 4. Просмотр файлов веб-проектов по умолчанию в Průzkumník řešení*

![Просмотрите файлы веб-проекта в Průzkumník řešení.](../images/pj15-hello-project-o-data-initial-solution-explorer.png)
...
1. В **Průzkumník řešení** перейдите в папку с именем Images.
...
1. Добавьте дополнительные **элементы** скрипта для библиотек JavaScript, которые использует надстройка. Шаблон проекта содержит ссылки на файлы jQuery _[version]_.js, office.js и MicrosoftAjax.js в **папке "Скрипты** ".
...
1. В **элементе body** удалите существующий код из шаблона и добавьте код для пользовательского интерфейса. Если элемент должен заполняться данными или изменяться оператором jQuery, элемент должен содержать уникальный атрибут **id**. В следующем коде атрибуты **идентификатора** для элементов **button**, **span** и **td** (определение ячеек таблицы), используемых функциями jQuery, отображаются полужирным шрифтом.
   ...
   Следующий HTML-код добавляет графическое изображение, которое может быть эмблемой компании. Вы можете использовать логотип по вашему выбору или скопировать файл NewLogo.png из скачанного пакета SDK project 2013,  `HelloProjectODataWeb\Images` а затем использовать Průzkumník řešení, чтобы добавить файл в папку.
...
1. Добавьте метод `parseODataResult` , который десериализует и обрабатывает ответ JSON из службы OData. `parseODataResult` Метод вычисляет средние значения затрат и рабочих данных с точностью до одного или двух десятичных разрядов, форматирует значения с правильным цветом и добавляет единицу (**$**, **hrs****%** или), а затем отображает значения в указанных ячейках таблицы.
...
1. Проверьте выходные данные в текстовом поле. В нем должны отображаться путь к документу, запрос REST, сведения о состоянии и результаты JSON из вызовов и `ajax` `parseODataResult`. Выходные данные помогают понять, создать и отладить код в `parseODataResult` таком методе, как `projCost += Number(res.d.results[i].ProjectCost);`.
