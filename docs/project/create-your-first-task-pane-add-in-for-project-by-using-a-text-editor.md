...
Создайте XML-файл в локальном каталоге. XML-файл содержит элементы и детские элементы, описанные в манифесте `OfficeApp` [XML Office надстройки.](../develop/add-in-manifests.md) Например, создайте файл с именем JSOM_SimpleOMCalls.xml, содержащий следующий XML (измените значение GUID `Id` элемента).
...
Для Project элемент `OfficeApp` должен включать значение `xsi:type="TaskPaneApp"` атрибута. Элемент `Id` GUID. Значение должно быть путем обмена файлами или URL SharePoint URL-адресом для источника HTML-файла надстройки или веб-приложения, которое выполняется в области `SourceLocation` задач. Описание других элементов файла манифеста см. в разделе [Task pane add-ins for Project](../project/project-add-ins.md).
...
1. Создайте HTML-файл с именем, указанным элементом в `SourceLocation` манифесте JSOM_SimpleOMCalls.xml. 
   ...
   Файл JSOMCall.html использует общий MicrosoftAjax.js для функциональности AJAX и Office.js для функций надстройки в Office 2013 г.
...
2. Создайте файл JavaScript с именем JSOM_Sample.js в том же каталоге, где находится файл JSOMCall.html. 
   ...
   Следующий код позволяет получить контекст приложения и сведения о документе с помощью функций в файле Office.js. Объект — это ID управления `text` в `textarea` HTML-файле.

   Переменная **\_ projDoc** инициализирована с `ProjectDocument` объектом. Код включает несколько простых функций обработки ошибок, а также функцию, которая получает свойства контекста приложения и `getContextValues` контекстного документа проекта. For more information about the JavaScript object model for Project, see [JavaScript API for Office](../reference/javascript-api-for-office.md).
   ...
   Сведения о функциях в файле Office.debug.js см. в Office [API JavaScript.](../reference/javascript-api-for-office.md) Например, функция `getDocumentUrl` получает URL-адрес или путь файла открытого проекта.
...
3. Добавьте функции JavaScript, которые вызывают асинхронные функции из файлов Office.js и Project-15.js для получения выбранных данных.
   ...
   - Функция `getTaskAsync` получает имя задачи и имена назначенного ресурса. Если задача находится в синхронизированном списке задач SharePoint, получает ID задачи в списке SharePoint; в противном случае SharePoint задачи — `getTaskAsync` 0.

     > [!NOTE]
     > В демонстрационных целях примерный код содержит ошибку. Если `taskGuid` не будет разопределена, функция `getTaskAsync` будет отключена. Если вы получаете допустимый GUID задачи, а затем выберите другую задачу, функция получает данные для последней задачи, которая была прооперирована `getTaskAsync` `getSelectedTaskAsync` функцией.
  
   - `getTaskFields`, `getResourceFields` и являются локальными функциями, которые звонят , или несколько раз, чтобы получить указанные поля `getProjectFields` задачи или `getTaskFieldAsync` `getResourceFieldAsync` `getProjectFieldAsync` ресурса. В файле project-15.debug.js, в переумении и в переумериях покажите, какие `ProjectTaskFields` `ProjectResourceFields` поля поддерживаются.

   - Функция получает тип представления (определенный в project-15.debug.js) и `getSelectedViewAsync` `ProjectViewTypes` имя представления.

   - Если проект синхронизирован со списком SharePoint задач, функция получает URL-адрес и имя `getWSSUrlAsync` списка задач. Если проект не синхронизирован со списком SharePoint задач, функция не `getWSSUrlAsync` работает.

     > [!NOTE]
     > Чтобы получить SharePoint url-адрес и имя списка задач, рекомендуется использовать функцию с константами в списке `getProjectFieldAsync` `WSSUrl` `WSSList` [ProjectProjectFields.](/javascript/api/office/office.projectprojectfields)
...
4. Добавьте обратные вызовы и функции обработчика событий JavaScript для регистрации обработчиков событий для случаев изменения выделения задачи, выделения ресурса и выделения представления, а также для отмены регистрации обработчиков событий. Функция добавляет или удаляет указанный обработник событий `manageEventHandlerAsync` в зависимости от _параметра_ операции. Операция может быть `addHandlerAsync` или `removeHandlerAsync` .

   Обработник событий, указанный параметром `manageTaskEventHandler` `manageResourceEventHandler` `manageViewEventHandler` _docMethod,_ можно добавить или удалить.
...
5. Что касается основной части документа HTML, добавьте кнопки, которые вызывают функции JavaScript для тестирования. Например, в элемент общего API JSOM добавьте кнопку `div` ввода, которая вызывает общую `getSelectedDataAsync` функцию.
...
6. Добавьте раздел с кнопками для функций задач, определенных для `div` проекта, и для `TaskSelectionChanged` события.
...
7. Добавление разделов с кнопками для методов и событий ресурсов, просмотра методов и `div` событий, свойств проекта и свойств контекста
...
8. Чтобы форматирование элементов кнопки, добавьте элемент `style` CSS. Например, добавьте следующее в качестве ребенка `head` элемента.
...
8. В столбце **Длительность** выберите ячейку, относящуюся к первой задаче, а затем нажмите кнопку **getSelectedDataAsync** в надстройке **Project OM Test**. Функция `getSelectedDataAsync` задает значение текстового окна, чтобы показать `2 days` . 

9. Выберите три ячейки **Длительность** для всех трех задач. Функция возвращает текстовые значения, разделенные полуколоном, для ячеек, выбранных в разных `getSelectedDataAsync` строках, например `2 days;4 days;0 days` .

   Функция возвращает текстовые значения, разделенные запятой, для `getSelectedDataAsync` ячеек, выбранных в строке. For example in Figure 3, the entire row for task T2 is selected. При выборе `getSelectedDataAsync` в текстовом окне показано следующее:  `,Auto Scheduled,T2,4 days,Thu 6/14/12,Tue 6/19/12,1,,<NA>`
...
10. Выберите любую ячейку в строке задачи T2 или всю строку задачи T2, затем нажмите кнопку **getSelectedTaskAsync**. В текстовом поле появится значение GUID задачи, например: `{25D3E03B-9A7D-E111-92FC-00155D3BA208}`. Project сохраняет это значение в глобальной переменной `taskGuid` **надстройки Project om Test.**

11. Выберите `getTaskAsync` . Если переменная содержит GUID для задачи T2, в текстовом окне `taskGuid` отображаются сведения о задачах. Значение **ResourceNames** является пустым.
...
12. Выберите **кнопку Получить поля** задач. Функция вызывает функцию несколько раз для имени `getTaskFields` `getTaskfieldAsync` задачи, индекса, даты начала, продолжительности, приоритета и примечаний задач.
...
13. Нажмите кнопку **getWSSUrlAsync**. Если проект относится к одному из указанных ниже типов, то в результатах появятся список задач, URL-адрес и имя.
    ...
    > [!NOTE]
    > Если Project профессиональный установлен на компьютере Windows Server, чтобы сохранить проект SharePoint, можно использовать диспетчер сервера  для добавления функции **Desktop Experience.**

    Если проект является локальным проектом или используется Project профессиональный для открытия проекта, управляемого Project Server, метод показывает `getWSSUrlAsync` неопределяемую ошибку.

    - URL-адрес SharePoint: `http://ServerName`
    - Имя списка: `Test task list`

14. Выберите **кнопку Добавить** в разделе **События TaskSelectionChanged,** которая вызывает функцию для регистрации измененного события выбора задач и возвращается в `manageTaskEventHandler` `In onComplete function for addHandlerAsync Status: succeeded` текстовом окне. Выберите другую задачу; в текстовом поле появится надпись `In task selection changed event handler`, которая является выводом функции обратного вызова для события изменения выделения задачи. Выберите **кнопку Удалить,** чтобы оторегистрировать обработитель событий.

15. Чтобы использовать методы ресурса, сначала выберите представление, например **лист** **ресурсов,** использование ресурсов или форму **ресурса,** а затем выберите ресурс в этом представлении. Выберите **getSelectedResourceAsync,** чтобы инициализировать переменную **resourceGuid,** а затем выберите Get **Resource Fields,** чтобы вызвать несколько раз для свойств `getResourceFieldAsync` ресурса. Можно также добавить или удалить обработчик событий для случаев изменения выделения ресурса.
...
16. Выберите **getSelectedViewAsync, чтобы** показать тип и имя активного представления. Можно также добавить или удалить обработчик событий для случаев изменения выделения представления. Например, если **форма ресурса** является активным представлением, функция показывает следующее в `getSelectedViewAsync` текстовом окне:

    - Тип представления: `6`
    - Имя: `Resource Form`

17. Выберите **Get Project Fields,** чтобы вызвать функцию несколько раз для `getProjectFieldAsync` различных свойств активного проекта. Если проект открыт из Project Web App, функция может получить `getProjectFieldAsync` URL-адрес экземпляра Project Web App.
...
19. Можно обновить надстройку после изменения исходного кода, закрыв и перезапустив Project. В **ленте Project** список  Office надстройки поддерживает список недавно используемых надстройок.
...
**Надстройка Project om Test** — это пример, который показывает использование некоторых функций JavaScript для Project 2013 г. в Project-15.js и Office.js файлах. Этот пример предназначен исключительно для тестирования, поэтому не содержит комплексной обработки ошибок. Например, если вы не выбираете ресурс и не запустите функцию, переменная не инициализирована и вызывает `getSelectedResourceAsync` `resourceGuid` возвращение `getResourceFieldAsync` ошибки. При разработке надстройки для производственной среды следует проверять поведение при возникновении определенных ошибок и игнорировать результаты, скрывать ненужные функциональные возможности или уведомлять пользователей о необходимости выбора представления и надлежащего параметра, прежде чем использовать функцию.

Для простого примера вывод ошибки в следующем коде включает переменную th, которая указывает действие, которое необходимо принять, чтобы избежать ошибки  `actionMessage` в `getSelectedResourceAsync` функции.
...
Следующий код в SurfaceErrors.js содержит  `throwError` th-функцию, которая создает `Toast` объект.
...
Чтобы использовать функцию, включайте библиотеку JQuery и сценарий SurfaceErrors.js в файл JSOMCall.html, а затем добавьте вызов в другие функции `throwError` JavaScript, такие как `throwError` `logMethodError` .
