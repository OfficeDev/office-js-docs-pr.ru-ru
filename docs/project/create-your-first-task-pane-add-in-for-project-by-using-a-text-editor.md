...
1. Создайте файл JavaScript с именем JSOM_Sample.js в том же каталоге, где находится файл JSOMCall.html.
   ...
   Следующий код получает контекст приложения и сведения о документе с помощью методов в Office.js файла. Объект `text` является идентификатором элемента управления `textarea` в HTML-файле.
...
1. Добавьте функции JavaScript, вызывающие асинхронные методы в Office.js и Project-15.js для получения выбранных данных:

   - Например, это `getSelectedDataAsync` общий метод в Office.js, который получает неформатированный текст для выбранных данных. Дополнительные сведения см. в разделе ["Объект Document"](/javascript/api/office/office.document#office-office-document-getselectedtaskasync-member(1)).
   ...
   - Функция `getTaskAsync` получает имя задачи и имена назначенных ресурсов. Если задача находится в синхронизированном списке задач SharePoint, `getTaskAsync` получает идентификатор задачи в списке SharePoint; в противном случае идентификатор задачи SharePoint — 0.
     ...
     > [!NOTE]
     > В демонстрационных целях примерный код содержит ошибку. Если `taskGuid` значение не определено, функция `getTaskAsync` выдает ошибку. Если вы получаете допустимый GUID задачи, а затем выбираете другую задачу, `getTaskAsync` функция получает данные для последней задачи, на которой выполнялась функция `getSelectedTaskAsync` .
   ...
   - Если проект синхронизирован со списком задач SharePoint, `getWSSUrlAsync` функция получает URL-адрес и имя списка задач. Если проект не синхронизирован со списком задач SharePoint, `getWSSUrlAsync` функция возвращает ошибку.

     > [!NOTE]
     > Чтобы получить URL-адрес SharePoint и имя списка задач, `getProjectFieldAsync` `WSSUrl` `WSSList` рекомендуется использовать метод с константами в перечислении [ProjectProjectFields](/javascript/api/office/office.projectprojectfields) .
...
1. Добавьте обратные вызовы и функции обработчика событий JavaScript для регистрации обработчиков событий для случаев изменения выделения задачи, выделения ресурса и выделения представления, а также для отмены регистрации обработчиков событий. Функция `manageEventHandlerAsync` добавляет или удаляет указанный обработчик событий в зависимости от *параметра* операции. Операция может быть или `addHandlerAsync` `removeHandlerAsync`.

   Функции `manageTaskEventHandler``manageResourceEventHandler`и функции `manageViewEventHandler` могут добавлять или удалять обработчик событий, как указано в *параметре docMethod*.
...
1. Нажмите **кнопку "Получить поля задачи** ". Функция `getTaskFields` вызывает метод несколько раз `getTaskFieldAsync` для имени задачи, индекса, даты начала, длительности, приоритета и заметок к задаче.
