...
1. Что касается основной части документа HTML, добавьте кнопки, которые вызывают функции JavaScript для тестирования. Например, в элемент общего API JSOM добавьте кнопку `div` ввода, которая вызывает общую `getSelectedDataAsync` функцию.
...
1. Добавьте раздел с кнопками для функций задач, определенных для `div` проекта, и для `TaskSelectionChanged` события.
...
1. Добавление разделов с кнопками для методов и событий ресурсов, просмотра методов и `div` событий, свойств проекта и свойств контекста
...
1. Чтобы форматирование элементов кнопки, добавьте элемент `style` CSS. Например, добавьте следующее в качестве ребенка `head` элемента.
...
1. Создайте общую папку для хранения манифеста JSOM_SimpleOMCalls.xml. Можно создать общую папку на локальном компьютере или на удаленном компьютере, если к нему есть доступ из сети. Например, если манифест находится в каталоге на локальном  `C:\Project\AppManifests` компьютере, запустите следующую команду.

    `Net share AppManifests=C:\Project\AppManifests`

1. Создайте сетевую папку для размещения файлов HTML и JavaScript, относящихся к надстройке Project OM Test. Убедитесь, что путь к сетевой папке совпадает с путем, указанным в манифесте JSOM_SimpleOMCalls.xml. Например, если файлы находятся в каталоге на локальном  `C:\Project\AppSource` компьютере, запустите следующую команду.
...
1. Выберите три ячейки **Длительность** для всех трех задач. Функция возвращает текстовые значения, разделенные полуколоном, для ячеек, выбранных в разных `getSelectedDataAsync` строках, например `2 days;4 days;0 days` .
...
1. Выберите `getTaskAsync` . Если переменная содержит GUID для задачи T2, в текстовом окне `taskGuid` отображаются сведения о задачах. Значение **ResourceNames** является пустым.
...
1. Выберите **кнопку Получить поля** задач. Функция вызывает функцию несколько раз для имени `getTaskFields` `getTaskfieldAsync` задачи, индекса, даты начала, продолжительности, приоритета и примечаний задач.
...
1. Выберите **Get Project Fields,** чтобы вызвать функцию несколько раз для `getProjectFieldAsync` различных свойств активного проекта. Если проект открыт из Project Web App, функция может получить `getProjectFieldAsync` URL-адрес экземпляра Project Web App.
