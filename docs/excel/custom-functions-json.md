...
На следующем рисунке показано различие между файлами `yo office` формирования шаблонов и записью JSON с нуля.
