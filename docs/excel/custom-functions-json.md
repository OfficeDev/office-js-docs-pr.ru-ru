...
Свойство `allowCustomDataForDataTypeAny` — это тип данных boolean. Настройка этого значения позволяет настраиваемой функции принимать типы данных в качестве `true` параметров и значений возврата. Дополнительные сведения см. в [пользовательских функциях и типах данных.](custom-functions-data-types-concepts.md)
