---
title: Excel карточка сущностей типов данных API JavaScript
description: Узнайте, как использовать карточки значений сущностей с типами данных в Excel надстройки.
ms.date: 05/19/2022
ms.topic: conceptual
ms.prod: excel
...
В этой статье описывается, как использовать [API JavaScript Excel](../reference/overview/excel-add-ins-reference-overview.md) для создания модальных окон карточек в пользовательском интерфейсе Excel с типами данных значений сущностей. Эти карточки могут отображать дополнительные сведения, содержащиеся в значении сущности, за пределами того, что уже отображается в ячейке, например связанные изображения, сведения о категории продукта и атрибуты данных.
...
На следующем снимке экрана показана карточка значения сущности, которая использует предыдущий фрагмент кода. На снимке экрана показаны сведения об **идентификаторе** **продукта, имени** **продукта, количестве** на единицу и ценах за единицу из предыдущего фрагмента кода.
...
На следующем снимке экрана показана карточка значения сущности, которая использует приведенные выше фрагменты кода. На снимке экрана показан `title` объект, который **использует название** продукта и имеет значение **Pavlova**. На снимке экрана также показано `sections`. Раздел **"Количество и цена** " является свертываемым и содержит количество за **единицу и** **цену за единицу**. Поле **"Дополнительные** сведения" свертывается и сворачивается при открытии карточки.
