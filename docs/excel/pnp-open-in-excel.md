...
3. При запуске примера отобразится веб-страница, аналогичная следующему снимку экрана. Используйте веб-страницу, чтобы создать новый документ Excel, содержащий Script Lab при его открытие.
...
- Использует расширение имени загруженного файла (XLSX, DOCX или PPTX) для определения типа файла. Это необходимо сделать с самого начала, так как open XML SDK обычно имеет отдельные API для каждого типа файла.
...
- Предоставляет основную бизнес-логику, которая в этом примере представляет собой метод, встраив в script Lab.
...
1. Чтобы создать и открыть пример кода для краткого запуска, взаимодействующих со службами Office 365, перейдите к этой [https://developer.microsoft.com/graph/quick-start](https://developer.microsoft.com/graph/quick-start) теме и следуйте этим шагам.
...
1. На странице **"Пожалуйста, сохраните** секрет приложения" сохраните секрет приложения в расположении файла, где его можно будет получить и использовать позже.
...
### <a name="read-custom-properties-when-your-add-in-starts"></a>Чтение настраиваемого свойства при старте надстройки
