...
В примере ниже показано, как добавить семь новых строк в таблицу **ExpensesTable** (Таблица расходов) на листе **Sample** (Пример). Новые строки будут добавлены в конец таблицы. Если приложение Excel, в котором [](../reference/requirement-sets/excel-api-requirement-sets.md) работает код, поддерживает набор требований **ExcelApi 1.2,** ширина столбцов и высота строк задаются таким образом, чтобы наилучшим образом соответствовать текущим данным в таблице.

> [!NOTE]
> Свойство объекта TableRow указывает номер индекса строки в коллекции `index` строк таблицы. [](/javascript/api/excel/excel.tablerow) Объект не содержит свойство, которое можно использовать в качестве `TableRow` `id` уникального ключа для идентификации строки.

> [!WARNING]
> Добавление строк в таблицу из контентной надстройки приведет к утечке памяти. Сведения о текущем состоянии и #1415 см. в #1415 [gitHub](https://github.com/OfficeDev/office-js/issues/1415) Issue. 
...
В примере кода ниже показано, как добавить новый столбец в таблицу **ExpensesTable** (Таблица расходов) на листе **Sample** (Пример). Новый столбец будет добавлен после всех существующих столбцов в таблице. Он будет содержать заголовок Day of the Week (День недели), а также данные для заполнения ячеек в столбце. Если приложение Excel, в котором [](../reference/requirement-sets/excel-api-requirement-sets.md) работает код, поддерживает набор требований **ExcelApi 1.2,** ширина столбцов и высота строк задаются таким образом, чтобы наилучшим образом соответствовать текущим данным в таблице.
...
В примере кода ниже показано, как добавить новый столбец в таблицу **ExpensesTable** (Таблица расходов) на листе **Sample** (Пример). Новый столбец будет добавлен в конец таблицы, будет содержать заголовок Type of the Day (Тип дня), и в нем будет использована формула для заполнения каждой ячейки столбца. Если приложение Excel, в котором [](../reference/requirement-sets/excel-api-requirement-sets.md) работает код, поддерживает набор требований **ExcelApi 1.2,** ширина столбцов и высота строк задаются таким образом, чтобы наилучшим образом соответствовать текущим данным в таблице.
...
В примере кода ниже показано, как изменить имя первого столбца в таблице на **Purchase date**. Если приложение Excel, в котором [](../reference/requirement-sets/excel-api-requirement-sets.md) работает код, поддерживает набор требований **ExcelApi 1.2,** ширина столбцов и высота строк задаются таким образом, чтобы наилучшим образом соответствовать текущим данным в таблице.
