...
| Свойство  | Описание |
|---------|---------|
| `id` | Уникальный идентификатор функции. Этот идентификатор может содержать только буквенно-цифровые символы и точки, его изменение после настройки не допускается. |
...
- Обратный вызов `onCanceled` определяет функцию, которая выполняется при отмене функции. Необходимо реализовать обработчик отмены следующим образом для любой функции потоковой передачи. Для получения дополнительных сведений см. статью [Отмена функции](#canceling-a-function).
...
При указании метаданных для функции потоковой передачи в файле метаданных JSON необходимо задать свойства `"cancelable": true` и `"stream": true` для объекта `options`, как показано в следующем примере.
...
В некоторых случаях может потребоваться отменить выполнение настраиваемой функции потоковой передачи, чтобы снизить потребление ею пропускной способности, рабочей памяти, а а также снизить загрузку процессора. Excel отменяет выполнение функции в следующих ситуациях.
...
Пользовательские функции могут сохранять данные в глобальных переменных JavaScript, которые можно использовать в последующих вызовах. Сохранение состояния может быть полезно при вызове пользователями одной настраиваемой функции из нескольких ячеек, поскольку все экземпляры функции могут совместно использовать ее состояние. Например, вы можете сохранить данные, возвращенные при вызове веб-ресурса, чтобы не пришлось делать дополнительные вызовы одного и того же веб-ресурса.
...
- Если пользователь вызывает функцию `streamTemperature` из нескольких ячеек в Excel, функция `streamTemperature` считывает данные из одной той же переменной `savedTemperatures` каждый раз, когда она запускается. 

- Функция `refreshTemperature` считывает температуру определенного термометра каждую секунду и сохраняет результат в переменной `savedTemperatures`. Поскольку функция `refreshTemperature` не предоставляется конечным пользователям в Excel, ее не нужно регистрировать в файле JSON.
