...
## <a name="learn-the-basics"></a>Познакомьтесь с основами
...
Например, предположим, что ваша функция возвращает второе наивысшее значение из диапазона чисел, хранящихся в Excel. Приведенная ниже функция принимает параметр`values`, относящийся к типу параметра `Excel.CustomFunctionDimensionality.matrix`. Обратите внимание, что в JSON-регистрации для этой функции необходимо для параметра `type` установить значение `matrix`.
