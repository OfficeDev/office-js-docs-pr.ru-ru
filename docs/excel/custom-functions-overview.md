...
1.  Примите участие в [программе предварительной оценки Office](https://products.office.com/ru-ru/office-insider), чтобы установить версию Excel 2016, которая требуется для применения специальных функций на компьютере (16.8711 или более позднюю). Чтобы ознакомительная версия пользовательских функций работала, необходимо выбрать канал программы предварительной оценки.
...
Ниже показан код из предыдущего примера с реализованной отменой. Объект `caller` в коде содержит свойство `onCanceled`, которое должно быть определено для каждой пользовательской функции. Чтобы приложение Excel могло вызвать функцию `onCanceled`, необходимо объявить поддержку отмены при регистрации функции, задав для параметра `cancelable` значение `true`.
