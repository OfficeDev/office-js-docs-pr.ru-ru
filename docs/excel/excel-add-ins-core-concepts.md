...
> [!NOTE]
> В API JavaScript для Excel **sync()** — это единственная асинхронная операция. Чтобы оптимизировать производительность, вам следует поместить в очередь как можно больше изменений, прежде чем вызвать метод **sync()**, и свести к минимуму количество вызовов метода **sync()**.
...
* `properties` — это список имен свойств или связей, которые необходимо загрузить, в формате строк с разделителями-запятыми или в виде массива имен. Дополнительные сведения см. в описаниях методов **load()**, определенных для объектов, в [справочнике по API JavaScript для Excel](https://dev.office.com/reference/add-ins/excel/excel-add-ins-reference-overview).
