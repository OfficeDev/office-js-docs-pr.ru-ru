...
- Свойству `showAlert` по умолчанию присвоено значение `true`.  Это означает, что Excel выводит сообщение об общем поступающем (тип `Stop` ), если вы не создаете настраиваемое оповещение, которое задает `showAlert` `false` или задает настраиваемое сообщение, заголовок и стиль. Этот код задает настраиваемое сообщение и заголовок.
