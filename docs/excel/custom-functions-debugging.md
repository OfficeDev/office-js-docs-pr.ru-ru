...
> [!NOTE]
> Для простоты в этой статье показана отладка в контексте использования Visual Studio Code для редактирования, выполнения задач и в некоторых случаях использования представления отладки. При использовании другого средства редактора или командной строки см. [](#commands-for-building-and-running-your-add-in) инструкции по командной строке в конце этой статьи.

## <a name="requirements"></a>Требования

Этот процесс отладки работает **только** для пользовательских функций без пользовательского интерфейса, которые не используют области задач или другие элементы пользовательского интерфейса. Чтобы создать настраиваемую функцию без пользовательского интерфейса, выполните действия в руководстве [Create custom functions in Excel](../tutorials/excel-tutorial-create-custom-functions.md), а затем удалите все элементы области задач и пользовательского интерфейса, установленные генератором [Yeoman](../develop/yeoman-generator-overview.md) для надстроек Office.
...
1. Выберите **термин > выполнить задачу и** введите или выберите **Часы**. Это позволит отслеживать и восстанавливать любые изменения файлов.
1. Выберите **термин > выполнить задачу и** введите или выберите **Сервер Разработчика**.
...
На этом этапе выполнение остановится на строке кода, где заданной точкой разрыва. Теперь вы можете ступить через код, установить часы и использовать VS Code функции отладки, которые вам нужны.
...
1. Откройте настраиваемую папку корневого проекта функций [в VS Code](https://code.visualstudio.com/).
2. Выберите **термин > выполнить задачу и** введите или выберите **Часы**. Это позволит отслеживать и восстанавливать любые изменения файлов.
3. Выберите **термин > выполнить задачу и** введите или выберите **Сервер Разработчика**.

### <a name="start-the-vs-code-debugger"></a>Запуск отладки VS Code

1. Выберите **просмотр > выполнить или** ввести **Ctrl+Shift+D,** чтобы перейти на отлагивание представления.
1. Из параметров отлаговки выберите **Office Online (Edge Chromium)**.
...
2. В **диалоговом Office** надстройки выберите вкладку **MY ADD-INS**, выберите Управление надстройками и **Upload надстройки**.
...
4. В **диалоговом Office** надстройки выберите вкладку **MY ADD-INS**, выберите Управление надстройками и **Upload надстройки**.
...
1. Из командной строки запустите `npm run watch` , чтобы следить за изменениями кода и восстанавливать их.
...
3. Если вы хотите запустить надстройку в настольной версии Excel, запустите следующую команду.
    ...
    Если надстройка не загружена в документе, выполните действия в [Sideload надстройки](#sideload-your-add-in) , чтобы выгрузить надстройку. Затем продолжайте отладку в следующем разделе.
...
- `npm run start:desktop`. Начинается Excel на рабочем столе и побок загрузки надстройки.
- `npm run start:web -- --document {url}`(где `{url}` URL-адрес файла Excel в OneDrive или SharePoint): запускает Excel в Интернете и перегружает надстройку.
