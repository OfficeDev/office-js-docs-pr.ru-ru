---
ms.date: 07/08/2021
description: Используйте теги JSDoc для динамического создания настраиваемой метаданных JSON функций.
...
Если пользовательская функция Excel написана в JavaScript или TypeScript, [теги JSDoc](https://jsdoc.app/) используются для предоставления дополнительной информации о пользовательской функции. Теги JSDoc используются при сборке для создания файла метаданных JSON. Использование тегов JSDoc позволяет сэкономить от попытки вручную редактировать [файл метаданных JSON](custom-functions-json.md).
...
В качестве типа последнего параметра функции должно быть указано `CustomFunctions.CancelableInvocation`. Функция может назначить свойству `oncanceled` функцию, чтобы обозначить результат при отмене функции.
...
Описание отображается пользователям в Excel при вводе функции и указывает, что делает эта функция. Описанию не требуется какой-либо конкретный тег. Создайте описание для пользовательской функции, добавив в комментарии JSDoc фразу, описывающую действие функции. По умолчанию любой текст без тегов в разделе комментариев JSDoc является описанием функции.
...
Последний параметр функции должен быть типом или `CustomFunctions.Invocation` производным типом для использования `@requiresAddress`. При вызове функции свойство `address` будет содержать адрес.

В следующем примере показано, как `invocation` использовать параметр в `@requiresAddress` сочетании с возвращением адреса ячейки, вызываемой вашей настраиваемой функцией. [Дополнительные сведения см. в параметре Вызов](custom-functions-parameter-options.md#invocation-parameter).
...
Последний параметр функции должен быть типом или `CustomFunctions.Invocation` производным типом для использования  `@requiresParameterAddresses`. Комментарий JSDoc также `@returns` должен включать тег, указывающий, что возвращаемая величина — это матрица, например `@returns {string[][]}` или `@returns {number[][]}`. [Дополнительные сведения см](#matrix-type). в матричных типах. 

Когда функция называется, свойство `parameterAddresses` будет содержать адреса параметров ввода.

В следующем примере показано, `invocation` как использовать параметр в сочетании `@requiresParameterAddresses` с возвращением адресов трех параметров ввода. [Дополнительные сведения см. в](custom-functions-parameter-options.md#detect-the-address-of-a-parameter) дополнительных сведениях Об обнаружении адреса параметра. 
...
Используйте тип двумерного массива, чтобы параметр или возвращаемое значение представляли собой матрицу значений. Например, тип указывает `number[][]` матрицу чисел `string[][]` и указывает матрицу строк.
