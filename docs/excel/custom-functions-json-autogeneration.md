...
Если пользовательская функция Excel написана в JavaScript или TypeScript, [теги JSDoc](https://jsdoc.app/) используются для предоставления дополнительной информации о пользовательской функции. Теги JSDoc используются при сборке для создания файла метаданных JSON. Использование тегов JSDoc позволяет вам не изменять файл метаданных [JSON вручную.](custom-functions-json.md)
...
В качестве типа последнего параметра функции должно быть указано `CustomFunctions.CancelableInvocation`. Функция может назначить функцию свойству, чтобы `oncanceled` обозначить результат при отмене функции.
...
Этот тег указывает, что функция JavaScript/TypeScript является настраиваемой функцией Excel. Необходимо создать метаданные для настраиваемой функции.
...
* `name` указывает параметр, к @param тегу. Это обязательно.
* `description` предоставляет описание, которое отображается в Excel для параметра функции. Это необязательный вариант.
...
В следующем примере показано, как использовать параметр в сочетании с возвратом адреса ячейки, которая `invocation` `@requiresAddress` вызывает пользовательскую функцию. Дополнительные [сведения см. в](custom-functions-parameter-options.md#invocation-parameter) параметре Invocation.
...
Потоковые функции не возвращают значения напрямую, а звонят `setResult(result: ResultType)` с помощью последнего параметра.
...
Функция может возвращать обещание, которое предоставляет значение, когда обещание разрешено. Если обещание отклоняется, оно будет высылать ошибку.
