...
`id` является инвариантным идентификатором для пользовательской функции.
...
* `name` указывает, к какому параметру относится тег @param. Обязательно.
* `description` предоставляет описание, которое отображается в Excel для параметра функции. Необязательное свойство.
