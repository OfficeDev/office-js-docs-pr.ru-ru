...
> [!IMPORTANT]
> Многие проблемы производительности можно устранить с помощью рекомендуемого использования и `load` вызовов `sync` . См. раздел "Улучшения производительности с помощью API для приложений" в разделе Ограничения ресурсов и оптимизация производительности для [Office](../concepts/resource-limits-and-performance-optimization.md#performance-improvements-with-the-application-specific-apis) надстройки для консультаций по эффективной работе с API, определенными для приложений.
...
API Excel JavaScript имеет ограничения по размеру для звонков API. Excel в Интернете имеет ограничение размера полезной нагрузки для запросов и ответов в размере 5 МБ, а API `RichAPI.Error` возвращает ошибку, если это ограничение превышено. На всех платформах диапазон для получения операций ограничен пятью миллионами ячеек. Большие диапазоны обычно превышают оба этих ограничения.
...
Если API возвращает `RequestPayloadSizeLimitExceeded` ошибку, используйте стратегии наилучшей практики, задокументированные в этой статье, чтобы оптимизировать скрипт и избежать ошибки.

### <a name="strategy-1-move-unchanged-values-out-of-loops"></a>Стратегия 1. Перемещение неизмененных значений из циклов

Ограничить количество процессов, которые происходят в циклах, чтобы повысить производительность. В следующем примере кода можно `context.workbook.worksheets.getActiveWorksheet()` выйти `for` из цикла, так как он не меняется в этом цикле.
...
В следующем примере кода показана логика, аналогичная предыдущему примеру кода, но с улучшенной стратегией производительности. Значение извлекается `context.workbook.worksheets.getActiveWorksheet()` перед циклом `for` , `for` так как это значение не требуется извлекать при каждом запуске цикла. В этом цикле должны быть извлечены только значения, которые изменяются в контексте цикла.
...
Один из способов создания меньшего диапазона объектов — разделить каждый массив диапазона на несколько массивов, а затем обработать каждый новый массив циклом и новым вызовом `context.sync()` .

> [!IMPORTANT]
> Используйте эту стратегию только в том случае, если вы впервые определили превышение предельного размера запроса полезной нагрузки. Использование нескольких циклов может уменьшить размер каждого запроса полезной нагрузки, чтобы избежать превышения лимита в 5 МБ, `context.sync()` но использование нескольких циклов и нескольких вызовов также отрицательно влияет на производительность.

В следующем примере кода предпринимается попытка обработки большого массива диапазонов в одном цикле, а затем одного вызова `context.sync()` . Обработка слишком многких значений диапазона в одном `context.sync()` вызове приводит к превышению размера запроса полезной нагрузки на 5МБ.
...
Другой способ создать меньше объектов диапазона — создать массив, использовать цикл, чтобы задать все данные в этом массиве, а затем передать значения массива в диапазон. Это дает преимущества как для производительности, так и для размера полезной нагрузки. Вместо вызова для `range.values` каждого диапазона в цикле, `range.values` называется один раз за пределами цикла.

В следующем примере кода показано, как создать массив, `for` задайте значения этого массива в цикле, а затем передайте значения массива в диапазон за пределами цикла.
