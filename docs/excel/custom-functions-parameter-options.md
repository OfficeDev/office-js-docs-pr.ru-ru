...
>[!NOTE]
> Если вы работаете с созданными вручную метаданными [JSON](custom-functions-json.md) для возврата адресов параметров вместо генератора Yo Office, `options` `requiresParameterAddresses` `true`объект должен иметь набор свойств, `result` `dimensionality` `matrix`а объект должен иметь свойство, заданное .
