...
В коде надстройки обработчик события [Office.initialize](https://docs.microsoft.com/javascript/api/office) добавляет обработчик события нажатия кнопки **Submit** (Отправить), расположенной на форме, и информирует пользователя о том, что надстройка готова.
...
> [!NOTE]
> Этот код отправляет файл PowerPoint или Word на веб-сервер в виде нескольких фрагментов. Веб-сервер или служба должна скомпилировать все отдельные фрагменты в один PPTX-файл, прежде чем можно будет выполнять с ним какие-либо действия.
