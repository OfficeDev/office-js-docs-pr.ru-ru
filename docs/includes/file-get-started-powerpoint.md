...
2. Откройте файл **Home.js** в корневой папке проекта веб-приложения. Этот файл содержит скрипт надстройки. Замените все его содержимое на приведенный ниже код и сохраните файл.
...
3. Откройте файл **Home.css** в корневой папке проекта веб-приложения. В этом файле определяются специальные стили надстройки. Замените все его содержимое указанным ниже кодом и сохраните файл.
...
1. Создайте на локальном диске папку и назовите ее `my-powerpoint-addin`. В ней вы будете создавать файлы для надстройки.
...
3. С помощью генератора Yeoman создайте проект надстройки PowerPoint. Выполните приведенную ниже команду, а затем укажите представленные ниже ответы на вопросы.
...
3. Замените элемент `<main>` внутри элемента `<body>` на приведенную ниже разметку и сохраните файл.
...
4. Откройте файл **app.js** в корневой папке проекта, чтобы указать скрипт для надстройки. Замените все его содержимое на приведенный ниже код и сохраните файл.
...
5. Откройте файл **app.css** в корневой папке проекта, чтобы указать специальные стили для надстройки. Замените все его содержимое на приведенный ниже код и сохраните файл.
...
3. Атрибут `DefaultValue` элемента `Description` содержит заполнитель. Замените его на строку **Надстройка области задач для PowerPoint**.
...
1. Следуя указаниям для нужной платформы, загрузите неопубликованную надстройку в PowerPoint.
    ...
    - PowerPoint Online: [загрузка неопубликованных надстроек Office в Office Online](../testing/sideload-office-add-ins-for-testing.md#sideload-an-office-add-in-on-office-online)
    - iPad и Mac: [загрузка неопубликованных надстроек Office на iPad и Mac](../testing/sideload-an-office-add-in-on-ipad-and-mac.md)

2. В PowerPoint создайте новый файл, выберите вкладку **Главная** и нажмите кнопку **Показать область задач** на ленте, чтобы открыть область задач надстройки.
...
Поздравляем, вы успешно создали надстройку PowerPoint! Теперь вы можете узнать больше о возможностях надстроек PowerPoint и изучить образцы кода для распространенных сценариев.

> [!div class="nextstepaction"]
> [Надстройки PowerPoint](../powerpoint/powerpoint-add-ins.md)
