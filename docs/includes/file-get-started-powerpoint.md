# <a name="build-your-first-powerpoint-add-in"></a>Создайте свою первую надстройку PowerPoint
...
2. В списке типов проекта разверните узел **Visual C#** или **Visual Basic**, разверните **Office/SharePoint**, а затем выберите **Надстройки** > **Веб-надстройка PowerPoint**. 

3. Назовите проект и нажмите кнопку **ОК**.

4. Чтобы создать проект, в диалоговом окне **Создание надстройки Office** выберите **Добавить новые функции в PowerPoint**, а затем нажмите кнопку **Готово**.
...
3. Атрибут `DefaultValue` элемента `DisplayName` содержит заполнитель. Замените его строкой **Моя надстройка Office**.

4. Атрибут `DefaultValue` элемента `Description` содержит заполнитель. Замените его строкой **Надстройка области задач для PowerPoint**.
...
1. Создайте на локальном диске папку и назовите ее `my-powerpoint-addin`.  В ней вы будете создавать файлы для приложения.
...
3. Используйте генератор Yeoman для создания проекта надстройки PowerPoint. Запустите указанную ниже команду, после чего ответьте на предлагаемые вопросы следующим образом:
...
3. Атрибут `DefaultValue` элемента `Description` содержит заполнитель. Замените его строкой **Надстройка области задач для PowerPoint**.
...
Поздравляем, вы успешно создали надстройку PowerPoint! Ознакомьтесь с руководством по надстройкам PowerPoint, чтобы узнать больше о возможностях надстройки PowerPoint и создать более сложную надстройку.

> [!div class="nextstepaction"]
> [Руководство по надстройкам PowerPoint](../tutorials/powerpoint-tutorial.yml)
