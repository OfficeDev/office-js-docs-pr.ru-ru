...
3. Под элементом `div` с кнопкой `sort-table` добавьте приведенный ниже код.
...
5. Под строкой, назначающей обработчик нажатия кнопки `sort-chart`, добавьте приведенный ниже код.
...
7. Замените `TODO1` на приведенный ниже код. Обратите внимание, что для исключения строки заголовков в коде вместо метода `getRange` используется метод `Table.getDataBodyRange`, чтобы получить нужный диапазон данных для диаграммы.
...
8. Замените `TODO2` на приведенный ниже код. Обратите внимание на указанные ниже параметры.
