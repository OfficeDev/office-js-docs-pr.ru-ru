...
## <a name="configure-the-manifest-to-add-a-second-ribbon-button"></a>Настройка манифеста для добавления второй кнопки ленты
...
1. В функции `toggleProtection` замените `TODO2` приведенным ниже кодом. Примечание:
...
9. Выберите ячейку так же, как для изменения ее содержимого. Появится сообщение об ошибке и защите листа.
