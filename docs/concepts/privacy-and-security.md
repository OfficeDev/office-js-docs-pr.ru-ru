...
description: Узнайте об аспектах конфиденциальности и безопасности платформы надстройок Office.
ms.date: 03/19/2021
...
- Не разрешено взаимодействие с модальными модулями , например вызовы в JavaScript, а функции не разрешены, так как `alert` `confirm` они являются `prompt` модальными.
...
В следующих разделах кратко описано, как архитектура времени работы поддерживает запуск надстройок в клиентах Office на устройствах на базе Windows, на устройствах Mac OS X и в веб-браузерах.
...
*Рис. 2. Среда запуска надстройки Office в клиентах Mac OS X*

![Схема, показывающая приложения для среды запуска Office на Mac OS X](../images/dk2-agave-overview-mac-02.png)
...
В поддерживаемых веб-клиентах надстройки Office находятся в **iframe,** который выполняется с использованием атрибута **песочницы** HTML5. Компоненты ActiveX и навигация по главной странице веб-клиента запрещены. Поддержка надстроек Office в веб-клиентах возможна благодаря интеграции API JavaScript для Office. API JavaScript управляет жизненным циклом надстройки и взаимодействием между надстройкой и веб-клиентом аналогично клиентским приложениям на компьютерах. Это взаимодействие реализуется с помощью специальной инфраструктуры отправки сообщений между фреймами. Для взаимодействия с веб-клиентом доступна та же библиотека JavaScript (Office.js), которая используется на клиентах на компьютерах. На следующем рисунке показана инфраструктура, которая поддерживает надстройки в Office, запущенные в браузере, и соответствующие компоненты (веб-клиент, **iframe,** office add-ins runtime и API JavaScript для Office), необходимые для их поддержки.
...
Конечные пользователи и ИТ-администраторы могут отключать [дополнительные возможности подключения в Office](/deployoffice/privacy/optional-connected-experiences) для клиентов рабочего стола и мобильных клиентов. Для надстройок Office последствия отключения  параметра необязательных подключенных подключений связаны с тем, что пользователи больше не могут получать доступ к надстройки или магазину Office через этих клиентов. Однако некоторые надстройки Майкрософт, которые считаются важными или критически важными для бизнеса, и [](../publish/centralized-deployment.md) надстройки, развернутые ИТ-администратором организации с помощью централизированного развертывания, по-прежнему будут доступны. Кроме того, надстройки и магазин остаются доступными в Outlook в Интернете независимо от состояния параметра.

Дополнительные данные о поведении, характерных для Outlook, см. в этой информации: Конфиденциальность, разрешения и безопасность [надстройок Outlook.](../outlook/privacy-and-security.md#optional-connected-experiences)

Обратите внимание, что если ИТ-администратор отключает использование подключенных интерфейсов [в Office,](/deployoffice/privacy/manage-privacy-controls#policy-setting-for-most-connected-experiences)он оказывает такое же влияние на надстройки, как и отключение просто необязательных подключенных интерфейсов.
...
> [!NOTE]
> Пользователи могут видеть запрос безопасности, чтобы доверять домену при первой загрузке надстройки. Это произойдет, если хост домена надстройки находится за пределами локального домена Exchange или Office Online Server.
...
- При общем доступе к документу пользователи также совместно используют вставленные в него или связанные с ним надстройки. Если пользователь открывает документ, содержащий надстройку, которую пользователь не использовал ранее, клиентская приложение Office предлагает пользователю предоставить разрешение на запуск надстройки в документе. В организационной среде клиентская заявка Office также подсказывет пользователю, если документ поступает из внешнего источника.

- Пользователи могут включить или отключить доступ к AppSource. Для надстройок области контента и области задач пользователи управляют доступом  к доверенным надстройки и каталогам из Центра доверия на клиенте хост-офиса (открыт из Центра доверия к файлам параметров центра доверия к доверенным каталогам надстройки).  >    >    >    >   Для надстройок Outlook можно управлять надстройки,  выбрав кнопку Управление надстройки: в Outlook on Windows выберите надстройки **File**  >  **Manage.** В Outlook on Mac выберите кнопку **Управление** надстройки на панели надстройки. В Outlook в Интернете выберите меню **Параметры** (значок шестеренки) > **Управление надстройками**. Администраторы также могут управлять доступом [с помощью групповой политики](/previous-versions/office/office-2013-resource-kit/jj219429(v=office.15)#using-group-policy-to-manage-how-users-can-install-and-use-apps-for-office).
...
- Конструкция платформы надстройки обеспечивает безопасность и производительность для конечных пользователей следующими способами.
  ...
  - Надстройка Office выполняется в управлении веб-браузером, которое находится в среде времени работы надстройки, отдельной от клиентского приложения Office. Эта конструкция обеспечивает безопасность и изоляцию производительности от клиентского приложения.
...
Ниже перечислены некоторые конкретные рекомендации по защите PII для вас как разработчика надстройок Office.
...
Дополнительные сведения о разрешениях для надстройок Outlook см. в следующих темах.
...
Поскольку надстройки Office — это веб-страницы, которые работают в управлении веб-браузером, они должны следовать политике одного и того же происхождения, примененной браузером. По умолчанию веб-страницы в одном домене не могут сделать вызовы [веб-службы XmlHttpRequest](https://www.w3.org/TR/XMLHttpRequest/) в другой домен, кроме домена, в котором он находится.
...
- Вместо свойства DOM [innerHTML](https://developer.mozilla.org/docs/Web/API/Element/innerHTML) используйте свойства [innerText](https://developer.mozilla.org/docs/Web/API/Node/innerText) и [textContent](https://developer.mozilla.org/docs/DOM/Node.textContent), где это возможно. Сделайте следующее для поддержки кросс-браузера Internet Explorer и Firefox.
    ...
    Сведения о различиях **между innerText** и **textContent** см. в [документе Node.textContent.](https://developer.mozilla.org/docs/DOM/Node.textContent) Дополнительные сведения о совместимости DOM в обычных браузерах см. в [w3C DOM Compatibility - HTML.](https://www.quirksmode.org/dom/w3c_html.html#t07)
...
Поскольку надстройки Office отрисовываны в iframe при работе в браузере с клиентских приложений Office, используйте следующие советы, чтобы свести к минимуму риск кражи кликов [—](https://en.wikipedia.org/wiki/Clickjacking) метод, используемый хакерами для обмана пользователей в раскрытии конфиденциальной информации.
