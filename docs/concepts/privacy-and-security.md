...
- Надстройки содержимого и области задач используют те же параметры SSL, что и браузер по умолчанию, и позволяют доставлять большинство содержимого только по протоколу SSL. Надстройки Outlook требуют предоставления всего контента с помощью SSL. Разработчики должны указать **\<SourceLocation\>** в элементе манифеста надстройки URL-адрес, использующий ПРОТОКОЛ HTTPS, чтобы определить расположение HTML-файла для надстройки.
  ...
  Чтобы убедиться, что надстройки не доставлять содержимое по протоколу HTTP, при тестировании надстроек разработчикам следует убедиться, что в параметрах интернета в لوحة التحكم выбраны следующие параметры **,** а в тестовых сценариях не отображаются предупреждения системы безопасности.
