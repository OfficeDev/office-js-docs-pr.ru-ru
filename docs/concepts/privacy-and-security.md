...
- Модальные взаимодействия запрещены, например вызовы JavaScript `alert``confirm``prompt` и методы, так как они являются модальными.
...
> [!NOTE]
> В некоторых сценариях различные функции надстройки выполняются в отдельных средах выполнения. Для простоты в этой статье используется единственная "среда выполнения". Дополнительные сведения см. в [разделе "Среды выполнения" надстроек Office](../testing/runtimes.md). 
