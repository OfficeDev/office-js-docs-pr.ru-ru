...
description: Сведения о конфиденциальности и безопасности платформы Office надстроек.
ms.date: 01/26/2022
...
- Доступ к Office пользовательского интерфейса клиентского приложения управляется.

- Разрешен только косвенный доступ к Office пользовательского интерфейса клиентского приложения.
...
Кроме того, платформа среды выполнения предоставляет следующие преимущества, чтобы гарантировать, что надстройка Office не может повредить среду пользователя.
...
В следующих разделах кратко описано, как архитектура среды выполнения поддерживает запуск надстроек в Office-клиентах на устройствах Windows, на устройствах Mac OS X и в веб-браузерах.
...
*Рис. 2. Office среды выполнения надстроек в клиентах Mac OS X*

![Схема, показывающая приложения для Office среды выполнения в Mac OS X.](../images/dk2-agave-overview-mac-02.png)
...
В поддерживаемых веб-клиентах Office надстройки размещаются в **iframe**, который выполняется с помощью атрибута **песочницы** HTML5. Компоненты ActiveX и навигация по главной странице веб-клиента запрещены. Поддержка надстроек Office в веб-клиентах возможна благодаря интеграции API JavaScript для Office. API JavaScript управляет жизненным циклом надстройки и взаимодействием между надстройкой и веб-клиентом аналогично клиентским приложениям на компьютерах. Это взаимодействие реализуется с помощью специальной инфраструктуры отправки сообщений между фреймами. Для взаимодействия с веб-клиентом доступна та же библиотека JavaScript (Office.js), которая используется на клиентах на компьютерах. На следующем рисунке показана инфраструктура, поддерживающая надстройки в Office, запущенные в браузере, и соответствующие компоненты (веб-клиент, **iframe**, среда выполнения надстроек Office и API JavaScript для Office), необходимые для их поддержки.
...
Конечные пользователи и ИТ-администраторы могут отключать [дополнительные возможности подключения в Office](/deployoffice/privacy/optional-connected-experiences) для клиентов рабочего стола и мобильных клиентов. Для Office надстроек отключение параметра "Необязательные  сетевые функции" означает, что пользователи больше не могут получить доступ к надстройке или Office Через эти клиенты. Однако некоторые надстройки Майкрософт, которые считаются важными или критически важными для бизнеса, и надстройки, развернутые ИТ-администратором [](/microsoft-365/admin/manage/centralized-deployment-of-add-ins) организации с помощью централизованного развертывания, по-прежнему будут доступны. Кроме того, надстройки и Магазин остаются доступными в Outlook в Интернете независимо от состояния параметра.

Дополнительные сведения о Outlook, связанных с конфиденциальностью, разрешениями и безопасностью для Outlook [надстроек](../outlook/privacy-and-security.md#optional-connected-experiences).

Обратите внимание, что если ИТ-администратор отключает использование сетевых функций в [Office, он](/deployoffice/privacy/manage-privacy-controls#policy-setting-for-most-connected-experiences) оказывает то же влияние на надстройки, что и отключение только необязательных сетевых функций.
...
> [!NOTE]
> При первой загрузке надстройки пользователи могут увидеть запрос на доверие домену. Это произойдет, если сайт домена надстройки находится за пределами домена Exchange локальной или Office Online Server.
...
- При общем доступе к документу пользователи также совместно используют вставленные в него или связанные с ним надстройки. Если пользователь открывает документ, содержащий надстройку, которую пользователь не использовал ранее, клиентское приложение Office предложит пользователю предоставить разрешение на запуск надстройки в документе. В среде организации клиентское Office также запрашивает у пользователя, поступает ли документ из внешнего источника.

- Пользователи могут включить или отключить доступ к AppSource. Для контентных надстроек и надстроек области задач пользователи управляют доступом к доверенным надстройким и  каталогам из центра управления безопасностью на клиенте узла Office (открыт из каталогов надстроек **FileOptionsTrust** >  >  **CenterTrust** >  **Center Параметры** >  **Trusted**). Для Outlook надстройки могут управлять надстройки, нажав кнопку "Управление  надстройки". В Outlook на Windows выберите надстройки **FileManage** > . В Outlook Mac **нажмите кнопку** "Управление надстройки" на панели надстроек. В Outlook в Интернете выберите меню **Параметры** (значок шестеренки) > **Управление надстройками**. Администраторы также могут управлять доступом [с помощью групповой политики](/previous-versions/office/office-2013-resource-kit/jj219429(v=office.15)#using-group-policy-to-manage-how-users-can-install-and-use-apps-for-office).
...
- Структура платформы надстроек обеспечивает безопасность и производительность для конечных пользователей следующими способами.
  ...
  - Надстройка Office запускается в элементе управления веб-браузера, размещенном в среде выполнения надстройки, отдельно от Office клиентского приложения. Такая структура обеспечивает безопасность и изоляцию производительности от клиентского приложения.
...
Ниже перечислены некоторые конкретные рекомендации по защите персональных данных для разработчиков Office надстроек.
...
Дополнительные сведения о разрешениях для Outlook надстроек см. в следующих разделах.
...
Так Office надстройки — это веб-страницы, которые выполняются в элементе управления веб-браузера, они должны следовать политике одного источника, применяемой браузером. По умолчанию веб-страница в одном домене не может выполнять вызовы [веб-службы XmlHttpRequest](https://www.w3.org/TR/XMLHttpRequest/) к другому домену, отличному от домена, в котором она размещена.
...
Поскольку Office надстройки отображаются в iframe при запуске в браузере с клиентскими приложениями Office, воспользуйтесь приведенными ниже советами, чтобы свести к минимуму риск [](https://en.wikipedia.org/wiki/Clickjacking) взлома . Это метод, используемый злоумышленниками, чтобы поопречить пользователей от раскрытия конфиденциальной информации.
...
| Блокировать веб-надстройки | Позволяет запретить пользователям запускать Office надстройки, использующие веб-технологии. |
| Заблокировать Магазин Office |  Позволяет запретить пользователям получать или запускать надстройки Office, которые поступают из Office Store. |
