...
Высокоуровневое представление о том, где в настоящее время поддерживаются надстройки Office, можно узнать в статье [клиентские приложения и платформы Office для надстроек Office](../overview/office-add-in-availability.md).
...
Для контентных надстроек и надстроек области задач в разделе Поддерживаемые клиентские приложения Office — Excel, PowerPoint, Project или Word для отправки XML-файла манифеста надстройки также необходим [Каталог приложений](../publish/publish-task-pane-and-content-add-ins-to-an-add-in-catalog.md) в SharePoint, или необходимо развернуть надстройку с помощью [централизованного развертывания](../publish/centralized-deployment.md).
...
Outlook для Mac, распространяемый в составе Microsoft 365, поддерживает надстройки Outlook. Запуск надстроек Outlook в Outlook в Mac имеет те же требования, что и Outlook в Mac: операционная система должна быть как минимум OS X версии 10.10 "Yosemite". Outlook для Mac использует WebKit в качестве обработчика макетов для преобразования страниц надстройки, поэтому дополнительные зависимости от браузеров отсутствуют.
