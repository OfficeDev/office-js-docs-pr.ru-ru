...
- Создавайте надстройки, которые помогают пользователям выполнять свои задачи быстро и эффективно. Основное внимание следует уделить сценариям, применимым для приложений Office. Например:
  ...
  - Встраить дополнительные службы в Office приложения.
...
- Разработайте обучающий интерфейс, чтобы помочь пользователям и персонализировать среду.
  ...
  ![Снимок экрана, показывающий сравнение "Do" и "Don't". В примере "Do" показана надстройка, включаемая кнопку, которую пользователи могут щелкнуть, чтобы начать работу. В примере "Don't" показана надстройка без вводных действий или кнопок.](../images/contoso-part-catalog-do-dont.png)
...
- Если ваша контентная надстройка привязывается к данным в документе, включите пример данных или шаблон, чтобы показать пользователям рекомендуемый формат данных.
  ...
  ![Снимок экрана, показывающий сравнение "Do" и "Don't". В примере "Do" показана надстройка, включаемая кнопку, которую пользователи могут щелкнуть, чтобы вставить пример данных. В примере "Don't" показана надстройка без примерных данных или кнопок.](../images/add-in-title.png)
...
- Убедитесь, что внешний вид и поведение вашей надстройки согласованы с интерфейсом Office. См[. проект пользовательского интерфейса Office надстройки](../design/add-in-design.md).
...
- Используйте [свойство Context.touchEnabled](/javascript/api/office/office.context#office-office-context-touchenabled-member), чтобы определить, включено ли Office приложение, надстройка включена.
...
> [!NOTE]
> Если вы используете Fluent [интерфейса React](../design/using-office-ui-fabric-react.md) элементов дизайна, многие из этих элементов встроены в систему проектирования.
...
- Используйте сеть доставки контента (CDN) для содержимого изображений, ресурсов и общих библиотек. Загружайте как можно больше компонентов из одного источника.
...
- Свести к минимуму обмен данными между надстройки и Office документом. Дополнительные сведения см. [в рублях Избегайте использования метода context.sync в циклах](correlated-objects-pattern.md).
...
## <a name="use-javascript-that-supports-internet-explorer"></a>Использование JavaScript, который поддерживает Internet Explorer
