...
- Разработайте обучающий интерфейс, чтобы помочь пользователям и персонализировать среду.
  ...
  ![Снимок экрана: сравнение "Do" и "Не". В примере "Do" показана надстройка, содержащая кнопку, которую пользователи могут нажать, чтобы приступить к работе. В примере "Не" показана надстройка без вводных шагов или кнопок.](../images/contoso-part-catalog-do-dont.png)
...
- Если ваша контентная надстройка привязывается к данным в документе, включите пример данных или шаблон, чтобы показать пользователям рекомендуемый формат данных.
  ...
  ![Снимок экрана: сравнение "Do" и "Не". В примере "Do" показана надстройка, содержащая кнопку, которую пользователи могут нажать, чтобы вставить образцы данных. В примере "Не" показана надстройка без образца данных или кнопок.](../images/add-in-title.png)
...
- Убедитесь, что внешний вид и поведение вашей надстройки согласованы с интерфейсом Office. См [. раздел "Проектирование пользовательского интерфейса надстроек Office"](../design/add-in-design.md).
...
- Используйте свойство [Context.touchEnabled](/javascript/api/office/office.context#office-office-context-touchenabled-member) , чтобы определить, включено ли сенсорное управление приложением Office, на котором работает надстройка.
...
- Протестируйте надстройку на настоящем устройстве, выполнив [загрузку неопубликованного приложения ](../testing/sideload-an-office-add-in-on-ipad.md).

> [!NOTE]
> Если вы используете [fluent UI React](../design/using-office-ui-fabric-react.md) для элементов конструктора, многие из этих элементов встроены в систему проектирования.
...
- Сведите к минимуму обмен данными между надстройки и документом Office. Дополнительные сведения см. в [разделе "Избегайте использования метода context.sync в циклах"](correlated-objects-pattern.md).
...
## <a name="support-internet-explorer-recommended-but-not-required"></a>Поддержка Internet Explorer (рекомендуется, но не требуется)

См [. раздел "Поддержка Internet Explorer 11"](../develop/support-ie-11.md).
