...
- В сценарии, где надстройке требуется читать большой объем из неограниченного набора данных, можно применить разбиение на страницы во время чтения данных из таблицы или уменьшить размер данных в каждой небольшой операции чтения вместо попыток выполнить чтение за одну операцию. 
   ...
   Пример кода JavaScript и jQuery, иллюстрирующий разбитие потенциально длительной и ресурсоемкой серии операций ввода-вывода с неограниченными данными см. в статье [Как временно вернуть управление браузеру во время интенсивной обработки JavaScript](http://stackoverflow.com/questions/210821/how-can-i-give-control-back-briefly-to-the-browser-during-intensive-javascript). В этом примере используется метод [setTimeout](https://developer.mozilla.org/docs/Web/API/WindowOrWorkerGlobalScope/setTimeout) глобального объекта, чтобы ограничить продолжительность операций ввода и вывода. Кроме того, обрабатываются данные с разбивкой на конкретные фрагменты, а не данные неограниченного размера в произвольном порядке.
