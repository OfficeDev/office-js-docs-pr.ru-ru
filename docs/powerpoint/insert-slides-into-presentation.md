...
Ваша надстройка вставляет слайды из другого PowerPoint в текущую презентацию методом [Presentation.insertSlidesFromBase64](/javascript/api/powerpoint/powerpoint.presentation#powerpoint-powerpoint-presentation-insertslidesfrombase64-member(1)). Ниже приводится простой пример, в котором все слайды из презентации источника вставляются в начале текущей презентации, а вставленные слайды держат форматирование исходных файлов. Обратите внимание`chosenFileBase64`, что это глобальная переменная, которая содержит базовую версию файла PowerPoint презентации.
...
1. Вызов новой функции [PowerPoint.run()](/javascript/api/powerpoint#PowerPoint_run_batch_) главной функции и передать возвращаемую (сопутованую символу #) `targetSlideId` `InsertSlideOptions` ИД в качестве значения свойства параметра. Ниже приведен пример.
