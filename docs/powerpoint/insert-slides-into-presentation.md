...
Надстройка PowerPoint может вставлять слайды из одной презентации в текущую презентацию с помощью библиотеки JavaScript, определенной для приложений PowerPoint. Вы можете контролировать, будут ли вставлены слайды сохранять форматирование исходных презентаций или форматирование целевой презентации.
...
1. Начните с получения ссылки на исходный файл PowerPoint. В этом примере мы будем использовать управление типом, чтобы побудить пользователя `<input>` `file` выбрать файл. Добавьте следующую разметку на страницу надстройки.
    ...
    > [!NOTE]
    > Существует множество других способов получения файла PowerPoint. Например, если файл хранится в OneDrive или SharePoint, его можно скачать с помощью Microsoft Graph. Дополнительные сведения см. в [материалах Working with files in Microsoft Graph](/graph/api/resources/onedrive) and Access Files with Microsoft [Graph.](/learn/modules/msgraph-access-file-data/)
...
3. Добавьте в него указанный ниже код. Обратите внимание на следующий код:
...
Ваша надстройка вставляет слайды из другой презентации PowerPoint в текущую презентацию методом [Presentation.insertSlidesFromBase64.](/javascript/api/powerpoint/powerpoint.presentation#insertslidesfrombase64-base64file--options-) Ниже приводится простой пример, в котором все слайды из презентации источника вставляются в начале текущей презентации, а вставленные слайды держат форматирование исходных файлов. Обратите `chosenFileBase64` внимание, что это глобальная переменная, которая содержит базовую версию файла презентации PowerPoint.
...
1. Вызывайте новую функцию в [PowerPoint.run()](/javascript/api/powerpoint#PowerPoint_run_batch_) основной функции и передайте возвращаемую (сопощенную с символом #) ИД в качестве значения свойства `targetSlideId` `InsertSlideOptions` параметра. Ниже приведен пример.
