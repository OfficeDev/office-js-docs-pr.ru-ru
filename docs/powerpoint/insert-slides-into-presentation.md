...
ms.locfileid: "63747023"
---
# <a name="insert-slides-in-a-powerpoint-presentation"></a>Вставка слайдов в PowerPoint презентации

Надстройка PowerPoint может вставлять слайды из одной презентации в текущую презентацию с помощью PowerPoint конкретной библиотеки JavaScript в PowerPoint приложения. Вы можете контролировать, будут ли вставлены слайды сохранять форматирование исходных презентаций или форматирование целевой презентации.
...
1. Используйте метод `insertSlidesFromBase64` , чтобы вставить один или несколько слайдов из файла base64 в текущую презентацию.
...
1. Начните с получения ссылки на исходный PowerPoint файл. В этом примере мы будем использовать `<input>` управление типом, `file` чтобы побудить пользователя выбрать файл. Добавьте следующую разметку на страницу надстройки.
    ...
    ![Снимок экрана, показывающий элемент управления вводом типа HTML-файла, предшествующего инструкции по чтению предложения "Выберите презентацию PowerPoint, из которой вставить слайды". Управление состоит из кнопки с меткой "Выберите файл", за которой следует предложение "Нет выбранного файла".](../images/powerpoint-html-file-input-control.png)

    > [!NOTE]
    > Существует множество других способов получения PowerPoint файла. Например, если файл хранится в OneDrive или SharePoint, для его Graph Microsoft. Дополнительные сведения см. в материалах [Working with files in Microsoft Graph](/graph/api/resources/onedrive) [и Access Files with Microsoft Graph](/learn/modules/msgraph-access-file-data/).

2. Добавьте следующий код в JavaScript надстройки, чтобы назначить функцию событию управления входом `change` . (Вы создаете функцию `storeFileAsBase64` на следующем шаге.)
...
3. Добавьте в него указанный ниже код. Обратите внимание на указанные ниже аспекты этого кода.
    ...
    - Метод `reader.readAsDataURL` преобразует файл в base64 и сохраняет его в свойстве `reader.result` . Когда метод завершается, он запускает обработник `onload` событий.
    - Обработник `onload` событий отделяет метаданные от закодированного файла и сохраняет закодированную строку в глобальной переменной.
...
Ваша надстройка вставляет слайды из другого PowerPoint презентации в текущую презентацию методом [Presentation.insertSlidesFromBase64](/javascript/api/powerpoint/powerpoint.presentation#powerpoint-powerpoint-presentation-insertslidesfrombase64-member(1)). Ниже приводится простой пример, в котором все слайды из презентации источника вставляются в начале текущей презентации, а вставленные слайды держат форматирование исходных файлов. Обратите внимание`chosenFileBase64`, что это глобальная переменная, которая содержит базовую версию файла PowerPoint презентации.
...
- Функция будет вставлять слайды из презентации источника сразу после слайда, указанного свойством `targetSlideId` . Значение этого свойства — строка из трех возможных форм: ***nnn*#**, **#* mmmmmmm***, или **_nnnmmm_#****, где *nnn — это ID* слайда (обычно 3 цифры), а *mmmmm* — это код создания слайда (обычно 9 цифр). Некоторые примеры , `267#763315295`и `267#``#763315295`.
...
1. Создайте функцию, которая получает ID выбранного в настоящее время слайда с помощью [метода Office.context.document.getSelectedDataAsync](/javascript/api/office/office.document#office-office-document-getselecteddataasync-member(1)) общих API JavaScript. Ниже приведен пример. Обратите внимание, что вызов встроен `getSelectedDataAsync` в функцию возврата обещаний. Дополнительные сведения о том, почему и как это сделать, см. в Common-APIs [в функциях возврата обещаний](../develop/asynchronous-programming-in-office-add-ins.md#wrap-common-apis-in-promise-returning-functions).
...
1. Вызов новой функции [PowerPoint.run()](/javascript/api/powerpoint#PowerPoint_run_batch_) главной функции и передать возвращаемую (сопутованую символу #) `targetSlideId` `InsertSlideOptions` ID в качестве значения свойства параметра. Ниже приведен пример.
