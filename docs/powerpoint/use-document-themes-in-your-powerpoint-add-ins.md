...
Чтобы просмотреть или настроить полный набор из 12 цветов темы в PowerPoint, в группе **Variants** на вкладке **Дизайн** нажмите кнопку More **drop-down** — затем выберите **Цвета** Настраивайте цвета, чтобы отобразить диалоговое окно Create New  >   Theme **Colors.**
...
Чтобы просмотреть или настроить шрифты темы в PowerPoint, в группе **Variants** на вкладке **Дизайн** нажмите кнопку More **drop-down** — затем выберите **Шрифты** Настраивайте шрифты, чтобы отобразить диалоговое окно Create New Theme  >   **Fonts.**
...
Office также позволяет выбирать между несколькими стандартными темами, которые определяют несколько цветов и шрифтов, используемых в пользовательском интерфейсе всех приложений Office. Для этого используется отсев Office учетной записи файла  >    >   (из любого Office приложения).
...
Чтобы использовать файл OfficeThemes.css в проекте надстройки, добавьте тег, который ссылается на файл `<link>` OfficeThemes.css внутри тега `<head>` веб-страниц (например, .html, aspx или php-файл), реализуя пользовательский интерфейс надстройки в этом формате.
...
5. На страницах HTML, которые реализуют пользовательский интерфейс надстройки, например Home.html в шаблоне по умолчанию, добавьте следующий тег в тег, который ссылается на `<link>` `<head>` файл OfficeThemes.css.
...
Во время запуска при вставки в презентацию с Office **темой** по умолчанию надстройка контента отрисовкается таким образом.
