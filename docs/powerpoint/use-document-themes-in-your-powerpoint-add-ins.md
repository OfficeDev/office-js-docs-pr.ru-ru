...
5. На страницах HTML, реализуют пользовательский интерфейс надстройки, например Home.html в шаблоне по умолчанию, добавьте следующий тег в тег, который ссылается на `<link>` `<head>` файл OfficeThemes.css.
