...
Предварительная версия поддерживается только в подписке Microsoft 365 на Office в Windows.
...
1. Когда команда надстройки выполняет код вместо открытия области задач, она должна выполнять код в среде выполнения, которая отделена от внедренного веб-представления, в котором выполняется код области задач. Поэтому манифест должен указывать дополнительную среду выполнения. Прокрутите страницу до свойства `extension.runtimes` и добавьте следующий объект в массив `runtimes`. Обязательно добавьте запятую после объекта, который уже находится в массиве. Обратите внимание на следующие особенности этой разметки.
