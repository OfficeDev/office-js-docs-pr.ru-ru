...
||[id](/javascript/api/excel/excel.commentmention#id)|Идентификатор объекта. Идентификатор соответствует одному из идентификаторов в файле `CommentRichContent.richContent` .|
...
||[changeType](/javascript/api/excel/excel.worksheetrowhiddenchangedeventargs#changetype)|Получает тип изменения, которое представляет способ запуска события. `Excel.RowHiddenChangeType`Для получения дополнительных сведений см.|
