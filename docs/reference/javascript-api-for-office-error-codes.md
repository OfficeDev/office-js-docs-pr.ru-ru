...
В этой статье документироваться сообщения об ошибках, с которыми вы можете столкнуться при использовании модели общего API. Эти коды ошибок не применяются к API, определенным приложениям, например Excel API JavaScript или API Word JavaScript.

[Дополнительные подробности](../develop/understanding-the-javascript-api-for-office.md#api-models) о различиях между общим API и моделями API для приложений см. в моделях API.
...
|:-----|:-----|:-----|:-----|
|1000|Недопустимый тип приведения|Указанный тип приведения не поддерживается|Тип принуждения не поддерживается в Office приложении. Например, типы приведений OOXML и HTML не поддерживаются в Excel.|
...
|1008|Ошибка чтения данных|Размер запрошенного набора данных превышает допустимый.|Пользователь просит считыть данные за пределами ограничений данных, определенных Office приложения.|
...
|2009|Ошибка записи данных|Размер указанного объекта данных превышает допустимый.|Пользователь пытается установить данные за пределами ограничений данных, определенных Office приложения.|
...
|3010|Неподдерживаемая операция привязки|Выбранное содержимое должно быть представлено в формате таблицы. Отформатируйте данные как таблицу и повторите попытку.|Разработчик пытается использовать или методы `addRowsAsync` `deleteAllDataValuesAsync` `TableBinding` объекта на данных матрицы типа принуждения.|
...
|5000|Ошибка устаревших параметров|Операция не поддерживается.|Операция не поддерживается в текущем приложении Office. Например, `document.getSelectionAsync` называется из Outlook.|
...
|5004|Недопустимый вызов API|Недопустимый вызов API в текущем контексте.|Недействительный вызов для контекста, например, `CustomXMLPart` при попытке использовать объект в Excel.|
...
|5012|Недопустимый или истекший сеанс|Срок действия Office сеанса браузера истек или является недействительным. Для продолжения обновите страницу.|Истек срок действия сеанса между клиентом Office и сервером либо дата, время или часовой пояс на вашем компьютере установлены неправильно.|
...
|9020|Общая ошибка ответа|Внутренняя ошибка.|Относится к внутреннему условию ошибки, которое может возникать по любым причинам.|
|9021|Сохранение ошибки|Ошибка подключения произошла при попытке сохранить элемент на сервере.|Элемент не удалось сохранить. Это может быть связано с ошибкой подключения к серверу при использовании режима Outlook на рабочем столе или из-за попытки повторного сохранения черновика элемента, который был удален с Exchange сервера.|
|9022|Сообщение в разных магазинах ошибка|ID EWS не может быть извлечен, так как сообщение сохраняется в другом магазине.|ID EWS для текущего сообщения не удалось получить, так как сообщение могло быть перемещено или почтовый ящик может измениться.|
...
|9043|Тип вложения, не поддерживаемый|Тип вложения не поддерживается.|API не поддерживает тип вложения. Например, `item.getAttachmentContentAsync` если вложение является встроенным изображением в формате "Богатый текст", или если это тип элемента, кроме элемента электронной почты или календаря (например, контакта или элемента задачи).|
...
- [Обработка ошибок с Excel API JavaScript](../excel/excel-add-ins-error-handling.md)
