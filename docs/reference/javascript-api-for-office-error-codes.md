...
|3010|Неподдерживаемая операция привязки|Выбранное содержимое должно быть представлено в формате таблицы. Отформатируйте данные как таблицу и повторите попытку.|Разработчик пытается использовать или методы объекта для данных матрицы `addRowsAsync` `deleteAllDataValuesAsync` типов `TableBinding` принудительных _типов._|
...
|5004|Недопустимый вызов API|Недопустимый вызов API в текущем контексте.|Для контекста сделан недопустимый вызов, например при попытке использовать `CustomXMLPart` объект в Excel.|
...
|9021|Ошибка сохранения|При попытке сохранить элемент на сервере произошла ошибка подключения.|Не удалось сохранить элемент. Это может быть вызвано ошибкой подключения к серверу при использовании интерактивного режима в настольном компьютере Outlook или попыткой повторно сохранить черновик, удаленный с сервера Exchange.|
|9022|Ошибка сообщения в другом хранилище|Не удается получить ИД EWS, так как сообщение сохраняется в другом хранилище.|Не удалось получить ИД EWS для текущего сообщения, так как сообщение, возможно, было перемещено или почтовый ящик отправляя мог измениться.|
