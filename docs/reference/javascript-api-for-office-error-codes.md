...
|3010|Неподдерживаемая операция привязки|Выбранное содержимое должно быть представлено в формате таблицы. Отформатируйте данные как таблицу и повторите попытку.|Разработчик пытается использовать или методы `addRowsAsync` `deleteAllDataValuesAsync` `TableBinding` объекта для данных матрицы _типов_ приведения.|
