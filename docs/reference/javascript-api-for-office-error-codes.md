...
---
# <a name="office-common-api-error-codes"></a>Коды ошибок общего API Office

В этой статье печататся сообщения об ошибках, которые могут возникнуть при использовании модели общего API. Эти коды ошибок не применяются к API для конкретных приложений, таким как API JavaScript для Excel или JavaScript для Word.

См. [модели API,](../develop/understanding-the-javascript-api-for-office.md#api-models) чтобы узнать больше о различиях между общим API и моделями API для конкретных приложений. 
...
|:-----|:-----|:-----|:-----|
|1000|Недопустимый тип приведения|Указанный тип приведения не поддерживается|Тип приведите не поддерживается в приложении Office. Например, типы приведений OOXML и HTML не поддерживаются в Excel.|
...
|1008|Ошибка чтения данных|Размер запрошенного набора данных превышает допустимый.|Пользователь запрашивает чтение данных за пределами ограничений, определенных приложением Office.|
...
|2009|Ошибка записи данных|Размер указанного объекта данных превышает допустимый.|Пользователь пытается установить данные за пределы, определенные приложением Office.|
...
|3010|Неподдерживаемая операция привязки|Выбранное содержимое должно быть представлено в формате таблицы. Отформатируйте данные как таблицу и повторите попытку.|Разработчик пытается использовать или методы объекта для данных `addRowsAsync` `deleteAllDataValuesAsync` `TableBinding` матрицы типов приведите.|
...
|5000|Ошибка устаревших параметров|Операция не поддерживается.|Операция не поддерживается в текущем приложении Office. Например, `document.getSelectionAsync` он вызван из Outlook.|
...
|5004|Недопустимый вызов API|Недопустимый вызов API в текущем контексте.|Недопустимый вызов сделан для контекста, например при попытке использовать `CustomXMLPart` объект в Excel.|
...
|9022|Ошибка сообщения в другом хранилище|Не удается получить ИД EWS, так как сообщение сохранено в другом хранилище.|Не удалось получить ИД EWS для текущего сообщения, так как сообщение могло быть перемещено или почтовый ящик отправляя мог измениться.|
...
|9043|Тип вложения не поддерживается|Тип вложения не поддерживается.|API не поддерживает тип вложения. Например, эта ошибка высылается, если вложение является внедренным изображением в формате RICH Text или типом элемента, кроме элемента электронной почты или календаря (например, контакт или элемент `item.getAttachmentContentAsync` задачи).|
...
|13nnn|||См. [причины и обработку ошибок из getAccessToken.](../develop/troubleshoot-sso-in-office-add-ins.md#causes-and-handling-of-errors-from-getaccesstoken)|
...
- [Обработка ошибок с помощью API JavaScript для Excel](../excel/excel-add-ins-error-handling.md)
