...
|2009|Ошибка записи данных|Размер указанного объекта данных превышает допустимый.|Пользователь пытается задать данные за пределами данных, определяемых приложением Office.|
...
|3010|Неподдерживаемая операция привязки|Выбранное содержимое должно быть представлено в формате таблицы. Отформатируйте данные как таблицу и повторите попытку.|Разработчик пытается использовать `addRowsAsync` `deleteAllDataValuesAsync` методы `TableBinding` объекта для данных _матрицы_типов приведения типов.|
...
|5000|Ошибка устаревших параметров|Операция не поддерживается.|Операция не поддерживается в текущем приложении Office. Например, `document.getSelectionAsync` вызывается из Outlook.|
