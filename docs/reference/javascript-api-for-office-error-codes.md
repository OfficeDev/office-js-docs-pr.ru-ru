...
|3010|Неподдерживаемая операция привязки|Выбранное содержимое должно быть представлено в формате таблицы. Отформатируйте данные как таблицу и повторите попытку.|Разработчик пытается использовать `addRowsAsynch` `deleteAllDataValuesAsynch` методы `TableBinding` объекта для данных _матрицы_типов приведения типов.|
