...
|13nnn|*Неприменимо.*|*Неприменимо.*|См [. сведения о причинах и обработке ошибок в getAccessToken](../develop/troubleshoot-sso-in-office-add-ins.md#causes-and-handling-of-errors-from-getaccesstoken).|
...
|:-----|:-----|:-----|
|Matrix|Диапазон ячеек (в том числе части таблиц и отдельные ячейки)|Для выделенных ячеек `matrix` создается привязка типа. В документе не ожидается каких-либо изменений.|
...
|Указанный тип привязки|Выделенный фрагмент|Поведение|
|:-----|:-----|:-----|
|Matrix|Текст|Создать привязку невозможно.|
...
|Matrix|Диапазон в таблице|Создать привязку невозможно.|
...
|Table|Диапазон в таблице|Создать привязку невозможно.|
