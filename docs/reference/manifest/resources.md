...
> - Office надстройки требуют возможности кэшировать ресурсы изображений для целей производительности. Поэтому сервер, на котором размещен ресурс изображения, не должен добавлять директивы CACHE-CONTROL в заголовок ответа. Эти директивы Office автоматически заменять общее или по умолчанию изображение. Чтобы принудительно использовать новые значки на компьютере разработки, очистить [Office кэш](../../testing/clear-cache.md). Чтобы заставить использовать новые значки на компьютерах конечного пользователя, необходимо предоставить новым значкам другой URL-адрес от старых.
