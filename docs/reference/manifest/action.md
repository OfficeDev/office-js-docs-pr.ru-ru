...
Указывает действие, которое необходимо выполнить, когда пользователь выбирает элемент управления [Кнопка](control.md#button-control) или [Меню](control.md#menu-dropdown-button-controls).
...
|  [Title](#title) | Определяет настраиваемый заголовок области задач.|
...
Необязательный элемент, когда для атрибута **xsi:type** задано значение "ShowTaskpane". Определяет настраиваемый заголовок области задач для этого действия. 
...
> [!NOTE]
> Примечание. В настоящее время элемент SupportsPinning поддерживается только в Outlook 2016 для Windows (сборка 7628.1000 или более поздней версии).
