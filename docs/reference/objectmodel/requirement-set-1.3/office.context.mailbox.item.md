...
В следующем примере задается время окончания встречи с помощью [`setAsync`](/javascript/api/outlook_1_3/office.time#setasync-datetime--options--callback-) метода `Time` объекта.
