...
- Добавлена поддержка и `AttachmentsChanged` `EnhancedLocationsChanged` события.
...
- [Добавлена AttachmentDetailsCompose](/javascript/api/outlook/office.attachmentdetailscompose?view=outlook-js-1.8&preserve-view=true): добавляется новый объект, который представляет детали вложения в режиме Compose.
