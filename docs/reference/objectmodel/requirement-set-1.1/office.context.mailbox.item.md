...
|[Минимальный уровень разрешений](https://docs.microsoft.com/outlook/add-ins/understanding-outlook-add-in-permissions)| ReadItem|
|[Применимый режим Outlook](https://docs.microsoft.com/outlook/add-ins/#extension-points)| Compose|
...
Предоставляет доступ к получателям копии (Cc) сообщения. Уровень доступа и тип объекта, зависит от режима текущего элемента.
...
Получает идентификатор беседы по электронной почте, содержащей конкретное сообщение.
...
####  <a name="end-datetimejavascriptapioutlook11officetime"></a>end :Date|[Время](/javascript/api/outlook_1_1/office.time)
...
Добавляет к сообщению элемент Exchange, например дополнительное сообщение, в виде вложения.
...
Если любой из параметров строки превышает указанные для него ограничения, `displayReplyAllForm` возвращает исключение.

> [!NOTE]
> В наборе требований 1.1 не поддерживается возможность включения вложений в вызов `displayReplyAllForm`. Поддержка вложений была добавлена в `displayReplyAllForm` в наборах требований 1.2 и более поздних версий.
...
| `callback` | function | &lt;необязательно&gt; | По завершении работы метода функция, переданная в параметре `callback`, вызывается с помощью одного параметра `asyncResult`, представляющего собой объект [AsyncResult](/javascript/api/office/office.asyncresult). |
...
Если любой из параметров строки превышает указанные для него ограничения, `displayReplyForm` возвращает исключение.

> [!NOTE]
> В наборе требований 1.1 не поддерживается возможность включения вложений в вызов `displayReplyForm`. Поддержка вложений была добавлена в `displayReplyForm` в наборах требований 1.2 и более поздних версий.
