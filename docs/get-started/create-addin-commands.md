...
**Область применения:** Excel 2016 | Надстройки Office | PowerPoint 2016 | Word 2016
...
![Обзор элементов команд надстройки в манифесте](office-js-docs/images/080da303-51c4-4882-b74a-7ba11517c0ad.png)
 
## <a name="step-1-start-from-a-sample"></a>Этап 1. Использование примера для ознакомления

Мы настоятельно рекомендуем начать с одного из примеров, представленных в статье [Примеры команд надстройки для Office](https://github.com/OfficeDev/Office-Add-in-Command-Sample). При необходимости можно создать собственный манифест, выполнив действия, описанные в данном руководстве. Можно проверить свой манифест с помощью XSD-файла на сайте примеров команд для надстройки Office. Перед использованием команд надстройки обязательно прочитайте статью [Команды надстройки для Excel, Word и PowerPoint](https://dev.office.com/docs/add-ins/design/add-in-commands).

## <a name="step-2-create-a-task-pane-add-in"></a>Этап 2. Создание надстройки области задач

Чтобы начать использовать команды надстройки, сначала необходимо создать надстройку области задач, а затем изменить манифест надстройки, как описано в статье. Команды надстройки нельзя использовать с надстройками контента. При обновлении существующего манифеста, вы можете добавить в него элемент **VersionOverrides**, как описано в разделе [Этап 3. Добавление элемента VersionOverrides ](#Step-3:-Add-VersionOverrides-element).
...
|**Атрибут**|**Описание**|
|:-----|:-----|
|**xmlns** <br/> | Обязательный атрибут. Схема должна располагаться по адресу "http://schemas.microsoft.com/office/taskpaneappversionoverrides". <br/> |
...
**ВАЖНО!** Убедитесь, что код JavaScript вызывает `Office.initialize`. 
...
**ВАЖНО!** Убедитесь, что для элементов, которые содержат атрибут ID, указан уникальный идентификатор. Рекомендуем указать название компании и ваш идентификатор. Например, используйте следующий формат. > <CustomTab id="mycompanyname.mygroupname"> 
...
В Excel и Word можно добавить команды надстройки на ленту при помощи стандартных вкладок пользовательского интерфейса Office. В приведенной ниже таблице перечислены свойства, которые можно использовать для атрибута **id** элемента **OfficeTab**. В значениях учитывается регистр символов.
...
-  [Команды надстроек для Excel, Word и PowerPoint](https://dev.office.com/docs/add-ins/design/add-in-commands-for-excel-and-word-preview)
      
-  [Определение команд надстройки в манифесте](https://dev.office.com/docs/add-ins/develop/define-add-in-commands)  
