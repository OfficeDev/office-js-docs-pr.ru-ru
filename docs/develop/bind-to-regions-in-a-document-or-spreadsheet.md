...
После создания привязки с помощью одного из трех методов addFrom объекта можно работать с данными и свойствами привязки с помощью методов соответствующего `Bindings` объекта: [MatrixBinding,] [TableBinding]или [TextBinding]. Все три объекта наследуют методы [getDataAsync] и [setDataAsync] объекта `Binding`, позволяющие работать со связанными данными.
...
В следующем примере показано, как добавить привязку к существующему элементу с именем в качестве привязки "матрицы" с помощью метода `myRange` [addFromNamedItemAsync] и назначить привязку как `id` "myMatrix".
...
**Для Excel** параметр метода `itemName` [addFromNamedItemAsync] может относиться к существующему именоватому диапазону, диапазону, указанному со стилем ссылки `A1` или `("A1:A3")` таблицей. По умолчанию при добавлении таблиц в Excel имя "Table1" назначается первой добавленной таблице, "Table2" — второй таблице и так далее. Чтобы назначить значимое имя для таблицы в пользовательском интерфейсе Excel, используйте свойство в таблице `Table Name` **Tools | Дизайн** вкладки ленты.

> [!NOTE]
> В Excel при указании таблицы в качестве элемента с именем необходимо полностью указать имя, чтобы включить имя таблицы в имя таблицы в этом формате:`"Sheet1!Table1"`

В следующем примере создается привязка в Excel к первым трем ячейкам в столбце A ( ), назначает id, а затем записывает три названия городов к этому `"A1:A3"` `"MyCities"` привязке.
...
**Для Word** параметр метода `itemName` [addFromNamedItemAsync] относится к свойству `Title` управления `Rich Text` контентом. (`Rich Text` — единственный элемент управления содержимым, поддерживающий привязку.)

По умолчанию для управления контентом не `Title*` назначено значение. Чтобы назначить понятное имя в пользовательском интерфейсе Word, после вставки элемента управления контентом **Форматированный текст** из группы **Элементы управления** на вкладке **Разработчик** ленты выберите команду **Свойства** в группе **Элементы управления**, чтобы открыть диалоговое окно **Свойства элемента управления контентом**. Затем установите свойство управления контентом на имя, которое `Title` необходимо ссылаться в коде.

В следующем примере создается текстовая привязка в Word для богатого управления текстовым контентом с именем , назначает `"FirstName"` **id,** а затем `"firstName"` отображает эту информацию.
...
Анонимная функция, которая передается в функцию по мере выполнения параметра `callback` по завершению операции. Функция называется с одним параметром, который содержит `asyncResult` массив привязки в документе. Массив перебирается для создания строки, содержащей идентификаторы привязок. Строка отображается в окне сообщения.
...
В примере первым параметром является ID привязки для `id` получения.
...
В примере первым параметром является значение, на который необходимо установить `myBinding` значение . Так как привязка текстовая, этим значением будет `string`. Привязки разных типов принимают разные типы данных.

Анонимная функция, которая передается в функцию, является обратной функцией, которая выполняется после завершения операции. Функция называется с одним `asyncResult` параметром, который содержит состояние результата.
...
Функция, передаваемая в функцию в качестве параметра второго обработера, — обработник событий, который выполняется при смене данных в `dataChanged` привязке.  Функция вызывается с одним параметром _eventArgs_, который содержит ссылку на привязку. Эта привязка может использоваться для получения обновленных данных.
