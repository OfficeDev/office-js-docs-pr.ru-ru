...
По умолчанию диалоговое окно открывается в новом элементе управления веб-представлением, а не в iframe. Это гарантирует, что откроется страница входа поставщика удостоверений. Как показано далее в этой статье, характеристики диалогового окна Office определяют, каким образом вы используете библиотеки проверки подлинности и авторизации, такие как MSAL и Passport.

> [!NOTE]
> Чтобы настроить открытие диалогового окна в плавающем iframe, передайте параметр `displayInIframe: true` во время вызова `displayDialogAsync`. *Не* делайте этого, если для входа используется Dialog API для Office.
