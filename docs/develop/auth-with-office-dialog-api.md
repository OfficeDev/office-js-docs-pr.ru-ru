...
Всегда используйте API Dialog для Office с целью проверки подлинности и авторизации пользователей с помощью надстройки Office. API Dialog для Office также следует использовать, если вы реализуете резервную проверку подлинности, когда не удается применить единый вход.

Многие удостоверяющие центры, которые также называются службами маркеров безопасности (STS), не разрешают открывать свою страницу входа в элементе iframe. К ним относятся Google, Facebook и службы, защищенные платформой удостоверений Майкрософт (прежнее название — Azure AD 2.0), например учетная запись Майкрософт, Microsoft 365 для образования или рабочая учетная запись, а также другие распространенные учетные записи. При этом возникает проблема с надстройками Office, потому что при работе с надстройкой в **Office в Интернете** область задач является элементом iframe. Пользователи надстройки могут войти только в одну из этих служб, если она открывает полностью отдельный экземпляр браузера. Вот почему Office предоставляет [Dialog API для Office](dialog-api-in-office-add-ins.md), а именно метод [displayDialogAsync](/javascript/api/office/office.ui).
...
- Из диалогового окна информация может направляться обратно в область задач с помощью метода [messageParent](/javascript/api/office/office.ui#office-office-ui-messageparent-member(1)). Рекомендуется вызывать этот метод только на странице, размещенной в том же домене, что и область задач, включая протокол, поддомены и порт. В противном случае с вызовом метода и обработкой сообщения могут возникать сложности. Дополнительные сведения см. в разделе [Междоменные сообщения в основной среде выполнения](dialog-api-in-office-add-ins.md#cross-domain-messaging-to-the-host-runtime).

По умолчанию диалоговое окно открывается в новом элементе управления веб-представлением, а не в iframe. Это гарантирует, что откроется страница входа поставщика удостоверений. Как показано далее в этой статье, характеристики диалогового окна Office определяют, каким образом вы используете библиотеки проверки подлинности и авторизации, такие как библиотека проверки подлинности Майкрософт (MSAL) и Passport.

> [!NOTE]
> Чтобы настроить открытие диалогового окна в плавающем iframe, передайте параметр `displayInIframe: true` во время вызова `displayDialogAsync`. *Не* делайте этого, если для входа используется API Dialog для Office.
...
- Ваш код в диалоговом окне направляет маркер доступа в главное окно, преобразовывая его в строку с помощью функции `messageParent` или сохраняя его там, откуда главное окно сможет его извлечь (и с помощью `messageParent`, чтобы сообщить главному окну о готовности маркера). Пока срок действия этого маркера не истек, главное окно может использовать его для прямого доступа к ресурсам пользователя без дополнительных запросов.

Примеры проверки подлинности в надстройках, использующих Dialog API для Office, приводятся в разделе [Примеры](#samples).

## <a name="use-authentication-libraries-with-the-dialog-box"></a>Использование библиотек проверки подлинности с помощью диалогового окна

Так как диалоговые окна Office и область задач находятся в разных экземплярах браузера и среды выполнения JavaScript, использование библиотек проверки подлинности и авторизации отличается от того, как они используются при выполнении проверки подлинности и авторизации в одном окне. В следующих разделах описывается, как можно и нельзя использовать эти библиотеки.
...
> [!NOTE]
> \* Имеется ошибка, которая повлияет на вашу стратегию обработки маркеров. Если надстройка работает в **Office в Интернете** с использованием браузера Safari или Microsoft Edge, у диалогового окна и области задач нет общего локального хранилища, поэтому его нельзя использовать для связи между ними.
