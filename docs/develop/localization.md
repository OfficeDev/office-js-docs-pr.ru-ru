...
Вы можете реализовать любую схему локализации, которая подходит вашему Надстройка Office. API JavaScript и схема манифеста платформы Надстройки Office предоставляют несколько вариантов. С помощью API JavaScript для Office можно определить региональные и отображаемые строки на основе региональных точки приложения Office, а также интерпретировать или отображать данные на основе региональных точки данных. Вы можете использовать манифест, чтобы указать расположение файла надстройки и описательной информации, зависящих от языковых параметров. Либо можно использовать сценарий Microsoft Ajax для поддержки глобализации и локализации.
...
API JavaScript для Office предоставляет два свойства, которые поддерживают отображение или интерпретацию значений, согласованных с региональными значениями приложения Office и данных:

- [Context.displayLanguage][displayLanguage] указывает языковой интерфейс (или язык) пользовательского интерфейса приложения Office. В следующем примере проверяется, используется ли в приложении Office региональный код en-US или fr-FR, и отображается приветствие, относя которое к конкретному локальному коду.
...
> [!NOTE]
> Если вам необходимо локализовать несколько областей в семействе языков, например `de-de` и `de-at`, рекомендуется разделить элементы `Override` для каждой области. Использование только имени языка в данном случае не поддерживается для всех комбинаций клиентских приложений `de` и платформ Office.
...
> [!NOTE]
> Чтобы скачать Visual Studio 2019, см. страницу [Visual Studio IDE.](https://visualstudio.microsoft.com/vs/) Во время установки потребуется выбрать рабочую нагрузку разработки для Office и SharePoint.
...
Чтобы запустить предоставленный пример кода, настройте Office на компьютере для использования дополнительных языков, чтобы можно было протестировать надстройку, переключив язык, используемый для отображения в меню и командах, для редактирования и проверки или и того, и других.
...
3. Назовите проект **WorldReadyAddIn** и выберите **"Создать".**
...
Если вы хотите изменить локализацию надстройки в зависимости от языка, используемого для отображения в меню и командах в клиентском приложении Office, используйте свойство **Office.context.displayLanguage,** чтобы получить языковой стандарт для этого языка. Например, если язык приложения использует испанский для отображения в меню и командах, свойство **Office.context.displayLanguage** возвращает код языка es-ES.
...
После того как вы знаете язык, который использует приложение, вы можете использовать **UIStrings,** чтобы получить набор локализованных строк, который соответствует языку приложения.

Замените код в файле Home.js на следующий код. В коде показано, как можно изменить строки, используемые в элементах пользовательского интерфейса в Home.html, в зависимости от языка интерфейса приложения или языка редактирования приложения.
