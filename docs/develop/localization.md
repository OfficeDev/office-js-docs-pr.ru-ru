...
Вы можете реализовать любую схему локализации, которая подходит вашему Надстройка Office. API JavaScript и схема манифеста платформы Надстройки Office предоставляют несколько вариантов. Вы можете использовать API JavaScript для Office для определения языкового стандарта и отображаемых строк на основе языкового стандарта приложения Office, а также для интерпретации или отображения данных на основе языкового стандарта данных. Вы можете использовать манифест, чтобы указать расположение файла надстройки и описательной информации, зависящих от языковых параметров. Либо можно использовать сценарий Microsoft Ajax для поддержки глобализации и локализации.
...
API JavaScript для Office предоставляет два свойства, которые поддерживают отображение и интерпретацию значений, совместимых с региональными параметрами приложения и данных Office:

- [Context. displayLanguage][displayLanguage] задает языковой стандарт (или язык) пользовательского интерфейса приложения Office. В следующем примере проверяется, используется ли в приложении Office Национальная настройка en/US или fr – FR, и отображается приветствие, зависящее от языкового стандарта.
...
> [!NOTE]
> Если вам необходимо локализовать несколько областей в семействе языков, например `de-de` и `de-at`, рекомендуется разделить элементы `Override` для каждой области. Использование только имени языка в данном случае `de` не поддерживается во всех комбинациях клиентских приложений и платформ Office.
...
Чтобы запустить приведенный пример кода, Настройте Office на вашем компьютере для использования дополнительных языков, чтобы можно было протестировать надстройку, переключив язык, используемый для отображения в меню и командах, для редактирования и проверки правописания.
...
Если вы хотите изменить локализацию надстройки на основе языка, используемого для отображения в меню и командах в клиентском приложении Office, используйте свойство **Office. Context. displayLanguage** , чтобы получить языковой стандарт. Например, если язык приложения использует Испанский для отображения в меню и командах, свойство **Office. Context. displayLanguage** будет возвращать код языка es – ES.

Если вы хотите изменить локализацию для надстройки на основе языка, используемого для редактирования контента документа, используйте свойство **Office. Context. contentLanguage** , чтобы получить языковой стандарт. Например, если язык приложения использует Испанский для редактирования контента документа, свойство **Office. Context. contentLanguage** будет возвращать код языка es – ES.

Когда вы узнаете, какой язык использует приложение, вы можете использовать **UIStrings** для получения набора локализованных строк, соответствующих языку приложения.

Замените код в файле Home.js на следующий код. В коде показано, как можно изменить строки, используемые в элементах пользовательского интерфейса, на Home.html на основе языка интерфейса приложения или языка редактирования приложения.
...
Чтобы протестировать локализованную надстройку, измените язык, используемый для отображения или редактирования в приложении Office, а затем запустите свою надстройку.
