...
Попытка отобразить диалоговое окно при Office в Интернете может привести к блокировке блокатора всплывающих окна браузера. Если это произойдет, Office в Интернете откроет запрос, аналогичный следующему.
...
Не может быть открыто несколько диалогов с данной хост-страницы, поэтому перед открытием другого диалоговое окно код должен вызвать [Диалог.закрыть](/javascript/api/office/office.dialog#office-office-dialog-close-member(1)) `displayDialogAsync` в открытом диалоговом ок. Метод `close` асинхронный. По этой причине, `displayDialogAsync` `close`если вы звоните сразу после звонка, первый диалог может не закрыться полностью, Office при попытке открыть второй. Если это произойдет, Office возвращает ошибку [12007](dialog-handle-errors-events.md#12007): "Операция не удалась, так как эта надстройка уже имеет активный диалог".
