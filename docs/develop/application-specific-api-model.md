...
---
# <a name="using-the-application-specific-api-model"></a>Использование модели API, зависящей от приложения
...
Некоторые методы и свойства метода доступа создают исключение, если нужный объект не существует. Например, если вы попытаетесь получить лист Excel, указав имя листа, которого нет в книге, `getItem()` метод создаст `ItemNotFound` исключение.

Любой `*OrNullObject` вариант позволяет проверить объект, не вызывая исключения. Эти методы и свойства возвращают нулевой объект (а не JavaScript `null` ), а не выдают исключение, если указанный элемент не существует. Например, вы можете вызвать `getItemOrNullObject()` метод для коллекции, например, с помощью **листов** , чтобы получить элемент из коллекции. Метод `getItemOrNullObject()` возвращает указанный элемент, если он существует. В противном случае возвращается нулевой объект. Возвращаемый нулевой объект содержит логическое свойство `isNullObject`, с помощью которого можно определить, существует ли объект.

Следующий пример кода пытается извлечь лист Excel с именем "Data" с помощью `getItemOrNullObject()` метода. Если метод возвращает нулевой объект, создается новый лист перед выполнением действий на листе.
...
* [Распространенные проблемы кодирования и неожиданное поведение платформы](/common-coding-issues.md).
