...
ms.locfileid: "25681842"
---
# <a name="create-and-debug-office-add-ins-in-visual-studio"></a>Создание и отладка надстроек Office в Visual Studio

Из этой статьи можно узнать, как создать свою первую надстройку Office с помощью Visual Studio. Действия, описанные в этой статье, относятся к версии Visual Studio 2017. Если вы используете другую версию Visual Studio, действия могут немного отличаться.
...
- Или вы можете выбрать **Вставить содержимое в слайды PowerPoint** для создания контентной надстройки.

**Excel** 
- Вы можете выбрать **Добавить новые функциональные возможности в Excel** для создания надстройки области задач.
- Или вы можете выбрать **Вставить содержимое в электронную таблицу Excel** для создания контентной надстройки.
    - При создании контентной надстройки вы также можете выбрать вариант **Базовая надстройка**, при котором создается проект надстройки с минимальным начальным кодом.
...
|**Home.js**|Файл JavaScript, связанный с страницей Home.html. Вы можете поместить любой код, специфичный для поведения страницы Home.html в файле Home.js. Файл Home.js содержит пример кода, который поможет вам начать работу.|
|**Home.css**|Определение стилей по умолчанию для применения к надстройке. для разработки дизайна и стилей мы рекомендуем использовать структуры Office UI Fabric. Подробнее см. статью [Office UI Fabric в надстройке Office](../design/office-ui-fabric.md).|

> [!NOTE]
> Вам не обязательно использовать эти файлы. Вы можете добавить другие файлы в проект и вместо этого использовать их. Если вы хотите, чтобы в качестве начальной страницы надстройки отображался другой HTML-файл, откройте редактор манифестов и задайте значение для свойства **SourceLocation**, указав имя нужного файла.
...
|**Документ при запуске** (только контентные надстройки и надстройки области задач)|Указывает, какой документ следует открыть при запуске проекта.|
...
7. В окне свойств выберите список **Документ при запуске**, а затем документ, который добавлен в проект. Теперь проект будет настроен на запуск надстройки в существующем документе.
...
Можно также изменить код и просмотреть результаты этих изменений в надстройке Outlook без необходимости закрытия надстройки Office и перезапуска проекта. В Outlook следует просто открыть контекстное меню для надстройки Outlook и нажать кнопку **Обновить**.
