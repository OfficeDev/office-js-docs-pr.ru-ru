...
3. В строке, назначенной свойству `issuer`, вы увидите заполнитель *{O365 tenant GUID}*. Замените его идентификатором клиента Office 365. Чтобы получить его, используйте один из способов, описанных в статье [Поиск идентификатора клиента Office 365](https://docs.microsoft.com/onedrive/find-your-office-365-tenant-id). В результате значение свойства `issuer` должно выглядеть примерно так:
...
5. Замените `TODO8` приведенным ниже кодом. Обратите внимание на следующие особенности этого кода:
...
1. Если вкладка **Разработчик** не отображается на ленте, включите ее с помощью следующих действий:
 ...
 2. Установите флажок, чтобы включить **разработчик** в дереве имен элементов управления в правой части страницы **Настройка ленты**.
 3. Нажмите кнопку **ОК**.
...
3. Если вы не вошли в Office, откроется всплывающее окно с предложением войти. Список файлов и папок появится через несколько секунд после входа. *Повторно нажимать кнопку не следует.*
