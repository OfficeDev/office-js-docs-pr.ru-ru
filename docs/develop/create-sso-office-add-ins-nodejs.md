...
1. Войдите ***в клиент Microsoft*** 365 с учетными данными администратора. Пример: MyName@contoso.onmicrosoft.com.
...
1. Выберите пункт **Предоставление API** в разделе **Управление**. Выберите ссылку **\<Set\>** . При этом будет создан URI идентификатора приложения в формате api://$App ID GUID$, где $App ИДЕНТИФИКАТОР GUID$ — это идентификатор приложения **(клиента**).
...
1. Используйте поле поиска **Выбрать разрешения**, чтобы найти нужные разрешения для надстройки. Выберите следующие параметры. Только первое действительно требуется самой надстройке. Но приложение `profile` Office должно получить маркер для веб-приложения надстройки.
...
1. Замените `TODO 3` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * Если для клиента Microsoft 365 настроена многофакторная проверка подлинности, `exchangeResponse` `claims` это свойство будет содержать сведения о дополнительных обязательных факторах. В этом случае следует снова вызвать `Office.auth.getAccessToken` с присвоением параметру `authChallenge` значения свойства утверждений. В результате AAD предложит пользователю пройти все необходимые проверки подлинности.
...
1. Под методом `getGraphData` добавьте следующую функцию. Обратите внимание`/auth`, что это серверный express-маршрут, который обменивается маркером начальной загрузки с Azure AD маркер доступа с разрешениями для Microsoft Graph.
...
1. В редких случаях маркер начальной загрузки, кэшированный Office, не используется, когда Office проверяет его, но срок его действия истекает к Azure AD для обмена. Служба Azure AD ответит ошибкой **AADSTS500133**. В этом случае надстройка должна просто позвонить еще `getGraphData` раз. Так как срок действия кэшированного маркера начальной загрузки истек, Office получит новый маркер из Azure AD. Поэтому замените `TODO 8` его следующим кодом.
