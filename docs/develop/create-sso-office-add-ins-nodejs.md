...
description: Узнайте, как создать надстройку на Node.js, которая использует Office единого входа.
ms.date: 06/10/2022
...
1. Войдите с ***учетными*** данными администратора в Microsoft 365 клиента. Пример: MyName@contoso.onmicrosoft.com.
...
1. Выберите **Новая регистрация**. На странице **Зарегистрировать приложение** задайте необходимые значения следующим образом.
    ...
    * Задайте тип приложения **"Интернет"**, а затем **задайте URI перенаправления**`https://localhost:44355/dialog.html`.
...
1. Выберите **Проверка подлинности** в разделе **Управление**. В разделе **"Неявное предоставление** " включите флажки для маркера **доступа** и **маркера идентификатора**. В примере используется резервная система авторизации, вызываемая при недоступности единого входа. В этой системе используется неявный поток.

1. Щелкните **Сохранить** в верхней части формы.

1. Выберите **Сертификаты и секреты** в разделе **Управление**. Нажмите кнопку **Новый секрет клиента**. Введите значение параметра **Описание**, выберите соответствующий вариант для параметра **Истекает срок действия** и нажмите кнопку **Добавить**.
    
    Веб-приложение использует секрет клиента для подтверждения своей личности при запросе маркеров. *Запишите это значение для использования на следующем шаге — оно отображается только один раз.*
    
1. Выберите пункт **Предоставление API** в разделе **Управление**. Щелкните **ссылку "Задать** ". При этом будет создан URI идентификатора приложения в формате api://$App ID GUID$, где $App ИДЕНТИФИКАТОР GUID$ — это идентификатор приложения **(клиента**).

1. В созданном идентификаторе вставьте (обратите внимание на косую черту "/", `localhost:44355/` добавленную к концу) между двойной косой чертой и GUID. По завершении весь идентификатор должен иметь форму `api://localhost:44355/$App ID GUID$`, например `api://localhost:44355/c6c1f32b-5e55-4997-881a-753cc1d563b7`.
...
1. `access_as_user` Заполните поля для настройки запросов согласия администратора и пользователя значениями, подходящими для области, которая позволяет клиентским приложениям Office использовать веб-API надстройки с правами текущего пользователя. Предложения:
    ...
    * **Описание согласия пользователя**: Office вызывать веб-API надстройки с тем же правами, что и у вас.
...
1. Нажмите кнопку **Добавить клиентское приложение**, в открывшейся панели присвойте параметру "Идентификатор клиента" соответствующий код GUID и установите флажок `api://localhost:44355/$App ID GUID$/access_as_user`.
...
1. Используйте поле поиска **Выбрать разрешения**, чтобы найти нужные разрешения для надстройки. Выберите следующие параметры. Только первое действительно требуется самой надстройке. но для `profile` получения маркера для веб-приложения надстройки Office требуется разрешение.
...
1. Откройте файл манифеста надстройки manifest\manifest_local.xml и прокрутите его до конца. Сразу над конечным `</VersionOverrides>` тегом вы найдете следующую разметку.
...
1. Замените заполнитель "$application_GUID here$" *в обоих местах* разметки идентификатором приложения, скопированным при регистрации надстройки. Символы "$" не входят в состав идентификатора, их не нужно вставлять. Это тот же идентификатор, который вы использовали для CLIENT_ID и аудитории в . ENV-файл.
...
1. Замените `TODO 1` приведенным ниже кодом. Обратите внимание на следующие сведения об этом коде:

    * `Office.auth.getAccessToken` предписывает Office получить маркер начальной загрузки из Azure AD. Маркер начальной загрузки — это маркер идентификатора, `scp` но он также имеет свойство (область) со значением `access-as-user`. Этот маркер может быть обменяться веб-приложением на маркер доступа с разрешениями для Microsoft Graph.
    * Установка значения `allowSignInPrompt` true означает, что если пользователь в настоящее время не Office, Office откроет всплывающее окно входа.
    * Установка значения `allowConsentPrompt` true означает, что если пользователь не предоставил надстройке доступ к профилю AAD пользователя, Office запрос на согласие. (Запрос позволяет пользователю давать согласие только на профиль AAD пользователя, а не на microsoft Graph области.)
    * `forMSGraphAccess` Установка для параметра true сигналов Office что надстройка намерена использовать маркер начальной загрузки для получения дополнительного маркера доступа с разрешениями для Microsoft Graph, а не просто использовать его в качестве маркера идентификатора. Если администратор клиента не предоставил согласие на доступ надстройки к Microsoft Graph, `Office.auth.getAccessToken` возвращает ошибку **13012**. Надстройка может отреагировать переходом на альтернативную систему проверки подлинности. Это необходимо, так как Office может запрашивать согласие только на доступ к профилю пользователя Azure AD, а не к областям Microsoft Graph. Резервная система авторизации требует, чтобы пользователь снова вошел в систему, и  пользователю может быть предложено дать согласие на доступ к microsoft Graph области. Таким образом, параметр `forMSGraphAccess` обеспечивает, что надстройка не будет выполнять замену маркера, которая завершится ошибкой из-за отсутствия согласия. (Так как вы предоставили согласие администратора на предыдущем шаге, этот сценарий не возникнет для этой надстройки. Но этот параметр добавлен в любом случае, чтобы продемонстрировать рекомендацию.)
...
1. Замените `TODO 5` следующим кодом:
...
1. Под методом `getGraphData` добавьте следующую функцию. Обратите внимание`/auth`, что это серверный express-маршрут, который обменивается маркером начальной загрузки с Azure AD на маркер доступа с разрешениями microsoft Graph.
...
Дополнительные сведения об этих ошибках см. в статье [Устранение ошибок единого входа в надстройках Office](troubleshoot-sso-in-office-add-ins.md).
...
1. В редких случаях маркер начальной загрузки, Office кэшированный, не используется, когда Office проверяет его, но истекает к моменту, когда он достигнет Azure AD для обмена. Служба Azure AD ответит ошибкой **AADSTS500133**. В этом случае надстройка должна просто позвонить еще `getGraphData` раз. Так как срок действия кэшированного маркера начальной загрузки истек, Office получит новый маркер из Azure AD. Поэтому замените `TODO 8` его следующим кодом.
...
1. Замените `TODO 9` следующим кодом:
...
1. Замените `TODO 13` приведенным ниже кодом. Обратите внимание на следующие сведения об этом коде:
    ...
    * Параметру формы `requested_token_use` присвоено значение 'on_behalf_of'. Это Azure AD, что надстройка запрашивает маркер доступа к Microsoft Graph с помощью потока On-Behalf-Of (OBO). Azure отвечает, проверяя, что маркер начальной загрузки, `assertion` назначенный параметру формы, `scp` имеет свойство, для которого задано значение `access-as-user`.
...
1. Замените `TODO 14` приведенным ниже кодом, дополняющим блок `else`. Обратите внимание на следующие сведения об этом коде:

    * Константе `tenant` присвоено значение 'common', так как вы сделали надстройку мультитенатной при ее регистрации в Azure AD; в частности, когда назначили параметру **Поддерживаемые типы учетных записей** значение **Учетные записи в любом каталоге организации и персональные учетные записи Майкрософт (например, Skype, Xbox, Outlook.com)**. Если вместо этого вы выбрали поддержку только учетных записей в том же клиенте Microsoft 365, где зарегистрирована надстройка, `tenant` то в этом коде будет задан GUID клиента.
