...
1. Замените `TODO 1` на приведенный ниже код. Обратите внимание на следующие сведения об этом коде:
    ...
    * `allowConsentPrompt` Установка значения true означает, что если пользователь не предоставил надстройке доступ к профилю AAD пользователя, Office запрос на согласие. (Запрос позволяет пользователю дать согласие только на профиль AAD пользователя, а не на Graph Microsoft.)
...
1. Под методом `getGraphData` добавьте следующую функцию. Обратите внимание, что это серверный express-маршрут, который обменивается маркером начальной загрузки с Azure AD на маркер доступа `/auth` с разрешениями microsoft Graph.
...
1. В редких случаях маркер начальной загрузки, кэшированный Office кэширования, не используется, Office проверяется, но истекает к моменту, когда он достигает Azure AD для обмена. Служба Azure AD ответит ошибкой **AADSTS500133**. В этом случае надстройка должна просто позвонить еще `getGraphData` раз. Так как срок действия кэшированного маркера начальной загрузки истек, Office получит новый маркер из Azure AD. Поэтому замените `TODO 8` его следующим кодом.
