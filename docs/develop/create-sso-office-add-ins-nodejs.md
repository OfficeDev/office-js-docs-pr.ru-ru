...
1. Выберите пункт **Предоставление API** в разделе **Управление**. Выберите **ссылку Set** . Это позволит создать URI ID приложения в виде Api://$App ID GUID$", в котором $App GUID является ID-адрес приложения **(клиента).**
...
1. Используйте поле поиска **Выбрать разрешения**, чтобы найти нужные разрешения для надстройки. Выберите следующие параметры. Только первая действительно требуется самой надстройки; но для `profile` приложения требуется разрешение Office для получения маркера в веб-приложении надстройки.
...
1. Замените `TODO 1` на приведенный ниже код. В этом коде обратите внимание на следующее:
    ...
    * Настройка параметра `allowSignInPrompt` true означает, что если пользователь в настоящее время не Office, Office откроет всплывающее подсказок для регистрации.
    * `allowConsentPrompt` Настройка параметра true означает, что если пользователь не дал согласия на доступ к профилю AAD надстройки, Office откроет запрос на согласие. (Подсказка позволяет только пользователю соглашаться на AAD, а не на области Microsoft Graph.)
