...
* Office 2016 версии 1708 (сборка 8424.nnnn) или более поздней (версия подписки на Office 365, иногда называемая "нажми и работай").

  Для получения этой версии может понадобиться быть участником программы предварительной оценки Office. Дополнительные сведения см. в статье [Примите участие в программе предварительной оценки Office](https://products.office.com/en-us/office-insider?tab=tab-1).
...
2. Откройте консоль Git bash в папке **Before**.

3. Введите в консоли команду `npm install`, чтобы установить все зависимости, указанные в файле package.json.

4. Введите в консоли команду `npm run build `, чтобы выполнить сборку проекта. 
...
1. Когда откроется страница настройки, скопируйте и сохраните **идентификатор приложения**. Он понадобится вам позже. 
    ...
    > [!NOTE]
    > Этот идентификатор представляет собой значение аудитории, используемое, когда другие приложения, например ведущее приложение Office (PowerPoint, Word, Excel и т. д.), пытаются получить авторизованный доступ к вашему приложению. Кроме того, он используется как идентификатор клиента, когда ваше приложение пытается получить авторизованный доступ к Microsoft Graph.
...
1. Будет создан **URI идентификатора приложения** в формате "api://{GUID идентификатора приложения}". Вставьте строку "localhost:3000" между двойной косой чертой и GUID. Весь идентификатор должен выглядеть следующим образом: `api://localhost:3000/{App ID GUID}`. 

    > [!NOTE]
    > Доменная часть имени в поле **Область**, указанная под **URI идентификатора приложения**, автоматически изменится соответствующим образом. Она должна выглядеть так: `api://localhost:3000/{App ID GUID}/access_as_user`.
...
1. Откройте раскрывающийся список **Область** рядом с каждым полем **Идентификатор приложения** и установите флажок `api://localhost:3000/{App ID GUID}/access_as_user`.
...
1. В диалоговом окне установите флажки следующих разрешений: 
    ...
    * profile
    ...
    > [!NOTE]
    > Разрешение `User.Read` может быть уже указано по умолчанию. Незачем запрашивать ненужные разрешения, поэтому рекомендуем снять флажок этого разрешения.
...
> [!NOTE]
> Эту процедуру необходимо выполнять только при разработке надстройки. После развертывания рабочей надстройки в AppSource или каталоге надстроек пользователи сделают ее доверенной в индивидуальном порядке во время установки.
...
1. Затем вам будет предложено предоставить надстройке разрешение на доступ к данным Microsoft Graph. Нажмите кнопку **Принять**. 

1. После этого будет выполнено перенаправление на **URL-адрес перенаправления**, указанный при регистрации надстройки, поэтому, если надстройка запущена, в браузере откроется ее домашняя страница. Если надстройка не запущена, возникнет ошибка и отобразится сообщение о том, что не удается найти или открыть ресурс по адресу localhost:3000. *Однако сам факт попытки перенаправления свидетельствует о том, что согласие администратора успешно предоставлено*. Таким образом, независимо от того, открылась ли домашняя страница, вы можете переходить к следующему этапу.
...
1. Над закрывающим тегом `</VersionOverrides>` вы найдете следующий код:
...
1. Замените заполнитель {application_GUID here} *в обоих местах* кода идентификатором приложения, скопированным во время регистрации надстройки. (Фигурные скобки не являются частью идентификатора, поэтому не нужно их включать.) Это тот же идентификатор, который использовался для ClientID и Audience в файле web.config.
...
11. После назначения для метода `Office.initialize` добавьте приведенный ниже код. Вот что нужно знать об этом коде:
    ...
    * Вам нужно создать метод `getDataWithToken` на следующем этапе. Но обратите внимание на то, что он задает параметру `forceConsent` значение `false`. Дополнительные сведения см. в описании следующего этапа.
...
1. Под методом `getOneDriveFiles` добавьте приведенный ниже код. Вот что нужно знать об этом коде:
    ...
    * Вы создадите метод `handleClientSideErrors` на одном из следующих этапов.
...
1. Под методом `getOneDriveFiles` добавьте приведенный ниже код. Вот что нужно знать об этом коде:
    ...
    * Вы создадите метод `handleServerSideErrors` на одном из следующих этапов.
...
1. Замените `TODO3` приведенным ниже кодом. Ошибка 13002 возникает, если вход или разрешение пользователя прерывается. Попросите пользователя повторить попытку, но не более одного раза.
...
1. Замените `TODO4` приведенным ниже кодом. Ошибка 13003 возникает, когда пользователь входит под учетной записью, отличной от рабочей, учебной или личной учетной записи Майкрософт. Попросите пользователя выйти, а затем войти с помощью учетной записи поддерживаемого типа.
    ...
    > [!NOTE]
    > Ошибки 13004 и 13005 не обрабатываются при использовании этого метода, так как они должны возникать только на стадии разработки. Их невозможно исправить с помощью кода среды выполнения, поэтому нет смысла сообщать о них пользователю.

1. Замените `TODO5` приведенным ниже кодом. Ошибка 13006 возникает, если происходит неопределенная ошибка ведущего приложения Office, которая может свидетельствовать о его нестабильном состоянии. Попросите пользователя перезапустить Office.
...
1. Замените `TODO7` приведенным ниже кодом. Ошибка 13008 возникает, когда пользователь запускает операцию, которая вызывает `getAccessTokenAsync`, до завершения предыдущего вызова.
...
1. Замените `TODO10` приведенным ниже кодом. Вот что нужно знать об этом коде:
...
1. Замените `TODO11` приведенным ниже кодом *непосредственно под последней закрывающей фигурной скобкой кода, который вы добавили на предыдущем этапе*. Вот что нужно знать об этом коде:

    * Ошибка 65001 означает, что согласие на доступ к Microsoft Graph не предоставлено (или было отозвано) для одного или нескольких разрешений. 
...
1. Замените `TODO12` приведенным ниже кодом *непосредственно под последней закрывающей фигурной скобкой кода, который вы добавили на предыдущем этапе*. Вот что нужно знать об этом коде:
...
1. Замените `TODO13` приведенным ниже кодом *непосредственно под последней закрывающей фигурной скобкой кода, который вы добавили на предыдущем этапе*. Вот что нужно знать об этом коде:
...
1. Замените `TODO14` приведенным ниже кодом *непосредственно под последней закрывающей фигурной скобкой кода, который вы добавили на предыдущем этапе*. Вот что нужно знать об этом коде:
...
1. Замените `TODO15` приведенным ниже кодом *непосредственно под последней закрывающей фигурной скобкой кода, который вы добавили на предыдущем этапе*.
...
2. Замените `TODO3` приведенным ниже кодом. Вот что нужно знать об этом коде:
...
3. Замените `TODO4` приведенным ниже кодом, который отправляет HTTP-запрос конечной точке маркера для службы токенов безопасности.
...
4. Замените `TODO5` приведенным ниже кодом. Обратите внимание на то, что выдача исключения *не* приводит к немедленной отправке текста "500 внутренняя ошибка сервера" клиенту. Вызов кода в файле server.js захватывает данное исключение и преобразует его в сообщение об ошибке, отправляемое клиенту.
...
6. Сохраните файл, но не закрывайте его.
...
6. Замените `TODO9` приведенной ниже строкой. Обратите внимание на указанные ниже особенности этого кода.
