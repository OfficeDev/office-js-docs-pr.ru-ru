...
description: Узнайте, как создать надстройку на Node.js, использующую Office один вход.
ms.date: 01/25/2022
...
1. Выберите **Проверка подлинности** в разделе **Управление**. В разделе **Неявный грант** впускаете почтовые ящики для маркера **Доступа и** **маркера ID**. В примере используется резервная система авторизации, вызываемая при недоступности единого входа. В этой системе используется неявный поток.
...
1. Выберите пункт **Предоставление API** в разделе **Управление**. Выберите **ссылку Set** . Это позволит создать URI ID приложения в виде "api://$App ID GUID$", в котором $App GUID ID является ИД приложения **(клиента).**

1. В сгенерированном ID `localhost:44355/` вставьте (обратите внимание на переназначенную полосу "/" прим. в конце) между двумя полосами вперед и GUID. Когда вы закончите, весь ID должен иметь форму `api://localhost:44355/$App ID GUID$`; например `api://localhost:44355/c6c1f32b-5e55-4997-881a-753cc1d563b7`.
...
1. `access_as_user` Заполните поля для настройки запросов на согласие администратора и пользователя со значениями, подходящими для области, которая позволяет приложению Office использовать веб-API надстройки с тем же правами, что и текущий пользователь. Предложения:
    ...
    * **Описание согласия пользователя**: Office для вызова веб-API надстройки с тем же правами, что и у вас.
...
1. Используйте поле поиска **Выбрать разрешения**, чтобы найти нужные разрешения для надстройки. Выберите следующие параметры. Только первая действительно требуется самой надстройки; но для `profile` приложения Office разрешение на получение маркера в веб-приложении надстройки.
...
1. Откройте файл манифеста надстройки manifest\manifest_local.xml и прокрутите его до конца. Чуть выше конца `</VersionOverrides>` тега вы найдете следующую разметку.
...
1. Добавьте следующий код ниже метода Office.onReady.
...
1. Замените `TODO 1` на приведенный ниже код. В этом коде обратите внимание на следующее:

    * `Office.auth.getAccessToken` предписывает Office получить маркер начальной загрузки из Azure AD. Маркер bootstrap — это маркер ID, `scp` но он также имеет свойство (область) со значением `access-as-user`. Этот маркер может быть обменяно веб-приложением для маркера доступа с разрешениями microsoft Graph.
    * Настройка параметра `allowSignInPrompt` true означает, что если пользователь в настоящее время не Office, Office откроет всплывающее входное подсказок.
    * `allowConsentPrompt` Настройка параметра true означает, что если пользователь не дал согласие на доступ к профилю AAD надстройки, Office откроет запрос на согласие. (Подсказка позволяет только пользователю соглашаться на AAD, а не на области Microsoft Graph.)
    * `forMSGraphAccess` Настройка параметра true signals Office, что надстройка намерена использовать маркер bootstrap для получения дополнительного маркера доступа с разрешениями microsoft Graph, а не просто использовать его в качестве маркера ID. Если администратор клиента не предоставил согласие на доступ надстройки к Microsoft Graph, `Office.auth.getAccessToken` возвращает ошибку **13012**. Надстройка может отреагировать переходом на альтернативную систему проверки подлинности. Это необходимо, так как Office может запрашивать согласие только на доступ к профилю пользователя Azure AD, а не к областям Microsoft Graph. Система авторизации отката требует, чтобы пользователь снова входил, и пользователю может быть предложено дать согласие на Graph microsoft. Таким образом, параметр `forMSGraphAccess` обеспечивает, что надстройка не будет выполнять замену маркера, которая завершится ошибкой из-за отсутствия согласия. (Так как вы предоставили согласие администратора на предыдущем шаге, этот сценарий не возникнет для этой надстройки. Но этот параметр добавлен в любом случае, чтобы продемонстрировать рекомендацию.)
...
1. Замените `TODO 5` следующим образом:
...
1. Под методом `getGraphData` добавьте следующую функцию. Обратите внимание`/auth`, что это серверный экспресс-маршрут, который обменивает маркер bootstrap с Azure AD на маркер доступа с разрешениями на microsoft Graph.
...
1. В редких случаях маркер загрузки, кэш которого Office кэшироваться, Office проверяет его, но истекает к тому времени, когда он достигнет Azure AD для обмена. Служба Azure AD ответит ошибкой **AADSTS500133**. В этом случае надстройка должна просто вызвать еще `getGraphData` раз. Так как срок действия кэшированного маркера начальной загрузки истек, Office получит новый маркер из Azure AD. Таким образом, замените `TODO 8` на следующее.
...
1. Замените `TODO 9` следующим образом:
...
1. Замените `TODO 13` приведенным ниже кодом. В этом коде обратите внимание на следующее:
    ...
    * Параметру формы `requested_token_use` присвоено значение 'on_behalf_of'. Это сообщает Azure AD, что надстройка запрашивает маркер доступа в Microsoft Graph с помощью потока On-Behalf-Of (OBO). Azure отвечает проверкой того, что маркер bootstrap, `assertion` который назначен для параметра формы, `scp` имеет свойство, заданное `access-as-user`.
...
1. Замените `TODO 14` приведенным ниже кодом, дополняющим блок `else`. В этом коде обратите внимание на следующее:

    * Константе `tenant` присвоено значение 'common', так как вы сделали надстройку мультитенатной при ее регистрации в Azure AD; в частности, когда назначили параметру **Поддерживаемые типы учетных записей** значение **Учетные записи в любом каталоге организации и персональные учетные записи Майкрософт (например, Skype, Xbox, Outlook.com)**. Если вместо этого вы решили поддерживать только учетные записи в том же Microsoft 365, где зарегистрирована надстройка, `tenant` то в этом коде будет задан guID клиента.
