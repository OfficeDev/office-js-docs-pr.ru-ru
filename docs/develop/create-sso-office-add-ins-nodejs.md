...
* Office 2016 версии 1708 (сборка 8424.nnnn) или более поздней (версия подписки на Office 365, иногда называемая "нажми и работай").
  ...
  Для получения этой версии может понадобиться быть участником программы предварительной оценки Office. Дополнительные сведения см. в статье [Примите участие в программе предварительной оценки Office](https://products.office.com/office-insider?tab=tab-1).
...
- При указании разрешений в диалоговом окне **Выбор разрешений** установите флажки для следующих разрешений. Для самой надстройки требуется только первое разрешение, но разрешение `profile` необходимо, чтобы ведущее приложение Office получило маркер для веб-приложения надстройки.
    * Files.Read.All
    * profile
...
2. В свойстве `client_id` замените заполнитель `{client GUID}` идентификатором приложения, сохраненным во время регистрации надстройки. Когда вы закончите, должен остаться только GUID в одинарных кавычках. Значение не должно содержать символов "{}".
...
3. В строке, назначенной свойству `issuer`, вы увидите заполнитель *{OID5  tenant GUID}*. Замените его идентификатором аренды Office 365. Для его получения используйте один из способов, описанных в статье [Поиск идентификатора клиента Office 365](https://docs.microsoft.com/onedrive/find-your-office-365-tenant-id). В результате значение свойства `issuer` должно выглядеть примерно так:
...
4. Замените `TODO7` приведенным ниже кодом, который проверяет маркер доступа, полученный от ведущего приложения Office. Метод `verifyJWT` определен в файле src\auth.ts. Он всегда проверяет аудиторию и издателя. С помощью необязательного параметра мы указываем на необходимость проверки того, указана ли в маркере доступа область `access_as_user`. Это единственное разрешение для надстройки, необходимое пользователю и ведущему приложению Office для того, чтобы получить маркер доступа к Microsoft Graph с помощью потока "от имени". 
    ...
    > [!NOTE]
    > Следует использовать только `access_as_user` область для авторизации API, которое обрабатывает поток "от имени" для надстроек Office. Другие API в службе должны иметь свои собственные требования области. Это ограничивает доступ, предоставляемый с использованием токенов, которые получает Office.
...
1. Создайте сетевую папку или [предоставьте общий доступ к папке через сеть](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc770880(v=ws.11)).
...
1. Если вкладка **Разработчик** не отображается на ленте, включите ее с помощью следующих действий:
 1. Перейдите к **файлу** | **Параметры** | **Настройка ленты**.
 2. Установите флажок для включения **разработчика** в дереве имен элементов управления в правой части страницы **Настройка ленты**.
 3. Нажмите кнопку **ОК**.

2. На вкладке в PowerPoint **Разработчик** выберите **Мои надстройки**.
