...
Основной шаблон заключается в создании асинхронного метода, который  возвращает объект Promise немедленно и устраняет  объект Promise, когда внутренний метод завершается, или отклоняет объект, если метод не удается. Ниже приводится простой пример
