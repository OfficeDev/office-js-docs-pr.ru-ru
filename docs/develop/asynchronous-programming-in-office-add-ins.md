...
Следующая функция анонимного `result` обратного вызова объявляет один параметр с именем, который извлекает данные из свойства [AsyncResult.value](/javascript/api/office/office.asyncresult#office-office-asyncresult-value-member) при возвращении обратного вызова.
...
В следующем примере показано, как передать эту функцию анонимного вызова в строку в контексте полного вызова метода "Async" к методу `Document.getSelectedDataAsync` .

- Первый аргумент _coercionType_ указывает `Office.CoercionType.Text`, чтобы вернуть выбранные данные в виде строки текста.

- Второй аргумент _вызова_ — анонимная функция, переданная методу в строке. При выполнении функции для отображения данных,  выбранных пользователем в документе, `value` `AsyncResult` используется параметр результатов для доступа к свойству объекта.
...
Дополнительные сведения об использовании метода см `getSelectedDataAsync` . в публикации [Read and write data to the active selection in a document or spreadsheet](read-and-write-data-to-the-active-selection-in-a-document-or-spreadsheet.md). 
...
Свойства `asyncContext`объекта `status`возвращают `error` `AsyncResult` те же виды информации в функцию обратного вызова, переданную всем методам async. Однако то, что возвращается `AsyncResult.value` в свойство, зависит от функциональности метода "Async".

Например, `addHandlerAsync` методы (объекты [Binding,](/javascript/api/office/office.binding) [CustomXmlPart](/javascript/api/office/office.customxmlpart), [Document](/javascript/api/office/office.document), [RoamingSettings](/javascript/api/outlook/office.roamingsettings) и [Параметры](/javascript/api/office/office.settings)) используются для добавления функций обработки событий в элементы, представленные этими объектами. Вы можете получить доступ к свойству из функции обратного вызова, передаемой любому из методов, но так как данные или объект не доступны при добавлении обработчицы событий,  `value` свойство всегда возвращается неопределенным, если вы попытаетесь получить к нему доступ.`AsyncResult.value` `addHandlerAsync`

С другой стороны, при `Document.getSelectedDataAsync` `AsyncResult.value` вызове метода он возвращает данные, выбранные пользователем в документе, в свойство обратного вызова. Или, если вы называете метод [Bindings.getAllAsync](/javascript/api/office/office.bindings#office-office-bindings-getallasync-member(1)) , `Binding` он возвращает массив всех объектов в документе. И если вы назовете метод [Bindings.getByIdAsync](/javascript/api/office/office.bindings#office-office-bindings-getbyidasync-member(1)) , он возвращает один `Binding` объект.

Описание того, что `AsyncResult.value` `Async` возвращается в свойство для метода, см. в разделе "Значение обратного вызова" справочной темы этого метода. Сводку всех `Async` объектов, которые предоставляют методы, см. в таблице в нижней части темы [объекта AsyncResult](/javascript/api/office/office.asyncresult) .
...
- Сначала вызывается метод [Bindings.getByIdAsync](/javascript/api/office/office.bindings#office-office-bindings-getbyidasync-member(1)) для получения доступа к привязке в документе с именем "MyBinding". Объект `AsyncResult` , возвращающийся к параметру `result` обратного вызова, предоставляет доступ к указанному объекту привязки из `AsyncResult.value` свойства.
- Затем объект привязки, доступный из первого `result` параметра, используется для вызова метода [Binding.getDataAsync](/javascript/api/office/office.binding#office-office-binding-getdataasync-member(1)) .
- Наконец, параметр `result2` вызова `Binding.getDataAsync` , переданного методу, используется для отображения данных в привязке.
...
В следующем примере две анонимные функции объявляются inline и передаются в вложенные `getByIdAsync` `getDataAsync` обратное вызовы и методы. Поскольку это простые и встроенные функции, их назначение сразу же становится понятным.
...
Параметр _selectorExpression_ `"bindings#bindingId"`принимает форму, в которой _bindingId_ — это имя (`id`) привязки, созданной ранее в документе или таблице (с помощью одного из методов addFrom `Bindings` коллекции: `addFromNamedItemAsync`, или `addFromPromptAsync``addFromSelectionAsync`). Например, выражение селектора `bindings#cities` указывает, что необходимо получить доступ к привязке с **id** "cities".

Параметр _onError_ `AsyncResult` `Error` — это функция обработки ошибок, которая принимает один параметр типа, который может использоваться для доступа к объекту, `select` если метод не может получить доступ к указанной привязке. В следующем примере показана базовая функция обработки ошибки, которую можно передать в параметр _onError_.
...
Замените _местообладатель BindingObjectAsyncMethod_ `Binding` вызовом на любой из четырех методов объекта, поддерживаемых объектом promise: `getDataAsync`, , `setDataAsync`, или `addHandlerAsync``removeHandlerAsync`. Вызовы этих методов не поддерживают дополнительные шаблоны promise. Их нужно вызывать с помощью [шаблона функции вложенного обратного вызова](#asynchronous-programming-using-nested-callback-functions).

После выполнения `Binding` обещания объекта его можно повторно использовать в цепном вызове метода, как при привязке (время выполнения надстройки не будет асинхронно выполнять обещание). Если обещание `Binding` объекта не может быть выполнено, время выполнения надстройки снова будет пытаться получить доступ к объекту привязки при следующем вызове одного из его асинхронных методов.

В следующем `select` `id` примере кода используется метод для получения привязки с "`cities`" `Bindings` из коллекции, а затем вызывает метод [addHandlerAsync](/javascript/api/office/office.binding#office-office-binding-addhandlerasync-member(1)) , чтобы добавить обработник событий для события [dataChanged](/javascript/api/office/office.bindingdatachangedeventargs) привязки.
...
> [!IMPORTANT]
> Обещание `Binding` объекта, возвращаемого методом `Office.select` , предоставляет доступ только к четырем методам `Binding` объекта. Если вам нужно получить доступ к `Binding` любому из других участников объекта, `Bindings.getAllAsync` `Document.bindings` `Bindings.getByIdAsync` вместо этого необходимо использовать свойство и методы для получения `Binding` объекта. Например, `Binding` если вам необходимо получить доступ к любым свойствам объекта (`document`свойствам `id``type` или свойствам) или получить доступ к свойствам объектов [MatrixBinding или TableBinding](/javascript/api/office/office.matrixbinding)[](/javascript/api/office/office.tablebinding), `getByIdAsync` `getAllAsync` `Binding` необходимо использовать или методы для получения объекта.
...
Вы можете создать объект JSON, содержащий дополнительные параметры в линию, `options` или путем создания объекта и передачи его в качестве _параметра параметра параметра параметра._
...
В этой форме синтаксиса вызовов два необязательных параметра, _coercionType_ и _asyncContext_, определяются как объект JSON, закрытый в скобки.

В следующем примере показано, как `Document.setSelectedDataAsync` вызвать метод, указав дополнительные параметры в линию.
...
Кроме того, можно `options` создать объект с именем, который указывает необязательные параметры отдельно от вызова метода, `options` а затем передать объект в качестве _аргумента параметра_ .

В следующем примере `options` показан один из способов создания объекта, `parameter1`в котором ( `value1`и т. д.) являются задатчиками фактических имен и значений параметров.
...
Это выглядит как следующий пример, когда используется для указания `ValueFormat` параметров `FilterType` и параметров:
...
> [!NOTE]
> При использовании любого метода создания `options` объекта можно указать необязательные параметры в любом порядке, если их имена указаны правильно.

В следующем примере показано, как `Document.setSelectedDataAsync` вызвать метод, указав необязательные параметры объекта `options` .
...
В обоих необязательных примерах параметров параметр _callback_ указывается как последний параметр (следуя за необязательными параметрами, или следуя объекту _аргумента параметра_ ). Кроме того, параметр _callback_ можно указать либо во встроенном объекте JSON, либо в объекте `options`. Однако параметр _callback_ можно передать только одним из способов: или в объекте _options_ (встроенном или созданном внешне), или в качестве последнего параметра.

## <a name="wrap-common-apis-in-promise-returning-functions"></a>Оберните общие API в функциях возврата обещаний

Общие методы API (Outlook API) не возвращают [обещания](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise). Поэтому нельзя использовать ожидание [для](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Operators/await) приостановки выполнения до завершения асинхронной операции. Если вам нужно `await` поведение, можно завернуть вызов метода в явно созданное обещание. 
...
Когда этот метод необходимо ожидать, `await` его можно назвать либо ключевым словом, либо функцией, передаемой функции `then` .

> [!NOTE]
> Этот метод особенно полезен при вызове одного из общих API `run` внутри вызова метода в одной из моделей объектов, определенных приложениям. Пример вышеуказанной функции см. вHome.js в примере [ Word-Add-in-JavaScript-MDConversion](https://github.com/OfficeDev/Word-Add-in-MarkdownConversion/blob/master/Word-Add-in-JavaScript-MDConversionWeb/Home.js).
