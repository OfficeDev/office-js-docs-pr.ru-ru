...
Тип пользователя не поддерживается. Пользователь не вписался в Office с помощью действительной учетной записи Майкрософт, Microsoft 365 для образования или работы. Например, это может произойти, если Office работает с учетной записью локального домена. Ваш код должен инициировать возврат к альтернативной системе проверки подлинности пользователя. В Outlook эта ошибка также [](/exchange/clients-and-mobile-in-exchange-online/enable-or-disable-modern-authentication-in-exchange-online) может возникать, если современная проверка подлинности отключена для клиента пользователя в Exchange Online. Дополнительные сведения см. в разделе [Требования и рекомендации](../develop/sso-in-office-add-ins.md#requirements-and-best-practices).

### <a name="13004"></a>13004

Недопустимый ресурс. (Эта ошибка должна быть видна только в разработке.) Манифест надстройки настроен неправильно. Обновите манифест. Дополнительные сведения см. в статье [Проверка манифеста надстройки Office](../testing/troubleshoot-manifest.md). Самая распространенная проблема заключается в том, что в элементе **Resource** (который входит в элемент **WebApplicationInfo**) указан домен, не соответствующий домену надстройки. Хотя часть протокола значения ресурса должна быть "api", а не "https", все остальные части доменного имени (включая порт, если таковой имеется) должны быть такими же, как и для надстройки.
...
- В рабочей среде эта ошибка может возникать по нескольким причинам. Вот некоторые из них:
    ...
    - Некоторые ситуации, которые могут привести к одной из других ошибок 13xxx с помощью учетной записи Microsoft 365 для образования или работы, могут привести к ошибке 13007 при работе с учетной записью MSA.
...
Пользователь запустил надстройку в Office в Microsoft EDGE или Internet Explorer. Домен Microsoft 365 пользователя и домен находятся в разных зонах безопасности в параметрах `login.microsoftonline.com` браузера. Эта ошибка возникает только в **Office в Интернете**. Если возвращается эта ошибка, то пользователь уже видел сообщение с соответствующим пояснением и ссылкой на инструкции по изменению конфигурации зон. Если ваша надстройка предоставляет функции, не требующие входа пользователя, то в коде следует отслеживать эту ошибку и позволять надстройке продолжать работу.

### <a name="13012"></a>13012

Существует несколько возможных причин. 

- Надстройка выполняется на платформе, которая не поддерживает API `getAccessToken`. Например, она не поддерживается на iPad. См. также статью [Наборы требований API Identity](../reference/requirement-sets/identity-api-requirement-sets.md).
...
Вызов вызывался слишком много раз за короткое время, поэтому Office отрежимал последний `getAccessToken` вызов. Обычно это вызвано бесконечным циклом вызовов метода. При отзыве метода возможны сценарии. Однако в коде следует использовать переменную счетчика или флага, чтобы гарантировать, что метод не будет повторно отозван. Если тот же путь к коду "retry" запущен повторно, код должен вернуться к альтернативной системе проверки подлинности пользователя. Пример кода см. в примере использования переменной в `retryGetAccessToken`HomeES6.jsили [ssoAuthES6.js. ](https://github.com/OfficeDev/Office-Add-in-NodeJS-SSO/blob/master/Complete/public/javascripts/ssoAuthES6.js) [](https://github.com/OfficeDev/Office-Add-in-ASPNET-SSO/blob/master/Complete/Office-Add-in-ASPNET-SSO-WebAPI/Scripts/HomeES6.js)
...
В определенных конфигурациях удостоверений в AAD и Microsoft 365 некоторые ресурсы, доступные с помощью Microsoft Graph, могут требовать многофакторной проверки подлинности (MFA), даже если в аренде Microsoft 365 пользователя нет. Когда служба AAD получает запрос на получение токена для доступа к защищенному с помощью MFA ресурсу, через поток выполнения от имени другого субъекта она возвращает веб-службе надстройки сообщение JSON, содержащее свойство `claims`. Свойство claims содержит сведения о том, какие еще факторы проверки подлинности требуются.
...
Независимо от архитектуры, если значение утверждений было отправлено из AAD, код должен отозвать и передать параметр `getAccessToken` `authChallenge: CLAIMS-STRING-HERE` в `options` параметре. Когда служба AAD обнаруживает эту строку, она предлагает пользователю указать дополнительные факторы, а затем возвращает новый маркер доступа, который будет принят в потоке выполнения от имени другого субъекта.
