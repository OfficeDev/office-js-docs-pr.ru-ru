...
Независимо от архитектуры, если значение утверждения было отправлено из AAD, ваш код должен отозваться `getAccessToken` и передать параметр `authChallenge: CLAIMS-STRING-HERE` в `options` параметре. Когда служба AAD обнаруживает эту строку, она предлагает пользователю указать дополнительные факторы, а затем возвращает новый маркер доступа, который будет принят в потоке выполнения от имени другого субъекта.
