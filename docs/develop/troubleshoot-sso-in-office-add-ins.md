...
Тип пользователя не поддерживается. Пользователь не выполнил вход в Office с помощью действительной учетной записи Майкрософт, Microsoft 365 для образования или рабочей учетной записи. Например, это может произойти, если Office работает с учетной записью локального домена. Ваш код должен инициировать возврат к альтернативной системе проверки подлинности пользователя. В Outlook эта ошибка также может возникать, [](/exchange/clients-and-mobile-in-exchange-online/enable-or-disable-modern-authentication-in-exchange-online) если современная проверка подлинности отключена для клиента пользователя в Exchange Online. Дополнительные сведения см. в разделе [Требования и рекомендации](../develop/sso-in-office-add-ins.md#requirements-and-best-practices).

### <a name="13004"></a>13004

Недопустимый ресурс. (Эта ошибка должна отображаться только в разработке.) Манифест надстройки настроен неправильно. Обновите манифест. Дополнительные сведения см. в статье [Проверка манифеста надстройки Office](../testing/troubleshoot-manifest.md). Наиболее распространенная проблема заключается в том **\<Resource\>** , что элемент ( **\<WebApplicationInfo\>** в элементе) имеет домен, который не соответствует домену надстройки. Хотя часть протокола значения ресурса должна быть "api", а не "https", все остальные части доменного имени (включая порт, если таковой имеется) должны быть такими же, как и для надстройки.

### <a name="13005"></a>13005

Недопустимое разрешение. Как правило, это означает, что приложение Office не получило предварительные разрешения для веб-службы надстройки. Дополнительные сведения см. в разделе [Создание приложения службы](sso-in-office-add-ins.md#register-your-add-in-with-the-microsoft-identity-platform) и статье [Регистрация надстройки в конечной точке Azure AD версии 2.0](register-sso-add-in-aad-v2.md). Это также может произойти, если пользователь не предоставил приложению службы разрешения на доступ к своему ресурсу `profile`, или отменил согласие. Ваш код должен инициировать возврат к альтернативной системе проверки подлинности пользователя.
...
- В рабочей среде эта ошибка может возникать по нескольким причинам. Вот некоторые из них:
  ...
  - В некоторых ситуациях, которые могут привести к одной из других ошибок 13xxx с Microsoft 365 для образования или рабочей учетной записью, при использовании MSA возникает ошибка 13007.
...
Пользователь выполняет надстройку в Office в Microsoft Edge. Домен пользователя Microsoft 365 `login.microsoftonline.com` и домен находятся в разных зонах безопасности в параметрах браузера. Эта ошибка возникает только в **Office в Интернете**. Если возвращается эта ошибка, то пользователь уже видел сообщение с соответствующим пояснением и ссылкой на инструкции по изменению конфигурации зон. Если ваша надстройка предоставляет функции, не требующие входа пользователя, то в коде следует отслеживать эту ошибку и позволять надстройке продолжать работу.

### <a name="13012"></a>13012

Существует несколько возможных причин.

- Надстройка выполняется на платформе, которая не поддерживает API `getAccessToken`. Например, она не поддерживается на iPad. См. также [наборы обязательных элементов API удостоверений](/javascript/api/requirement-sets/common/identity-api-requirement-sets).
...
Вызов `getAccessToken` был вызван слишком много раз за короткий промежуток времени, поэтому Office регулирует последний звонок. Обычно это вызвано бесконечным циклом вызовов метода. При отзыве метода рекомендуется использовать сценарии. Однако код должен использовать переменную счетчика или флага, чтобы гарантировать, что метод не будет отозван повторно. Если тот же путь к коду "повторить" выполняется снова, код должен вернуться к альтернативной системе проверки подлинности пользователя. Пример кода см. в описании `retryGetAccessToken` использования переменной [ вHomeES6.jsили ](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/auth/Office-Add-in-ASPNET-SSO/Complete/Office-Add-in-ASPNET-SSO-WebAPI/Scripts/HomeES6.js) [ssoAuthES6.js](https://github.com/OfficeDev/Office-Add-in-samples/tree/main/Samples/auth/Office-Add-in-NodeJS-SSO/Complete/public/javascripts/ssoAuthES6.js).
...
В определенных конфигурациях удостоверений в AAD и Microsoft 365 некоторые ресурсы, доступные в Microsoft Graph, могут требовать многофакторную проверку подлинности (MFA), даже если клиент Microsoft 365 пользователя не является клиентом Microsoft 365. Когда служба AAD получает запрос на получение токена для доступа к защищенному с помощью MFA ресурсу, через поток выполнения от имени другого субъекта она возвращает веб-службе надстройки сообщение JSON, содержащее свойство `claims`. Свойство claims содержит сведения о том, какие еще факторы проверки подлинности требуются.
...
Независимо от архитектуры, если значение утверждений было отправлено из AAD, `getAccessToken` `authChallenge: CLAIMS-STRING-HERE` код должен отозвать и передать параметр в параметре `options` . Когда служба AAD обнаруживает эту строку, она предлагает пользователю указать дополнительные факторы, а затем возвращает новый маркер доступа, который будет принят в потоке выполнения от имени другого субъекта.
...
### <a name="invalid-audience-error-in-the-access-token-for-microsoft-graph"></a>Ошибка недопустимой аудитории в маркере доступа для Microsoft Graph
