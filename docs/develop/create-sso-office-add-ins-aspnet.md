...
1. В настоящий момент версия библиотеки MSAL (Microsoft.Identity.Client), которая нужна для единого входа (версия `1.1.1-alpha0393`), не включена в стандартный каталог NuGet, поэтому не указана в package.config. Ее нужно установить отдельно. 

   > 1. В меню **Сервис** выберите **Диспетчер пакетов NuGet** > **Консоль диспетчера пакетов**. 

   > 2. В консоли выполните указанную ниже команду. Выполнение может занять минуту или больше времени, даже при быстром подключении к Интернету. Когда все будет готово, в нижней части окна консоли отобразится такое сообщение: **"Microsoft.Identity.Client 1.1.1-alpha0393" успешно установлено...**.
   ...
   > 3. В **обозревателе решений** щелкните правой кнопкой мыши узел **Ссылки**. Убедитесь, что в него включена библиотека **Microsoft.Identity.Client**. Если ее нет или она есть, но рядом с нею отображается значок предупреждения, удалите эту запись, а затем с помощью мастера добавления ссылок Visual Studio добавьте ссылку в сборку, указав **... \[Begin | Complete]\packages\Microsoft.Identity.Client.1.1.1-alpha0393\lib\net45\Microsoft.Identity.Client.dll**
...
1. Будет создан **URI идентификатора приложения** в формате "api://{App ID GUID}". Вставьте строку "localhost:44355" между двойной косой четой и GUID. Весь идентификатор должен выглядеть следующим образом: `api://localhost:44355/{App ID GUID}`. 

    > [!NOTE]
    > Доменная часть имени в поле **Область** под **URI идентификатора приложения** автоматически изменится соответствующим образом. Она должна выглядеть так: `api://localhost:44355/{App ID GUID}/access_as_user`.
...
1. В диалоговом окне выберите указанные ниже разрешения. Для самой надстройки требуется только первое разрешение, а `offline_access` и `openid` требуются для библиотеки MSAL, используемой кодом на стороне сервера. Разрешение `profile` необходимо, чтобы ведущее приложение Office получило токен для веб-приложения надстройки.
    * Files.Read.All
    * offline_access
    * openid
    * profile
    ...
    > [!NOTE]
    > Разрешение `User.Read` может быть уже указано по умолчанию. Незачем запрашивать ненужные разрешения, поэтому рекомендуем снять флажок рядом с этим разрешением.

1. Нажмите кнопку **ОК** в нижней части диалогового окна.

1. Нажмите кнопку **Сохранить** в нижней части страницы регистрации.

## <a name="grant-admin-consent-to-the-add-in"></a>Предоставление надстройке разрешений администратора

> [!NOTE]
> Эту процедуру необходимо выполнять только при разработке надстройки. После развертывания производственной надстройки в AppSource или каталоге надстроек будет два варианта развития событий. В первом случае пользователи будут подтверждать доверие в индивидуальном порядке, во втором — администратор во время установки предоставит разрешения для всей организации.
...
4. Введите указанные ниже значения для соответствующих ключей.
...
1. Замените заполнитель {application_GUID here} *в обоих местах* в разметке на идентификатор приложения, скопированный во время регистрации надстройки. Фигурные скобки не являются частью идентификатора, поэтому не нужно их включать. Это тот же идентификатор, который использовался для ключей ClientID и Audience в файле web.config.
...
1. Откройте файл Home.js в папке **Scripts**. В нем уже есть следующий код:
    ...
    * Метод `logErrors` для входа в консоли ошибок, которые не предназначены для пользователя.
...
1. Под назначением методу `Office.initialize` добавьте приведенный ниже код. Вот что нужно знать об этом коде:
    ...
    * Вы создадите метод `getDataWithToken` на следующем шаге, но обратите внимание, что он устанавливает для вызываемого параметра `forceConsent` значение `false`. Дополнительные сведения см. в описании следующего шага.
...
1. Под методом `getOneDriveFiles` добавьте приведенный ниже код. Вот что нужно знать об этом коде:
    ...
    * `getAccessTokenAsync` — это новый API в Office.js, позволяющий надстройке запрашивать у ведущего приложения Office (Excel, PowerPoint, Word и т. д.) маркер доступа к надстройке (для вошедшего в Office пользователя). Ведущее приложение Office, в свою очередь, запрашивает маркер у конечной точки Azure AD 2.0. Так как вы предварительно авторизовали ведущее приложение Office для надстройки во время ее регистрации, Azure AD отправит маркер.
    * Если вход в Office не выполнен, ведущее приложение Office предложит пользователю войти.
    * Параметр options задает для свойства `forceConsent` значение `false`, поэтому пользователю не нужно будет предоставлять ведущему приложению Office доступ к надстройке при каждом ее использовании. При первом запуске надстройки произойдет ошибка вызова `getAccessTokenAsync`, но логика обработки ошибок, которую вы добавите позже, автоматически выполнит повторный вызов с параметром `forceConsent`, имеющим значение `true`, и пользователю будет предложено предоставить разрешения. Такая процедура выполняется только в первый раз.
...
1. Под методом `getOneDriveFiles` добавьте приведенный ниже код. Вот что нужно знать об этом коде:
    ...
    * Этот служебный метод вызывает указанную конечную точку веб-API и передает ей тот же маркер доступа, который ведущее приложение Office использовало для доступа к надстройке. На стороне сервера этот маркер доступа будет использоваться в потоке "от имени" для получения маркера доступа к Microsoft Graph.
...
1. Под методом `getData` добавьте приведенный ниже метод. Этот метод будет обрабатывать ошибки в клиенте надстройки, когда основное приложение Office не сможет получить маркер доступа к веб-службе надстройки. Сообщения о таких ошибках содержат код ошибки, поэтому данный метод различает их с помощью оператора `switch`.
...
1. Замените `TODO2` приведенным ниже кодом. Ошибка 13001 возникает, когда пользователь не выполнил вход или отменил запрос второй проверки подлинности. В любом случае код повторно выполняет метод `getDataWithToken` и устанавливает принудительный запрос на вход.
...
1. Замените `TODO3` приведенным ниже кодом. Ошибка 13002 возникает, когда вход или предоставление разрешений прерывается. Попросите пользователя повторить попытку (не более одного раза).
...
1. Замените `TODO4` приведенным ниже кодом. Ошибка 13003 возникает, когда пользователь входит, используя неподдерживаемую учетную запись. Попросите пользователя выйти, а затем войти, используя рабочую или учебную учетную запись или учетную запись Майкрософт.
    ...
    > [!NOTE]
    > Ошибки 13004 и 13005 не обрабатываются при использовании этого метода, так как они возникают только на стадии разработки. Их невозможно исправить с помощью кода среды выполнения, поэтому нет смысла сообщать о них пользователю.

1. Замените `TODO5` приведенным ниже кодом. Ошибка 13006 возникает, когда в основном приложении Office возникает неопределенная ошибка, которая может свидетельствовать о его нестабильном состоянии. Попросите пользователя перезапустить Office.
...
1. Замените `TODO6` приведенным ниже кодом. Ошибка 13007 возникает, когда нарушается взаимодействие основного приложения Office с AAD, из-за чего основное приложение не может получить маркер доступа к веб-службе/приложению надстройки. Это может быть вызвано сбоем в работе сети. Попросите пользователя повторить попытку позже.
...
1. Замените `TODO7` приведенным ниже кодом. Ошибка 13008 возникает, когда пользователь запускает операцию, которая вызывает метод `getAccessTokenAsync`, до завершения предыдущего вызова.
...
1. Замените `TODO8` приведенным ниже кодом. Ошибка 13009 возникает, когда надстройка не поддерживает принудительное предоставление разрешений, но метод `getAccessTokenAsync` был вызван с параметром `forceConsent`, имеющим значение `true`. Обычно в таком случае код должен автоматически повторно запустить метод `getAccessTokenAsync` с параметром, имеющим значение `false`. Однако в некоторых случаях вызов метода с параметром `forceConsent`, имеющим значения `true`, сам по себе является автоматическим ответом на ошибку при вызове метода с параметром, имеющим значение `false`. В этом случае код должен не повторять попытку, а предложить пользователю выйти и войти заново.
...
1. Замените `TODO9` приведенным ниже кодом.
...
1. Замените `TODO12` приведенным ниже кодом. Вот что нужно знать об этом коде:
...
1. Замените `TODO13` приведенным ниже кодом. Вот что нужно знать об этом коде:

    * Ошибка 70011 имеет несколько значений. Главное для этой надстройки — запрос недопустимого разрешения, поэтому код проверяет наличие полного описания ошибки, а не только номера.
...
1. Замените `TODO14` приведенным ниже кодом. Вот что нужно знать об этом коде:
...
1. Замените `TODO15` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * В этом случае надстройка должна начать весь процесс проверки подлинности путем сброса переменных счетчика и флажка, а затем повторно вызвать метод обработчика кнопки.
...
1. Замените `TODO16` приведенным ниже кодом.
...
5. Замените `TODO1` приведенным ниже кодом, чтобы убедиться, что в токене указано разрешение `access_as_user`.
    ...
    > [!NOTE]
    > Для авторизации API, который отвечает за поток выполнения от имени другого субъекта, в случае надстроек Office используйте только область `access_as_user`. Для других API в службе должны быть предусмотрены отдельные требования, касающиеся областей. Это ограничивает объекты, к которым можно получить доступ с помощью маркеров, полученных набором приложений Office.

6. Замените `TODO2` приведенным ниже кодом. Вот что нужно знать об этом коде:
...
7. Замените `TODO3` приведенным ниже кодом. Вот что нужно знать об этом коде:
...
8. Замените `TODO3a` приведенным ниже кодом. Вот что нужно знать об этом коде:

    * Если ресурс Microsoft Graph требует многофакторной проверки подлинности, а пользователь еще не предоставил соответствующие данные, AAD вернет состояние "400 Bad Request" с ошибкой AADSTS50076 и свойство **Claims**. MSAL выдает исключение **MsalUiRequiredException** (которое наследуется от **MsalServiceException**), используя эту информацию. 
    * Значение свойства **Claims** необходимо передать клиенту, который передаст его ведущему приложению Office. Последнее добавит его в запрос на получение нового токена. AAD предложит пользователю пройти все необходимые проверки подлинности.
...
9. Замените `TODO3b` и `TODO3c` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    *  Объект **MsalUiRequiredException** передается методу `SendErrorToClient`. Это гарантирует, что свойство **ExceptionMessage**, содержащее информацию об ошибке, будет включено в HTTP-ответ.
...
10. Замените `TODO3d` приведенным ниже кодом. Обратите внимание, что код повторно выдает исключение, а не передает его в собственном HTTP-ответе с состоянием **HttpStatusCode.Forbidden** (401). В результате ASP.NET отправляет собственный HTTP-ответ с состоянием "500 Server Error".
...
11. Замените `TODO4` приведенным ниже кодом. Вот что нужно знать об этом коде:
...
12. Замените `TODO5` приведенным ниже кодом. Вот что нужно знать об этом коде: 
...
13. Добавьте приведенный ниже метод под методом Get. Вот что нужно знать об этом коде:  
