...
Сначала выполните действия в [Quickstart:](/azure/active-directory/develop/quickstart-register-app) зарегистрируйте приложение с помощью платформа удостоверений Майкрософт для регистрации надстройки.
...
1. Укажите атрибуты области в области **Добавить область** .
    ...
    |**Имя отображения согласия пользователя** | Office можете действовать так же, как и вы.|
...
1. В разделе **Авторизованные клиентские приложения** укажите приложения, которые необходимо авторизовать для веб-приложения надстройки. Необходимо обеспечить предварительную авторизацию для всех указанных ниже идентификаторов.
    ...
    > [!NOTE]
    > ID ea5a67f6-b6f3-4338-b240-c655ddc3cc8e включает все остальные перечисленные ИД и может использоваться сингулярно для предварительной авторизации всех конечных точек Office для использования с помощью службы в потоке SSO надстройки Office надстройки.
...
1. В **статье Выбор разрешений** выберите следующие разрешения.
    ...
    |profile        |Просмотр основного профиля пользователей. Необходимо, чтобы Office приложение для получения маркера в веб-приложении надстройки. |
...
1. Возвращаясь в **Обозреватель** решений, выберите (не щелкните правой кнопкой мыши) **проект Office-Add-in-ASPNET-SSO-WebAPI**. Откроется область **Свойства**. Убедитесь, что для параметра **SSL включен** задано значение **True**. Убедитесь, что **URL-адрес SSL** указан как `http://localhost:44355/`.
...
1. В **обозревателе** решений **выберите Office-надстройку-ASPNET-SSO** и откройте файл манифеста надстройки "Office-Add-in-ASPNET-SSO.xml" и прокрутите его в нижней части файла. Чуть выше конца тега `</VersionOverrides>` вы найдете следующую разметку.
...
1. Замените `TODO 1` следующим кодом, чтобы получить маркер доступа из Office хоста. Параметр **параметра** параметра содержит следующие параметры, переданные из предыдущей функции **getGraphData()** .
    ...
    * `forMSGraphAccess` настроено на верное. В этом Office возвращается ошибка (код 13012), если пользователь или администратор не предоставили Graph области для надстройки. Чтобы получить Graph microsoft, надстройка должна обменять маркер доступа на новый маркер доступа через поток от имени. Настройка `forMSGraphAccess` true помогает избежать сценария, при котором **getAccessToken ()** успешно, но затем поток от имени не удается позже для Microsoft Graph. Код на стороне клиента может реагировать на ошибку 13012, переходя на резервную систему авторизации.
...
1. Замените `TODO 3` приведенным ниже кодом. Во всех других случаях надстройка переходит на резервную систему авторизации. Дополнительные сведения об этих ошибках см. в Office [SSO](troubleshoot-sso-in-office-add-ins.md) устранения неполадок. В этой надстройке система отката открывает диалоговое окно, которое требует, чтобы пользователь входил, даже если пользователь уже есть.
...
1. Замените `TODO 1` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * Код предписывает OWIN убедиться, что аудитория, указанная в маркере bootstrap, которое исходит от Office приложения, должна соответствовать значению, указанному в web.config.
...
1. Замените `TODO 2` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * URL-адрес, который передается методу, заключается в том, что посредник OWIN получает инструкции по получению ключа, необходимого для проверки подписи на маркере bootstrap, полученном из Office приложения. Сегмент URL-адреса "Полномочия" предоставляется файлом web.config. Это либо строка "common", либо GUID для однотенантной надстройки.
...
1. Замените `TODO 3a` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * Значение **свойства Claims** должно передаваться клиенту, который должен передать его приложению Office, которое затем включает его в запрос на новый маркер bootstrap. Azure AD предложит пользователю пройти все необходимые проверки подлинности.
...
1. В области **Свойства** откройте раскрывающийся список **Начальный документ** и выберите один из трех вариантов (Excel, Word или PowerPoint).
    ...
    ![Выберите нужное Office клиентского приложения: Excel, PowerPoint или Word.](../images/SelectHost.JPG)
