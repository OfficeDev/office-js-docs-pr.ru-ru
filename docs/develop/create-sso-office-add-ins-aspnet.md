...
1. В настоящий момент версия библиотеки MSAL (Microsoft.Identity.Client), которая нужна для единого входа (версия `1.1.1-alpha0393`), не включена в стандартный каталог NuGet, поэтому не указана в package.config. Ее нужно установить отдельно. 
   ...
   > 2. На консоли выполните следующую команду. Для её выполнения может потребоваться минута или больше, несмотря на быстрое подключение к Интернету. Когда она завершится, вы должны увидеть **Successfully installed 'Microsoft.Identity.Client 1.1.4-preview0002' ...** ближе к концу вывода в консоли.

   >    `Install-Package Microsoft.Identity.Client -Version 1.1.4-preview0002`

   > 3. В **обозревателе решений** щелкните правой кнопкой мыши **Ссылки**. Убедитесь, что ссылка **Microsoft.Identity.Client** есть в списке. Если ее нет или рядом с ней отображается значок предупреждения, удалите эту запись, а затем с помощью мастера добавления ссылок Visual Studio добавьте ссылку в сборку, указав **… \[Begin | Complete]\packages\Microsoft.Identity.Client.1.1.4-alpha0002\lib\net45\Microsoft.Identity.Client.dll**
