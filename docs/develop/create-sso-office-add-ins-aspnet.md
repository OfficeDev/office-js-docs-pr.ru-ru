...
Следующие инструкции содержат общую информацию, поэтому их можно использовать в нескольких местах. Для этой статьи сделайте следующее:
...
- При указании разрешений в диалоговом окне **Выбрать Разрешения** установите флажки для следующих разрешений. Для самой надстройки требуется только первое разрешение, а `offline_access` и `openid` требуются для библиотеки MSAL, используемой кодом на стороне сервера. Разрешение `profile` необходимо, чтобы ведущее приложение Office получило токен для веб-приложения надстройки.
    * Files.Read.All
    * offline_access
    * openid
    * profile
...
## <a name="grant-administrator-consent-to-the-add-in"></a>Предоставление надстройке согласия администратора

[!INCLUDE[](../includes/grant-admin-consent-to-an-add-in-include.md)]

## <a name="configure-the-add-in"></a>Конфигурация надстройки

1. В следующей строке замените заполнитель "{tenant_ID}" на идентификатор клиента Office 365. Для его получения используйте один из методов, описанных в статье [Как найти свой идентификатор клиента Office 365](https://docs.microsoft.com/onedrive/find-your-office-365-tenant-id).
...
1. Замените заполнитель {application_GUID here} *в обоих местах* кода идентификатором приложения, скопированным во время регистрации надстройки. "{}" не являются частью идентификатора, поэтому не включайте их. Это тот же идентификатор, который использовался для ClientID и Audience в файле web.config.
...
1. Откройте вкладку **Предупреждения** в **списке ошибок** в Visual Studio. Предупреждение о том, что `<WebApplicationInfo>` не является действительным потомком `<VersionOverrides>`, означает, что используемая вами версия Visual Studio 2017 Preview не может распознать разметку единого входа. В качестве обходного решения в надстройке Word, Excel или PowerPoint можно выполнить указанные ниже действия. Если вы работаете с надстройкой Outlook, вы найдете решение ниже.
...
1. Замените `TODO11` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * Основное приложение Office должно получить новый маркер со значением **Claims** в качестве параметра `authChallenge`. Так AAD получит команду отобразить для пользователя запрос на прохождение всех форм проверки подлинности. 
...
1. Замените `TODO12` приведенным ниже кодом. Вы замените три `TODO`s в этом коде с помощью*внутреннего* условного блока в следующих шагах.
...
1. Замените `TODO12A` приведенным ниже кодом. (Это создает первую часть *внутреннего* условного блока). Обратите внимание на аспекты этого кода:
...
1. Замените `TODO12B` приведенным ниже кодом. Вот что нужно знать об этом коде:
...
1. Замените `TODO12C` приведенным ниже кодом. Вот что нужно знать об этом коде:
...
1. Замените `TODO13` приведенным ниже кодом. (Это является частью *внешнего* условного блока, и должно следовать сразу же после закрывающихся скобок структуры, которая начинается с `else if (exceptionMessage) {` и на одном уровне отступа.) Обратите внимание на аспекты этого кода:
...
1. Замените `TODO14` приведенным ниже кодом.
...
3. Над строкой с объявлением `ValuesController` добавьте атрибут `[Authorize]`. Это гарантирует, что надстройка будет выполнять процесс авторизации, настроенный в последней процедуре, при каждом вызове метода контроллера. Вызывать методы контроллера можно только при наличии действительного маркера доступа к надстройке.
    ...
    > [!NOTE]
    > Производственная служба веб-API на основе ASP.NET MVC должна иметь специальную логику для потока "от имени" в одном или нескольких пользовательских классах [FilterAttribute](https://docs.microsoft.com/previous-versions/aspnet/web-frameworks/hh834645(v=vs.108)). В этом примере логика помещается в главный контроллер, чтобы можно было легко проследить весь поток авторизации и логику получения данных. Такая же модель используется в примерах авторизации в разделе [Azure Samples](https://github.com/Azure-Samples/).    
...
5. Замените `TODO1` приведенным ниже кодом, чтобы убедиться, что в токене указано разрешение `access_as_user`.
    ...
    > [!NOTE]
    > Следует использовать только `access_as_user` область для авторизации API, которое обрабатывает поток "от имени" для надстроек Office. Другие API-интерфейсы в службе должны иметь свои собственные требования области. Это ограничивает доступ, предоставляемый с использованием токенов, которые получает Office.
