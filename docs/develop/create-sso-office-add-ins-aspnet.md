...
- При указании разрешений в диалоговом окне **Выбрать Разрешения** установите флажки для следующих разрешений. Для самой надстройки требуется только первое разрешение, но для библиотеки MSAL, в которой используется код на стороне сервера, требуются `offline_access` и `openid`. Разрешение `profile` необходимо, чтобы ведущее приложение Office получило токен для веб-приложения надстройки.
    * Files.Read.All
    * offline_access
    * openid
    * профиль
...
1. В следующей строке замените заполнитель "{tenant_ID}" на идентификатор клиента Office 365. Для его получения используйте один из методов, описанных в статье [Как найти свой идентификатор клиента Office 365](https://support.office.com/en-us/article/Find-your-Office-365-tenant-ID-6891b561-a52d-4ade-9f39-b492285e2c9b).

    `https://login.microsoftonline.com/{tenant_ID}/v2.0`

2. Откройте файл web.config в Visual Studio. В разделе **appSettings** есть ключи, которым необходимо назначить значения.
...
1. Откройте вкладку **Предупреждения** в **списке ошибок** в Visual Studio. Предупреждение о том, что `<WebApplicationInfo>` не является действительной дочерней темой `<VersionOverrides>`, означает, что используемая вами версия Visual Studio 2017 Preview не может распознать разметку единого входа. В качестве обходного решения в надстройке Word, Excel или PowerPoint можно выполнить указанные ниже действия. Если вы работаете с надстройкой Outlook, вы найдете решение ниже.
