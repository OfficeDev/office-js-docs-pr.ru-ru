...
1. На странице **Office-Add-in-ASPNET-SSO** скопируйте и сохраните значения для **идентификатора приложения (клиента)** и **идентификатора каталога (клиента)**. Они понадобятся вам позже.
    ...
    > [!NOTE]
    > Этот идентификатор является значением "аудитория", если другие приложения, такие как клиентское приложение Office (например, PowerPoint, Word, Excel), ищут авторизованный доступ к приложению. Кроме того, он используется как идентификатор клиента, когда приложение, в свою очередь, пытается получить авторизованный доступ к Microsoft Graph.
