...
1. В настоящий момент версия библиотеки MSAL (Microsoft.Identity.Client), которая нужна для единого входа (версия `1.1.4-preview0002`), не включена в стандартный каталог NuGet, поэтому не указана в package.config. Ее нужно установить отдельно. 
   ...
   > 3. В **обозревателе решений** щелкните правой кнопкой мыши узел **Ссылки**. Убедитесь, что в него включена библиотека **Microsoft.Identity.Client**. Если ее нет (если она есть, но рядом с ней отображается значок предупреждения, удалите эту запись), добавьте ссылку в сборку с помощью мастера добавления ссылок Visual Studio, указав **... \[Begin | Complete]\packages\Microsoft.Identity.Client.1.1.4-preview0002\lib\net45\Microsoft.Identity.Client.dll**
