...
* По крайней мере несколько файлов и папок, хранимые в OneDrive для бизнеса в подписке Microsoft 365.
...
1. Вопишитесь с учетными данными ***администратора*** в аренду Microsoft 365. Пример: MyName@contoso.onmicrosoft.com.
...
1. На странице **Office-Add-in-ASPNET-SSO** скопируйте и сохраните значения для ID приложения **(клиента)** и **ID Каталога (клиента).** Они понадобятся вам позже.

    > [!NOTE]
    > Этот **ID приложения (клиента)** является значением "аудитория", когда другие приложения, например клиентские приложения Office (например, PowerPoint, Word, Excel), ищут авторизованный доступ к приложению. Кроме того, он используется как идентификатор клиента, когда приложение, в свою очередь, пытается получить авторизованный доступ к Microsoft Graph.
...
1. Заполните поля для настройки запросов на согласие администратора и пользователя со значениями, подходящими для области, которая позволяет клиентского приложения Office использовать веб-API надстройки с тем же правами, что и текущий `access_as_user` пользователь. Предложения:
    ...
    * **Имя отображения согласия пользователя.** Office может действовать так же, как и вы.
    * **Описание согласия пользователя.** Уполномойте Office вызывать веб-API надстройки с тем же правами, что и у вас.
...
1. Используйте поле поиска **Выбрать разрешения**, чтобы найти нужные разрешения для надстройки. Выберите следующие параметры. Только первая действительно требуется самой надстройки; но для приложения Office требуется разрешение на получение маркера в `profile` веб-приложении надстройки. (Для надстройки требуются только разрешения Files.Read.All и profile. Остальные два необходимо запросить для библиотеки MSAL.NET.)
...
1. В файле web.config используйте значения, скопированные ранее. Для **ida:ClientID** и **ida:Audience** укажите **идентификатор приложения (клиента)**, для **ida:Password** — секрет клиента. Кроме того, установите **ida:Domain** (в конце нет косой черты `http://localhost:44355` "/"). 

    > [!NOTE]
    > ID **приложения (клиента)** — это значение "аудитория", когда другие приложения, например клиентские приложения Office (например, PowerPoint, Word, Excel), ищут авторизованный доступ к приложению. Кроме того, он используется как идентификатор клиента, когда приложение, в свою очередь, пытается получить авторизованный доступ к Microsoft Graph.
...
1. В обозревателе решений выберите проект **Office-Add-in-ASPNET-SSO** и откройте файл манифеста надстройки "Office-Add-in-ASPNET-SSO.xml" и прокрутите его в нижней части файла. Над закрывающим тегом `</VersionOverrides>` вы найдете следующую разметку:
...
1. Замените `TODO 1` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * `allowConsentPrompt` сообщает Office, чтобы он дал согласие на доступ к надстройки в профиле AAD пользователя, если согласие еще не получено. (Результатовая подсказка *не* позволяет пользователю соглашаться на какие-либо области Microsoft Graph.)
...
1. Замените `TODO 3` приведенным ниже кодом. Во всех других случаях надстройка переходит на резервную систему авторизации. Дополнительные сведения об этих ошибках см. в дополнительных сведениях об устранении неполадок [SSO в надстройки Office.](troubleshoot-sso-in-office-add-ins.md) В этой надстройке система отката открывает диалоговое окно, которое требует, чтобы пользователь входил, даже если пользователь уже есть.
...
1. Замените `TODO 1` приведенным ниже кодом. Что нужно знать об этом коде:
    ...
    * Код предписывает OWIN убедиться, что аудитория, указанная в маркере bootstrap, которое исходит от приложения Office, должна соответствовать значению, указанному в web.config.
    ...
    * Настройка `SaveSigninToken` причины `true` OWIN для сохранения необработанных маркеров загрузок из приложения Office. Он необходим надстройке, чтобы получить маркер доступа к Microsoft Graph в потоке "от имени".
...
1. Замените `TODO 2` приведенным ниже кодом. Что нужно знать об этом коде:
    ...
    * URL-адрес, который передается методу, заключается в том, что посредник OWIN получает инструкции по получению ключа, необходимого для проверки подписи на маркере bootstrap, полученном из приложения Office. Сегмент URL-адреса "Полномочия" предоставляется файлом web.config. Это либо строка "common", либо GUID для однотенантной надстройки.
...
1. Замените `TODO 2` приведенным ниже кодом, чтобы собрать все сведения, необходимые для получения маркера для Microsoft Graph, используя поток "от имени". Вот что нужно знать об этом коде:
    ...
    * Ваша надстройка больше не играет роли ресурса (или аудитории), к которому приложению Office и пользователю необходим доступ. Теперь она сама является клиентом, которому необходим доступ к Microsoft Graph. `ConfidentialClientApplication` — это объект "контекста клиента" MSAL.
    ...
    * Для работы библиотеки MSAL требуются области `openid` и `offline_access`, но если код их избыточно запрашивает, возникает ошибка. Кроме того, будет допущена ошибка, если ваш код запрашивает код, который используется только тогда, когда клиентская заявка Office получает маркер в `profile` веб-приложение надстройки. Поэтому явным образом запрашивается только `Files.Read.All`.
...
1. Замените `TODO 3` приведенным ниже кодом. Что нужно знать об этом коде:
...
1. Нажмите кнопку **Получить имена файлов OneDrive**. Если вы вошли в Office с учетной записью Microsoft 365 Education или work, или учетной записью Майкрософт, и SSO работает как ожидалось, первые 10 имен файлов и папок в oneDrive для бизнеса отображаются на области задач. Если вы не выполнили вход или используете сценарий, не поддерживающий единый вход, или единый вход не работает по какой-то причине, появится запрос на вход. После входа в систему отобразятся имена файлов и папок.

## <a name="updating-the-add-in-when-you-go-to-staging-and-production"></a>Обновление надстройки при становлении и производстве

Как и все веб-надстройки Office, когда вы готовы перейти на постаново-производственный сервер, необходимо обновить домен в манифесте с помощью `localhost:44355` нового домена. Кроме того, необходимо обновить домен в web.config файле.

Так как домен отображается в регистрации AAD, необходимо обновить эту регистрацию, чтобы использовать новый домен, где бы он `localhost:44355` ни появился.
