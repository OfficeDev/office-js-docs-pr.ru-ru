...
В этой статье вы проходите процесс включения единого входного знака (SSO) в надстройку, построенную с ASP.NET.

## <a name="prerequisites"></a>Предварительные требования

* Visual Studio 2019 или более поздней версии.

* Рабочая **нагрузка Office/SharePoint** при настройке Visual studio.
...
Сначала выполните действия в [Quickstart:](/azure/active-directory/develop/quickstart-register-app) зарегистрируйте приложение платформа удостоверений Майкрософт для регистрации надстройки.

Используйте следующие параметры для регистрации приложения.

* Имя: `Office-Add-in-ASPNET-SSO`
* Поддерживаемые типы учетных записей: учетные записи в любом организационном каталоге (любой каталог Azure AD — многоступенчатый) и личные учетные записи **Майкрософт (например, Skype, Xbox)**

    > [!NOTE]
    >  Если вы хотите, чтобы надстройка была возможной только пользователями в аренде, где вы ее регистрируете, вы можете выбрать учетные записи в этом организационном каталоге только **...** Подробнее **о настройке для одного клиента** см. в этой статье.

* Платформа: **Web**
* Перенаправление URI: **https://localhost:44355/AzureADAuth/Authorize**
* Секрет клиента: `*********` (зафиксировать это значение после создания — оно отображается только один раз)

### <a name="expose-a-web-api"></a>Обналичить веб-API

1. В созданной регистрации приложения выберите **Expose aPI > добавить область**.
   Если вы еще не настроены, вам будет предложено установить **URI ID** приложения.

    URI ID приложения выступает в качестве префикса для областей, которые будут ссылаться в коде API, и он должен быть уникальным во всем мире. Используйте форму `api://localhost:44355/[application-id-guid]`; например `api://localhost:44355/c6c1f32b-5e55-4997-881a-753cc1d563b7`.

1. Укажите атрибуты области в области **Добавить область** .
    ...
    |**Описание согласия администратора** | Включить Office для вызова веб-API надстройки с тем же правами, что и текущий пользователь.|
    |**Имя отображения согласия пользователя** | Office может действовать как вы.|
    |**Описание согласия пользователя** | Включить Office для вызова веб-API надстройки с тем же правами, что и у вас.|

1. Установите состояние **включено**, а затем выберите **область Добавить**.
...
1. В разделе **Авторизованные клиентские приложения** укажите приложения, которые необходимо авторизовать для веб-приложения надстройки. Необходимо обеспечить предварительную авторизацию для всех указанных ниже идентификаторов.
    ...
    |Идентификатор клиента                              |Для приложений  |
    |---------------------------------------|-----------------|
    ...
    > [!NOTE]
    > ID ea5a67f6-b6f3-4338-b240-c655ddc3cc8e включает все остальные перечисленные ИД и может использоваться сингулярно для предварительной авторизации всех конечных точек Office хостов для использования с вашей службой в потоке SSO надстройки Office.

    Для каждого ИД клиента необходимо предпринять следующие действия.

    а. Выберите **Добавить клиентское приложение**. На открываемой панели установите ИД клиента в соответствующий GUID и проверьте поле `api://localhost:44355/[application-id-guid]/access_as_user`для .

    б. Нажмите кнопку **Добавить приложение**.

### <a name="configure-microsoft-graph-permissions"></a>Настройка разрешений Graph Майкрософт
...
1. В **статье Выбор разрешений** выберите следующие разрешения.

    |Разрешение     |Описание  |
    |---------------|-------------|
    |Files.Read.All |Ознакомьтесь со всеми файлами, к которые пользователь может получить доступ. |
    |profile        |Просмотр основного профиля пользователей. Необходимо, чтобы Office для получения маркера в веб-приложении надстройки. |
...
1. Выберите **Добавить разрешения для** завершения процесса.

При настройке разрешений пользователям приложения при входе в приложение будет предложено разрешить приложению доступ к API ресурса от их имени. В качестве администратора вы также можете предоставить согласие от имени всех пользователей, чтобы им не было предложено сделать это.
...
1. На этой же странице нажмите кнопку **Предоставить согласие администратора для [имя клиента]** и выберите **Принять** в появившемся запросе подтверждения.
    ...
    > [!NOTE]
    > После нажатия кнопки **Предоставить согласие администратора для [имя клиента]** может появиться сообщение баннера с просьбой повторить попытку через несколько минут, чтобы можно было создать запрос на продолжение. Если это так, можно приступить к работе в следующем разделе, но не забудьте вернуться на портал и **_нажать эту кнопку_**!
...
1. Возвращаясь в **Обозреватель** решений, выберите (не нажимайте правой кнопкой мыши) **проект Office-Add-in-ASPNET-SSO-WebAPI**. Откроется область **Свойства**. Убедитесь, что для параметра **SSL включен** задано значение **True**. Убедитесь, что **URL-адрес SSL** указан как `http://localhost:44355/`.

1. В файле web.config используйте значения, скопированные ранее. Для **ida:ClientID** и **ida:Audience** укажите **идентификатор приложения (клиента)**, для **ida:Password** — секрет клиента. Кроме того, установите **ida:Domain** `http://localhost:44355` (в конце нет косой черты "/").

    > [!NOTE]
    > **ID приложения (клиента)** — это значение "аудитория", когда другие приложения, например клиентская Office (например, PowerPoint, Word, Excel), ищут авторизованный доступ к приложению. Кроме того, он используется как идентификатор клиента, когда приложение, в свою очередь, пытается получить авторизованный доступ к Microsoft Graph.
...
1. По-прежнему в **обозревателе решений выберите Office-надстройку-в-ASPNET-SSO** и откройте файл манифеста надстройки "Office-Add-in-ASPNET-SSO.xml" и прокрутите его в нижней части файла. Чуть выше конца тега `</VersionOverrides>` вы найдете следующую разметку.
...
Если при регистрации надстройки вы выбрали только "Учетные  записи в этом организационном каталоге" для ПОДДЕРЖИВАЕМЫХ ТИПОВ учетных записей, необходимо предпринять дополнительные действия по настройке.
...
1. Откройте файл HomeES6.js в папке **Scripts**. В нем уже есть код.
    ...
    * Код, который реализует систему авторизации отката, которую надстройка будет использовать в сценариях, где SSO не поддерживается или произошла ошибка.
...
1. Добавьте указанную ниже функцию под функцией `getGraphData`. Обратите внимание, что функция `handleClientSideErrors` будет создана позже.
    ...
    > [!NOTE]
    > Чтобы отличить два маркера доступа, с помощью которых вы работаете в этой статье, маркер, возвращенный из getAccessToken() называется маркером bootstrap. Затем он обменивается через поток On-Behalf-Of для нового маркера с доступом к Microsoft Graph.
...
1. Замените `TODO 1` следующим кодом, чтобы получить маркер доступа из Office хоста. Параметр **параметра** параметра содержит следующие параметры, переданные из предыдущей функции **getGraphData()** .
    ...
    * `forMSGraphAccess` настроено на верное. В этом Office возвращается ошибка (код 13012), если пользователь или администратор не предоставили Graph области для надстройки. Чтобы получить доступ Graph microsoft, надстройка должна обменять маркер доступа на новый маркер доступа через поток от имени. Настройка `forMSGraphAccess` true помогает избежать сценария, при котором **getAccessToken ()** успешно, но затем поток от имени не удается позже для Microsoft Graph. Код на стороне клиента может реагировать на ошибку 13012, переходя на резервную систему авторизации.

    Кроме того, обратите внимание на следующий код:

    * Вы создадите функцию `getData` позже.
    * Параметр `/api/values` — URL-адрес контроллера на стороне сервера, который будет использовать поток от имени для обмена маркера на новый маркер доступа для вызова Microsoft Graph.
...
1. Замените `TODO 3` приведенным ниже кодом. Во всех других случаях надстройка переходит на резервную систему авторизации. Дополнительные сведения об этих ошибках см. в Office [SSO](troubleshoot-sso-in-office-add-ins.md). В этой надстройке система отката открывает диалоговое окно, которое требует, чтобы пользователь входил, даже если пользователь уже есть.
...
1. Замените `TODO 1` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * Код предписывает OWIN убедиться, что аудитория, указанная в маркере bootstrap, который поступает из приложения Office, должна соответствовать значению, указанному в web.config.
    * Учетные записи Майкрософт имеют GUID эмитента, который отличается от любого guID клиента организации, поэтому для поддержки обоих типов учетных записей мы не проверяем эмитента.
    * Настройка `SaveSigninToken` причины `true` OWIN для сохранения необработанных маркеров загрузки из Office приложения. Он необходим надстройке, чтобы получить маркер доступа к Microsoft Graph в потоке "от имени".
...
1. Замените `TODO 2` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * URL-адрес, который передается методу, заключается в том, что среднее повечение OWIN получает инструкции по получению ключа, необходимого для проверки подписи на маркере bootstrap, полученном от Office приложения. Сегмент URL-адреса "Полномочия" предоставляется файлом web.config. Это либо строка "common", либо GUID для однотенантной надстройки.
...
1. Замените `TODO 2` приведенным ниже кодом, чтобы собрать все сведения, необходимые для получения маркера для Microsoft Graph, используя поток "от имени". Вот что нужно знать об этом коде:
    ...
    * Ваша надстройка больше не играет роли ресурса (или аудитории), к которому Office и пользователю необходим доступ. Теперь она сама является клиентом, которому необходим доступ к Microsoft Graph. `ConfidentialClientApplication` — это объект "контекста клиента" MSAL.
    ...
    * MSAL будет `profile`бросать ошибку, если ваш код запрашивает , который на самом деле используется только Office клиентского приложения получает маркер в веб-приложении надстройки. Поэтому явным образом запрашивается только `Files.Read.All`.
...
1. Замените `TODO 3a` приведенным ниже кодом. Вот что нужно знать об этом коде:
    ...
    * Значение **свойства Claims** должно передаваться клиенту, который должен передать его в приложение Office, которое затем включает его в запрос на новый маркер bootstrap. Azure AD предложит пользователю пройти все необходимые проверки подлинности.
...
1. Затем добавьте следующую строку в верхней части `try` блока в том же методе, чуть выше вызова `getAccessToken`.

    ```javascript
    throw new MockSSOError("13003");
    ```

## <a name="updating-the-add-in-when-you-go-to-staging-and-production"></a>Обновление надстройки при становлении и производстве

Как и Office веб-надстройки, когда вы готовы перейти на сервер постановки или производства, `localhost:44355` необходимо обновить домен в манифесте с помощью нового домена. Кроме того, необходимо обновить домен в web.config файле.

Так как домен отображается в AAD регистрации, `localhost:44355` необходимо обновить эту регистрацию, чтобы использовать новый домен на месте, где бы он ни появился.
