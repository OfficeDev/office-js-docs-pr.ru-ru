...
* Последняя доступная версия Visual Studio 2019.
...
1. Откройте вкладку **Предупреждения** в **списке ошибок** в Visual Studio. Если на ней есть предупреждение о том, что `<WebApplicationInfo>` не является допустимым дочерним элементом узла `<VersionOverrides>`, это означает, что используемой вами версии Visual Studio не удается распознать разметку единого входа. В качестве обходного решения в надстройке Word, Excel или PowerPoint можно выполнить указанные ниже действия. Если вы работаете с надстройкой Outlook, вы найдете решение ниже.
