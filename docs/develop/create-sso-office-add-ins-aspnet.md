...
1. На странице **Office-Надстройка-ASPNET-SSO** скопируйте и сохраните значение для ID приложения **(клиента).** Это потребуется в более поздних процедурах.
...
1. В разделе **Управление** выберите **Сертификаты и секреты**. Нажмите кнопку **Новый секрет клиента**. Введите значение параметра **Описание**, выберите соответствующий вариант для параметра **Истекает срок действия** и нажмите кнопку **Добавить**. *Скопируйте секретное* значение клиента (а не секретный ID) немедленно и сохраните его с помощью ID приложения перед тем, как оно потребуется в более поздней процедуре.
...
1. Возвращаясь в **обозреватель** решений, выберите (не щелкните правой кнопкой мыши) **Office-add-in-ASPNET-SSO-WebAPI.** Откроется область **Свойства**. Убедитесь, что для параметра **SSL включен** задано значение **True**. Убедитесь, что **URL-адрес SSL** указан как `http://localhost:44355/`.
...
1. По-прежнему в **обозревателе решений выберите Office-надстройку-в-ASPNET-SSO** и откройте файл манифеста надстройки "Office-Add-in-ASPNET-SSO.xml", а затем прокрутите в нижней части файла.  Чуть выше конца `</VersionOverrides>` тега вы найдете следующую разметку.
...
1. Замените `TODO 2` приведенным ниже кодом, чтобы собрать все сведения, необходимые для получения маркера для Microsoft Graph, используя поток "от имени". Вот что нужно знать об этом коде:
    ...
    * MSAL будет бросать ошибку, если ваш код запрашивает , который на самом деле используется только Office клиентского приложения получает маркер в `profile` веб-приложении надстройки. Поэтому явным образом запрашивается только `Files.Read.All`.
...
1. Добавьте следующий код в верхнюю часть метода в `getDataWithToken` HomeES6.js файле.
...
1. Затем добавьте следующую строку в верхней части блока в том же `try` методе, чуть выше вызова `getAccessToken` .
...
Как и Office веб-надстройки, когда вы готовы перейти на сервер постановки или производства, необходимо обновить домен в манифесте с помощью `localhost:44355` нового домена. Кроме того, необходимо обновить домен в web.config файле.

Так как домен отображается в AAD регистрации, необходимо обновить эту регистрацию, чтобы использовать новый домен на месте, где `localhost:44355` бы он ни появился.
