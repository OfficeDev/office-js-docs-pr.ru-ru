...
Объект поддерживает четыре способа доступа разработчиков к `Document` содержимому документов:
...
Существует три различных типа привязки, которые вы указываете с параметром  _bindingType_ при создании привязки с `addFromSelectionAsync` помощью или `addFromPromptAsync` `addFromNamedItemAsync` методов:
