...
Возможность открытия диалогового окна с помощью области задач, контентной надстройки или [команды надстройки](https://dev.office.com/docs/add-ins/design/add-in-commands) может позволить следующее:
...
>**Примечание.** Из главного окна можно открыть только одно диалоговое окно. При попытке открыть еще одно диалоговое окно произойдет ошибка. Если пользователь, например, откроет диалоговое окно из области задач, он не сможет открыть второе диалоговое окно на другой странице в области задач. Но при открытии диалогового окна с помощью [команды надстройки](https://dev.office.com/docs/add-ins/design/add-in-commands) каждый раз открывается новый (невидимый) HTML-файл. При этом создается новое (невидимое) главное окно, которое может запускать собственное диалоговое окно. Дополнительные сведения см. в статье [Ошибки displayDialogAsync](#errors-from-displaydialogAsync).
...
`displayInIframe` — дополнительное свойство в объекте конфигурации, которое можно передать `displayDialogAsync`. Когда этому свойству присвоено значение `true`, а надстройка запущена для документа в Office Online, диалоговое окно будет открываться быстрее, потому что будет выступать в качестве перемещаемого элемента iframe. Пример.

```js
Office.context.ui.displayDialogAsync('https://myDomain/myDialog.html', {height: 30, width: 20, displayInIframe: true});
```

Значение по умолчанию: `false`. Использование этого значения равнозначно пропуску всего свойства. Если надстройка не работает в Office Online, `displayInIframe` игнорируется.

> **Примечание.** ***Не*** следует использовать `displayInIframe: true`, если диалоговое окно будет выполнять перенаправление на страницу, которую невозможно открыть в элементе iframe. Например, страницы входа многих популярных веб-служб, например с помощью учетной записи Майкрософт или Google, невозможно открыть в элементе iframe.

### <a name="send-information-from-the-dialog-box-to-the-host-page"></a>Отправка данных из диалогового окна главной странице
...
Код в диалоговом окне использует функцию `messageParent` для отправки логического значения или строки на главную страницу. Строка может быть словом, предложением, большим двоичным объектом XML, преобразованными данными JSON или любыми другими объектами, которые можно сериализовать в строку. Пример.
...
> - Функция `messageParent` принадлежит одному из *двух* API Office, которые можно вызывать в диалоговом окне. Другой — `Office.context.requirements.isSetSupported`. Дополнительные сведения см. в статье [Указание ведущих приложений Office и обязательных элементов API](https://github.com/OfficeDev/office-js-docs/blob/master/docs/overview/specify-office-hosts-and-api-requirements.md).
...
В следующем примере `googleProfile` — это преобразованная версия профиля пользователя Google.
...
Чтобы главная страница получила сообщение, ее необходимо настроить. Для этого добавьте параметр обратного вызова в исходные данные вызова метода `displayDialogAsync`. При обратном вызове назначается обработчик для события `DialogMessageReceived`. Пример.
...
> - Office передает объект [AsyncResult](https://dev.office.com/reference/add-ins/shared/asyncresult) в функцию обратного вызова. Он представляет результат попытки открыть диалоговое окно. Он не представляет результат событий в диалоговом окне. Дополнительные сведения об этой особенности см. в разделе [Обработка ошибок и событий](#handling-errors-and-events).
> - Свойство `value` объекта `asyncResult` приравнено к объекту [Dialog](https://dev.office.com/reference/add-ins/shared/officeui.dialog), который существует на главной странице, а не в контексте выполнения диалогового окна.
...
Пример надстройки, в которой используются эти методы, см. [здесь](https://github.com/OfficeDev/Office-Add-in-Dialog-API-Simple-Example).

Если надстройка должна открыть другую страницу области задач после получения сообщения, можно использовать метод `window.location.replace` (или `window.location.href`) в последней строке обработчика. Пример.
...
Так как из диалогового окна можно выполнить несколько вызовов `messageParent`, но на главной странице есть только один обработчик для события `DialogMessageReceived`, обработчику необходимо использовать условную логику для различения сообщений. Например, если диалоговое окно предлагает пользователю войти в учетную запись Майкрософт, Google или другого поставщика удостоверений, оно отправляет профиль пользователя в виде сообщения. Если выполнить проверку подлинности не удается, диалоговое окно отправляет сведения об ошибке главной странице, как показано в приведенном ниже примере.
...
Код обработчика на главной странице использует значение свойства `messageType` для перехода, как показано в приведенном ниже примере. Обратите внимание, что здесь используется та же функция `showUserName`, что и в примере выше, а функция `showNotification` отображает сообщение об ошибке на главной странице.
...
Вы можете добавить в диалоговое окно кнопку, которая будет его закрывать. Для этого обработчик событий кнопки должен использовать `messageParent`, чтобы сообщить главной странице, что кнопка нажата. Пример.
...
Обработчик главной страницы для `DialogMessageReceived` вызовет `dialog.close`, как показано в этом примере. (Примеры инициализации объекта dialog см. в примерах выше.)
...
При вызове `displayDialogAsync` он всегда передает объект [AsyncResult](https://dev.office.com/reference/add-ins/shared/asyncresult) в функцию обратного вызова. Если вызов выполнен, т. е. диалоговое окно открыто, свойство `value` объекта `AsyncResult` представляет собой объект [Dialog](https://dev.office.com/reference/add-ins/shared/officeui.dialog). См. пример в разделе [Отправка данных из диалогового окна главной странице](#sending-information-from-the-dialog-to-the-host-page). Если вызвать `displayDialogAsync` не удается, то окно не создается, свойству `status` объекта `AsyncResult` присваивается значение "не выполнено", а также заполняется свойство `error` объекта. У вас всегда должна быть функция обратного вызова, которая проверяет `status` и сообщает об ошибке. Ниже приведен пример кода, сообщающий об ошибке, независимо от ее кода.
...
Код может назначить обработчика для события `DialogEventReceived` при вызове `displayDialogAsync`. Ниже приведен простой пример.
...
1.  Создайте страницу с окном iframe. Атрибут `src` элемента iframe указывает на видео из Интернета. URL-адрес видео должен быть защищен с помощью протокола HTTP**S**. В этой статье мы назовем эту страницу "video.dialogbox.html". Ниже приведен пример кода.
...
>**Примечание.** Когда вы используете Dialog API для этого сценария, *не* применяйте параметр `displayInIframe: true` при вызове `displayDialogAsync`. Подробное описание этого параметра см. в разделе [Использование параметра производительности в Office Online](#take-advantage-of-a-performance-option-in-Office-Online) этой статьи.

Ниже описан простой и типичный поток аутентификации.

1. Сначала в диалоговом окне открывается локальная страница или другой ресурс, размещенный в домене надстройки (то есть в домене главного окна). На этой странице может быть простая надпись "Подождите, мы перенаправляем вас на страницу, где вы сможете войти в учетную запись *ИМЯ_ПОСТАВЩИКА*". Код на этой странице создает URL-адрес страницы входа поставщика удостоверений, используя данные, передаваемые диалоговому окну, как описано в разделе [Передача данных диалоговому окну](#pass-information-to-the-dialog-box).
...
Когда пользователь вызывает функцию в приложении, которая получает доступ к его данным в службе ресурса, ему будет сначала предложено войти в службу, а затем — предоставить приложению необходимые разрешения. Служба затем перенаправляет пользователя на зарегистрированный URL-адрес и передает маркер доступа. Приложение использует маркер доступа для доступа к ресурсам пользователя.

Вы можете управлять этим процессом с помощью Dialog API, используя поток, похожий на тот, который обеспечивает возможность входа пользователей, или вариант, описанный в разделе [Решение для медленного соединения](#addressing-a-slow-network). Единственные отличия:
...
- [Авторизация внешних служб в надстройке Office](https://dev.office.com/docs/add-ins/develop/auth-external-add-ins)
