...
> [!NOTE]
> Когда вы используете Dialog API для этого сценария, *не* применяйте параметр `displayInIframe: true` при вызове `displayDialogAsync`. Сведения об этом параметре приведены выше, в разделе [Использование параметра производительности в Office в Интернете](#take-advantage-of-a-performance-option-in-office-on-the-web).
