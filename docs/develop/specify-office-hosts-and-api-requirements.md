...
> [!NOTE]
> Для представления на высоком уровне о том, Office надстройки в настоящее время поддерживаются, см. в Office клиентского приложения и доступности платформы для [Office](../overview/office-add-in-availability.md) надстройки.
...
Чтобы убедиться, что надстройка устанавливается в подмножество Office приложениях, используйте элементы [Hosts](../reference/manifest/hosts.md) и [Host](../reference/manifest/host.md) в манифесте.
...
> [!NOTE]
> Office поддерживаются на разных платформах и запускаются на настольных компьютерах, веб-браузерах, планшетах и мобильных устройствах. Обычно нельзя указать, какую платформу можно использовать для запуска надстройки. Например, если указать`Workbook`, Excel в Интернете и Windows можно использовать для запуска надстройки. Однако, если указать`Mailbox`, ваша надстройка не будет работать Outlook мобильных клиентов, если вы не определите точку [расширения для мобильных устройств](../reference/manifest/extensionpoint.md#mobilemessagereadcommandsurface).
...
- Второй параметр необязателен. Это строка, которая указывает минимальную версию набора требований, которую `if` Office приложение должно поддерживать для запуска кода в заявлении (например, "**1.9**"). Если не используется, предполагается версия "1.1".
...
В следующем примере кода проверяется, поддерживается ли Office приложение`document.setSelectedDataAsync`.
...
> [!TIP]
> Нет смысла повторять элемент **Requirement** в **versionOverrides** , который уже отображается в базовом манифесте. Если требование указано в базовом манифесте, надстройка не может установить там, где это требование не поддерживается, поэтому Office даже не разсмеяет элемент **VersionOverrides**. 
