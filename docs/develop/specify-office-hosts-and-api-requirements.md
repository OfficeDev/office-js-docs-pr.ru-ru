...
- Используйте API JavaScript для Office, доступные только в некоторых версиях Office. Например, версия единой покупки Excel 2016 поддерживает не все API, связанные с Excel, в библиотеке JavaScript для Office.
...
- У надстройки есть функция, которая требует использования метода API JavaScript для Office, который поддерживается в некоторых версиях приложения Office, например в подписке Excel, но не поддерживается в других, например для однофакторной покупки Excel 2016. Но у надстройки есть и другие функции, для которых требуются только методы  API JavaScript для Office, которые поддерживаются в Excel 2016. В этом сценарии надстройку необходимо установить на Excel 2016, но функция, требуемая для неподдерживаемого метода, должна быть скрыта от пользователей Excel 2016.
