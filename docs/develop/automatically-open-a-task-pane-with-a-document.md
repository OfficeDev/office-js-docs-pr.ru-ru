...
- Используйте функцию автоматического открытия, если она повысит эффективность работы ваших пользователей надстроек в подобных случаях:
...
- Разрешите пользователям включать и выключать функцию автоматического открытия. Добавьте параметр в пользовательский интерфейс, чтобы автоматическое открытие области задач надстройки можно было отключить.  
...
Вы можете пометить документ, чтобы активировать функцию автозапуска одним из двух способов. Выберите альтернативу, которая лучше всего подходит для вашего сценария.  
...
| **`storeType` Значение** | **`id` Значение**    |**`store` Значение** | **`version` Значение**|
|:---------------|:---------------|:---------------|:---------------|
|OMEX (AppSource)|Идентификатор ресурса AppSource для надстройки (см. примечание)|Языковой стандарт для AppSource (например, "ru-ru").|Версия каталога AppSource (см. примечание)|
...
|Registry (реестр системы)|GUID надстройки в ее манифесте.|"разработчик"|Версия в манифесте надстройки.|

> [!NOTE]
> Для поиска идентификатора ресурса и версии надстройки в AppSource перейдите на целевую страницу AppSource для соответствующей надстройки. Идентификатор ресурса отображается в адресной строке браузера. Версия указана в разделе **Сведения** на этой странице.
...
## <a name="test-and-verify-opening-taskpanes"></a>Тестирование и проверка открывающихся областей задач
...
```
Чтобы протестировать предыдущий пример, рассмотрите возможность присоединения к программе разработчика [Office 365](https://docs.microsoft.com/office/developer-program/office-365-developer-program) и подписки на учетную запись разработчика [Office 365](https://developer.microsoft.com/office/dev-program), если у вас еще нет этой подписки. Фактически вы можете протестировать Централизованное развертывание и убедиться, что ваша надстройка работает так, как ожидалось.


## <a name="see-also"></a>См. также

Для примера, показывающего, как использовать функцию автозапуска, см. [примеры команд надстройки Office](https://github.com/OfficeDev/Office-Add-in-Commands-Samples/tree/master/AutoOpenTaskpane). [Присоединяйтесь к программе разработчика Office 365](https://docs.microsoft.com/office/developer-program/office-365-developer-program). 
