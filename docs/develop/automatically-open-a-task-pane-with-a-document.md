...
ms.locfileid: "65244738"
---
# <a name="automatically-open-a-task-pane-with-a-document"></a>Автоматическое открытие области задач с документом

Вы можете использовать команды надстройки в надстройке Office, чтобы расширить пользовательский интерфейс Office путем добавления кнопок на Приложение Office ленты. Когда пользователи нажимают кнопки, выполняются различные операции (например, открывается область задач).
...
Вы можете протестировать предыдущий пример, используя подписку Microsoft 365, чтобы протестировать централизованное развертывание и убедиться, что надстройка работает должным образом. Если у вас еще нет подписки Microsoft 365, вы можете получить бесплатную 90-дневную возобновляемую подписку Microsoft 365, присоединившись к Microsoft 365 [разработчику](https://developer.microsoft.com/office/dev-program).
...
- [Присоединяйтесь к Microsoft 365 разработчика.](/office/developer-program/office-365-developer-program)
