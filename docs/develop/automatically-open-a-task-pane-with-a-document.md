...
Когда пользователь запускает надстройки, для которых не используются команды надстроек, например, надстройки, выполняемые в Office 2013, они вставляются в документ и сохраняются в этом документе. В результате, когда другие пользователи открывают документ, им предлагается установить надстройку, и откроется область задач. Проблема с этой моделью состоит в том, что во многих случаях пользователи не хотят, чтобы надстройка сохранялась в документе. Например, студент, использующий надстройку словаря в документе Word, может не попытаться установить эту надстройку при открытии документа в своих классматес или преподавателях.
...
- Не используйте функцию автоматического открытия для искусственного увеличения использования надстройки. Если вы не хотите, чтобы ваша надстройка автоматически открывалась с определенными документами, эта функция может раздражать пользователей.
