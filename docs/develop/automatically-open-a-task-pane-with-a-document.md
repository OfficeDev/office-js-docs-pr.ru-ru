...
description: Узнайте, как настроить надстройку Office для автоматического открытия документа.
ms.date: 07/07/2020
...
ms.locfileid: "49839959"
---
# <a name="automatically-open-a-task-pane-with-a-document"></a>Автоматическое открытие области задач с документом

Команды надстройки Office можно использовать для расширения пользовательского интерфейса Office путем добавления кнопок на ленту приложения Office. Когда пользователи нажимают кнопки, выполняются различные операции (например, открывается область задач).
...
- Используйте обнаружение набора требований, чтобы определить, доступна ли функция автоматического использования, и в случае ее нее обеспечить откат.
- Не используйте функцию автоматического открытия, чтобы искусственно увеличивать показатели использования надстройки. Если не имеет смысла автоматически открывать надстройки с определенными документами, эта функция может раздражать пользователей.
...
|EXCatalog (развертывание через Exchange Server) |GUID надстройки в ее манифесте.|EXCatalog. Строка EXCatalog — это строка для использования с надстройки, которые используют централизованное развертывание в Центре администрирования Microsoft 365.|Версия в манифесте надстройки.
...
Вы можете протестировать предыдущий пример, используя подписку на Microsoft 365, чтобы протестировать централизованное развертывание и убедиться, что надстройка работает ожидаемым образом. Если у вас еще нет подписки на Microsoft 365, вы можете получить бесплатную 90-дневную подписку на Microsoft 365, присоединившись к программе для разработчиков [Microsoft 365.](https://developer.microsoft.com/office/dev-program)
