...
В следующих разделах показаны элементы манифеста в том порядке, в котором они должны отображаться. Существуют различия в зависимости от того, имеет `type` ли атрибут `<OfficeApp>` элемента значение `TaskPaneApp`, `ContentApp`или. `MailApp` Чтобы эти разделы не стали слишком громоздкими, строго сложный `<VersionOverrides>` элемент разбивается на отдельные разделы.

> [!Note]
> Не все указанные элементы являются обязательными. Если `minOccurs` значение элемента равно **0** в [схеме](https://github.com/OfficeDev/office-js-docs-pr/tree/master/docs/overview/schemas), элемент является необязательным.
