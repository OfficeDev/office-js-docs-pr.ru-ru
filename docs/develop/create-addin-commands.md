...
JavaScript должен вызывать `Office.initialize` в HTML-файле, на который ссылается элемент **FunctionFile**. Элемент **FunctionName** (описание см. в разделе [Элементы управления "Кнопка"](https://docs.microsoft.com/office/dev/add-ins/reference/manifest/control#Button-control)) использует функции в элементе **FunctionFile**.
...
|**Items** <br/> |Обязательный. Содержит элементы **Item** для каждого элемента подменю. Каждый элемент **Item** содержит те же дочерние элементы, что и [Элементы управления ''Кнопка''](https://docs.microsoft.com/office/dev/add-ins/reference/manifest/control#Button-control).  <br/> |
