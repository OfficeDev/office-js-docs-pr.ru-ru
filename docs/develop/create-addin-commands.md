...
Используйте **[VersionOverrides](../reference/manifest/versionoverrides.md)** в манифесте для определения команд надстройки для Excel, PowerPoint и Word. Команды надстроек позволяют легко настроить пользовательский интерфейс Office по умолчанию, добавив конкретные элементы интерфейса, выполняющие действия. С помощью команд надстройки можно следующее:
...
> [!NOTE]
> В Outlook также поддерживаются команды надстроек. Дополнительные сведения см. [в дополнительных командах надстройки для Outlook](../outlook/add-in-commands-for-outlook.md)
...
Мы рекомендуем создать проект, следуя одному из быстрых стартов, например [сборке надстройки Excel области задач](../quickstarts/excel-quickstart-jquery.md). Каждое быстрое начало Excel, Word и PowerPoint создает проект, который уже содержит команду надстройки (кнопку), чтобы показать области задач. Убедитесь, что перед использованием команд надстройки Excel[, Word и PowerPoint](../design/add-in-commands.md) надстройки.
...
|**Requirements** <br/> |Необязательный параметр. Задает минимальные набор требований и версию библиотеки Office.js, необходимые надстройке. Этот элемент **требования к ребенку** переопределяет элемент **Requirements** в родительской части манифеста. Дополнительные сведения см. в [Office приложений и требований API](../develop/specify-office-hosts-and-api-requirements.md).  <br/> |
...
JavaScript должен вызывать `Office.initialize` в HTML-файле, на который ссылается элемент **FunctionFile**. Элемент **FunctionName** (описание см. в разделе [Элементы управления "Кнопка"](../reference/manifest/control-button.md)) использует функции в элементе **FunctionFile**.
...
|**Items** <br/> |Обязательный. Содержит элементы **Item** для каждого элемента подменю. Каждый элемент **Item** содержит те же дочерние элементы, что и [Элементы управления ''Кнопка''](../reference/manifest/control-button.md).  <br/> |
