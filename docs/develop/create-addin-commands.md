...
- Выполнять действия при выборе команды надстройки. Варианты действий:
  - Показать пользователю одну или несколько надстроек области задач. В надстройке области задач можно отобразить HTML-код, использующий Fluent пользовательского интерфейса для создания пользовательского интерфейса.
...
![Общие сведения об элементах команд надстройки в манифесте. Верхний узел здесь — VersionOverrides с дочерними узлами и ресурсами. В разделе "Узлы" находятся host, а затем DesktopFormFactor. В разделе DesktopFormFactor находятся FunctionFile и ExtensionPoint. В extensionPoint находятся CustomTab или OfficeTab и Office Menu. В разделе CustomTab или Office вкладка "Группировать", а затем "Управление действием". В Office меню выберите "Управление" и "Действие". В разделе Resources (дочерние объекты VersionOverrides) находятся изображения, URL-адреса, shortStrings и LongStrings.](../images/version-overrides.png)

## <a name="step-1-create-the-project"></a>Шаг 1. Создание проекта

Мы рекомендуем создать проект, следуя одному из кратких руководств, например [Excel](../quickstarts/excel-quickstart-jquery.md) надстройке области задач. Каждое краткое руководство для Excel, Word и PowerPoint создает проект, который уже содержит команду надстройки (кнопку) для отображения области задач. Прежде чем использовать команды надстройки, убедитесь, что Excel[, Word и PowerPoint](../design/add-in-commands.md) надстройки.

## <a name="step-2-create-a-task-pane-add-in"></a>Этап 2. Создание надстройки области задач

Чтобы начать использовать команды надстройки, необходимо сначала создать надстройку области задач, а затем изменить манифест надстройки, как описано в этой статье. С контентными надстройкими нельзя использовать команды надстроек. При обновлении существующего манифеста необходимо добавить соответствующие пространства имен **XML** , а также добавить элемент **VersionOverrides** в манифест, как описано в разделе "Шаг 3. Добавление [элемента VersionOverrides](#step-3-add-versionoverrides-element)".
...
|**Requirements** <br/> |Необязательное. Задает минимальные набор требований и версию библиотеки Office.js, необходимые надстройке. Этот **дочерний элемент Requirements** переопределяет элемент **Requirements** в родительской части манифеста. Дополнительные сведения см. [в Office приложениях и требованиях К API](../develop/specify-office-hosts-and-api-requirements.md).  <br/> |
...
Элемент **Hosts** содержит один или несколько элементов **Host**. Элемент **Host** указывает определенное Office приложения. Элемент **Host** содержит дочерние элементы, указывающие команды надстройки, отображаемые после установки надстройки в Office приложения. Чтобы отобразить одинаковые команды надстройки в двух или более разных Office приложениях, необходимо дублировать дочерние элементы в каждом **узле**.
...
JavaScript должен вызывать `Office.initialize` в HTML-файле, на который ссылается элемент **FunctionFile**. Элемент **FunctionName** (описание см. в разделе [Элементы управления "Кнопка"](/javascript/api/manifest/control-button)) использует функции в элементе **FunctionFile**.
...
|**OfficeTab** <br/> |Требуется, если вы хотите расширить вкладку ленты Приложение Office по умолчанию (с помощью **PrimaryCommandSurface**). Невозможно использовать элементы **OfficeTab** и **CustomTab** одновременно. <br/> Дополнительные значения вкладок для использования с атрибутом **id** см. в разделе "Значения табуляции" для Приложение Office [вкладок ленты](/javascript/api/manifest/officetab).  <br/> |
...
|**Items** <br/> |Обязательный. Содержит элементы **Item** для каждого элемента подменю. Каждый элемент **Item** содержит те же дочерние элементы, что и [Элементы управления ''Кнопка''](/javascript/api/manifest/control-button).  <br/> |
...
### <a name="tab-values-for-default-office-app-ribbon-tabs"></a>Значения вкладок для вкладок Приложение Office ленты по умолчанию
