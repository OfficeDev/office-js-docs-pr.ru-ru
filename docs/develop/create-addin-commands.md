...
![Обзор элементов команд надстройки в манифесте. Верхний узел — VersionOverrides с дочерними узлами и ресурсами. В разделе hosts — Host DesktopFormFactor. В разделе DesktopFormFactor — FunctionFile и ExtensionPoint. В разделе ExtensionPoint — CustomTab или OfficeTab, а также меню Office. В разделе CustomTab или на вкладке Office выберите элемент Управление затем действие. В разделе меню Office — управление действием. В разделе Resources (Child of VersionOverrides) находятся изображения, URL-адреса, ShortStrings и LongStrings.](../images/version-overrides.png)
...
Чтобы начать работу с командами надстроек, необходимо сначала создать надстройку области задач, а затем изменить манифест надстройки, как описано в этой статье. Вы не можете использовать команды надстроек с контентными надстройками. При обновлении существующего манифеста необходимо добавить соответствующие **пространства имен XML** , а также добавить элемент **VersionOverrides** в манифест, как описано в [шаге 3: Add VersionOverrides element](#step-3-add-versionoverrides-element).
...
|**Requirements** <br/> |Необязательный атрибут. Задает минимальные набор требований и версию библиотеки Office.js, необходимые надстройке. Этот элемент дочерних **требований** переопределяет элемент **требований** в родительской части манифеста. Дополнительную информацию можно узнать в статье [Указание приложений Office и требований к API](../develop/specify-office-hosts-and-api-requirements.md).  <br/> |
|**Hosts** <br/> |Обязательный. Задает коллекцию приложений Office. Дочерний элемент **Hosts** переопределяет элемент **Hosts** в родительской части манифеста. Необходимо включить атрибут **xsi:type**, для которого задано значение "Книга" или "Документ". <br/> |
