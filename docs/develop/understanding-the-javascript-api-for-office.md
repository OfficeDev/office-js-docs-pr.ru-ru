...
Но ваш код запуска не должен вызывать любой API Office.js, пока библиотека не будет полностью загружена. Существует два способа, с помощью которых ваш код может проверять, загружена ли библиотека полностью. Они описаны в следующих разделах: 
...
Сведения о различиях описанных ниже приемов см. в статье [Основные различия между Office.initialize и Office.onReady()](#major-differences-between-officeinitialize-and-officeonready). Дополнительные сведения о последовательности событий при инициализации надстройки см. в статье [Загрузка модели DOM и среды выполнения](loading-the-dom-and-runtime-environment.md).

### <a name="initialize-with-officeonready"></a>Инициализация с использованием Office.onReady()

`Office.onReady()` — это асинхронное метод, который возвращает объект обещания во время проверки, загружена ли библиотека Office.js полностью. Когда, и только когда, библиотека будет загружена, обещание сопоставляется в качестве объекта, который определяет хост-приложение Office с `Office.HostType` числовым значением (`Excel`, `Word` и т. д.) и платформу с `Office.PlatformType` числовым значением (`PC`, `Mac`, `OfficeOnline`и т.д.). Если библиотека уже загружена, когда вызывается `Office.onReady()`, тогда обещание сопоставляется незамедлительно.

Одним из способов вызова `Office.onReady()` подразумевает использование его в качестве метода обратного вызова. Пример:
...
Событие инициализации запускается, когда библиотека Office.js будет полностью загружена и готова к взаимодействия с пользователем. Вы можете назначить обработчик `Office.initialize` для реализации вашей логики инициализации. Например, приведенный ниже код проверяет, поддерживает ли версия Excel пользователя использование API, которые может вызывать надстройка.
...
Дополнительные сведения см. в статьях [Событие Office.initialize](https://docs.microsoft.com/javascript/api/office?view=office-js) и [Перечисление InitializationReason](https://docs.microsoft.com/javascript/api/office/office.initializationreason?view=office-js).

> [!NOTE]
> В настоящее время, вам необходимо задать `Office.Initialize` независимо от того, вызывается ли `Office.onReady()`. Если вы не используете `Office.Initialize`, как показано в приведенном ниже примере, можно задать пустую функцию.
> 
>```js
>Office.initialize = function () {};
>```
...
> [!NOTE]
> Даже если у вас нет логики запуска, вы можете назначить пустую функцию для `Office.initialize`, когда ваша надстройка загружает JavaScript, как показано в приведенном ниже примере. Некоторые комбинации хоста и платформы Office не загружают область задач до выполнения события инициализации и запуска функции обработчика указанного события.
> 
>```js
>Office.initialize = function () {};
>```
