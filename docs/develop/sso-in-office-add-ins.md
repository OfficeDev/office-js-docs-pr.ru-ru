...
7. Маркер — это как **маркер доступа**, так и **маркер удостоверений**. Его можно использовать в качестве маркера удостоверений для анализа и проверки утверждений о пользователе, таких как имя пользователя и адрес электронной почты.
...
Если надстройке необходимо проверить удостоверение пользователя, маркер доступа, возвращенный из `getAccessToken()`, содержит сведения, которые можно использовать для установления удостоверения. Ниже перечислены утверждения из маркера, связанные с удостоверениями.
